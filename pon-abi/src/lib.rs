#![doc = "Stable public ABI façade for Pon runtime embedders and generated code."]
#![doc = ""]
#![doc = "This crate intentionally contains no runtime implementation. It names the"]
#![doc = "object layouts, helper declarations, GC types, and version constants that"]
#![doc = "external code may depend on while the concrete implementation remains in"]
#![doc = "`pon-runtime` and `pon-gc`."]

use core::mem::{offset_of, size_of};

pub use pon_gc::{
    DEFAULT_HEAP_ALIGNMENT, FinalizeFn, GcTypeInfo, PreciseStackRootFn, TraceFn,
    TypeId as GcTypeId,
};
pub use pon_runtime::abi::{
    AbiTy, CodeInfo, FStrPartRaw, HandlerInfo, HelperDecl, ParamSpec, PyFrame,
    TStrPartRaw,
};
pub use pon_runtime::feedback::{FeedbackCell, FeedbackVec, TypeTag};
pub use pon_runtime::object::{
    BinaryFunc, CallFunc, DescrGetFunc, DescrSetFunc, GcMeta, GetAttrFunc,
    HashFunc, InitFunc, InquiryFunc, LenFunc, NewFunc, ObjObjArgProc, ObjObjProc,
    PyAsyncMethods, PyDunderSlots, PyMappingMethods, PyNumberMethods, PyObject,
    PyObjectHeader, PySequenceMethods, PyType, RichCmpFunc, SendFunc, SetAttrFunc,
    SSizeArgFunc, SSizeObjArgProc, TernaryFunc, UnaryFunc,
};
pub use pon_runtime::thread_state::PonThreadState;

/// ABI contract major version. Increment this for layout or symbol changes that
/// can break generated code or native embedders compiled against an older façade.
pub const ABI_VERSION_MAJOR: u16 = 0;
/// ABI contract minor version. Increment this for additive symbols/layout fields
/// that older consumers may ignore.
...
/// Every helper exported by `pon-runtime`, in the order used by codegen imports.

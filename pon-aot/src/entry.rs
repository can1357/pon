...
/// interner prefix before `pon_runtime_init` registers builtins, so embedded
/// ids name the same strings in the generated process.
pub fn define_aot_name_initializer(
...
/// Define exported `pon_module_main() -> PyObject*` as the runtime-facing AoT
/// ABI.
///
...
pub fn define_module_main_wrapper(
	module: &mut ObjectModule,
	body_id: FuncId,
) -> ModuleResult<FuncId> {
...
pub fn define_zero_arg_body_wrapper(
	module: &mut ObjectModule,
	body_id: FuncId,
	symbol: &str,
) -> ModuleResult<FuncId> {
...
pub fn define_aot_module_registrar(
	module: &mut ObjectModule,
	modules: &[EmbeddedModuleSpec],
) -> ModuleResult<FuncId> {
	let ptr_ty = module.target_config().pointer_type();
	...
	register_sig.params.push(AbiParam::new(ptr_ty));
	let register_id =
		module.declare_function(AOT_REGISTER_MODULE, Linkage::Import, &register_sig)?;
	...
	for (spec, body_ref) in modules.iter().zip(body_refs) {
		...
		builder
			.ins()
			.call(register_ref, &[name, len, is_package, body]);
	}

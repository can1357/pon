...
pub fn finish_to_object_file(module: ObjectModule, triple: &Triple, path: &Path) -> anyhow::Result<()> {
    let mut product = module.finish();
    stamp_macho_build_version(&mut product.object, triple);
    let bytes = product.emit().context("failed to serialize Cranelift object")?;

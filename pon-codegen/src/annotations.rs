...
//!
//! The baseline lowerer intentionally keeps annotations out of tier-0
//! semantics. This module reads the same source AST out-of-band, turns
//! representative Python annotations into the IR [`Type`] lattice, and provides
//! an opt-only AST scrubber so annotated code can still lower through the
//! existing boxed frontend.

use pon_ir::{
	LocalId, Type,
	lower::{LowerError, scope},
};
use ruff_python_ast::{
	ElifElseClause, Expr, ExprContext, ModModule, Parameters, Stmt, StmtAnnAssign, StmtAssign,
	StmtFunctionDef, StmtPass,
};

/// Annotation-derived type seeds for an entire lowered IR module.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct ModuleAnnotations {
	/// Function annotations in the same append order used by the existing
	/// lowerer. Index `0` is always the synthetic `__main__` body.
	pub functions: Vec<FunctionAnnotations>,
}

impl ModuleAnnotations {
	/// Return annotations for the IR function at `index` when the AST order
	/// still matches the lowered module.
	#[must_use]
...
///
/// Supported spellings are bare `int`, `float`, `bool`, and `str` names, plus
/// the same names written as string-literal forward annotations. `int` seeds
/// the speculative `IntI64` tier type so the Phase-D region planner can form an
...
///
/// Runtime annotation side effects are not part of the typed AoT acceptance
/// slice; this scrubber is deliberately only used by `pon build --opt`. Default
/// boxed AoT lowering continues to reject the same annotated constructs it
/// rejected before.
...
fn collect_function_annotations(
	...
	for stmt in body {
		...
		let child_scope = parent_scope
			...
			.ok_or_else(|| {
				LowerError::unsupported(format!("scope metadata missing for function `{}`", def.name))
			})?;
		...
		out.push(annotations_for_body(def.name.as_str(), &def.body, child_scope, Some(def)));
	}
	Ok(())
}
...
fn annotations_for_body(
	...
	let mut annotations = FunctionAnnotations {
		...
		locals:      Vec::new(),
		return_type: def
			.and_then(|def| def.returns.as_deref())
...
fn collect_parameter_annotations(
	...
	for parameter in parameters.iter() {
		...
		out.push(LocalAnnotation { slot: LocalId(slot), ty, source: AnnotationSource::Parameter });
	}
}

fn collect_ann_assigns(
	body: &[Stmt],
	scope_info: &scope::ScopeInfo,
	out: &mut Vec<LocalAnnotation>,
) {
...
fn push_ann_assign(
	...
	out.push(LocalAnnotation { slot: LocalId(slot), ty, source: AnnotationSource::AnnAssign });
}
	...
	match stmt {
		...
		Stmt::AnnAssign(assign) => {
			let replacement = if let Some(value) = assign.value.take() {
				...
				Stmt::Pass(StmtPass { node_index: assign.node_index.clone(), range: assign.range })
			};
			*stmt = replacement;
		},

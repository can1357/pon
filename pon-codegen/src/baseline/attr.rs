...
/// Lower `StoreAttr` through the runtime setter helper and return the stored value.
#[allow(clippy::too_many_arguments)]
pub(crate) fn lower_store_attr(
    ...
    let runtime_name = builder.ins().iconst(ir::types::I32, i64::from(names.runtime_id(name)?));
    let call = builder.ins().call(helpers.set_attr, &[obj, runtime_name, val]);
...
fn emit_status_check(builder: &mut FunctionBuilder<'_>, status: ir::Value, exception_exit: ir::Block) {
    let ok = builder.ins().icmp_imm(ir::condcodes::IntCC::SignedGreaterThanOrEqual, status, 0);
    let continue_block = builder.create_block();
    builder.ins().brif(ok, continue_block, &[], exception_exit, &[]);

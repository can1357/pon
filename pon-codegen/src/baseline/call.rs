...
const CODE_INFO_NAME_OFFSET: usize =
	core::mem::offset_of!(pon_runtime::abi::CodeInfo, name_interned);
const CODE_INFO_N_LOCALS_OFFSET: usize =
	core::mem::offset_of!(pon_runtime::abi::CodeInfo, n_locals);
const CODE_INFO_N_FEEDBACK_OFFSET: usize =
	core::mem::offset_of!(pon_runtime::abi::CodeInfo, n_feedback);
...
const PARAM_SPEC_TOTAL_OFFSET: usize =
	core::mem::offset_of!(pon_runtime::abi::ParamSpec, total_param_count);
const PARAM_SPEC_POSONLY_OFFSET: usize =
	core::mem::offset_of!(pon_runtime::abi::ParamSpec, positional_only_count);
const PARAM_SPEC_POS_OFFSET: usize =
	core::mem::offset_of!(pon_runtime::abi::ParamSpec, positional_count);
const PARAM_SPEC_KWONLY_OFFSET: usize =
	core::mem::offset_of!(pon_runtime::abi::ParamSpec, keyword_only_count);
const PARAM_SPEC_VARARGS_OFFSET: usize =
	core::mem::offset_of!(pon_runtime::abi::ParamSpec, varargs_name);
const PARAM_SPEC_VARKW_OFFSET: usize =
	core::mem::offset_of!(pon_runtime::abi::ParamSpec, varkw_name);
...
pub(crate) fn lower_make_function<M: Module>(
	...
	let runtime_name = builder
		.ins()
		.iconst(ir::types::I32, i64::from(names.runtime_id(name_interned)?));
...
pub(crate) fn lower_build_class<M: Module>(
	...
	let runtime_name = builder
		.ins()
		.iconst(ir::types::I32, i64::from(names.runtime_id(class.name.0)?));
	...
	for decorator in class.decorators.iter().rev().copied() {
		...
		let slot = builder.create_sized_stack_slot(StackSlotData {
			...
			size:        ptr_bytes
				.try_into()
				.map_err(|_| CodegenError::OffsetTooLarge { offset: ptr_bytes })?,
...
pub(crate) fn lower_make_function_full<M: Module>(
	...
	let func_id = *func_ids
		.get(func_index as usize)
		.ok_or(CodegenError::FunctionIndexOutOfRange { func_index })?;
	let code_info = declare_code_info(
		module,
		builder,
		func_id,
		target,
	...
	let kwdefault_count = builder
		.ins()
		.iconst(ptr_ty, function.kwdefaults.len() as i64);
	...
	let annotation_count = builder
		.ins()
		.iconst(ptr_ty, function.annotations.len() as i64);
	...
	let closure = build_closure_array(
		builder,
		helpers,
		state,
		function.closure,
...
fn declare_code_info<M: Module>(
	...
	let params_data =
		declare_param_spec_data(module, names_data, params, total_param_count, ptr_bytes)?;
	...
	put_u32(
		&mut bytes,
		CODE_INFO_N_LOCALS_OFFSET,
		u32::try_from(target.n_locals)
			.map_err(|_| CodegenError::OffsetTooLarge { offset: target.n_locals })?,
	);
...
fn declare_param_spec_data<M: Module>(
	...
	put_u32(
		&mut bytes,
		PARAM_SPEC_TOTAL_OFFSET,
		u32::try_from(total_param_count)
			.map_err(|_| CodegenError::OffsetTooLarge { offset: total_param_count })?,
	);
	put_u32(
		&mut bytes,
		PARAM_SPEC_POSONLY_OFFSET,
		u32::try_from(params.positional_only_count)
			.map_err(|_| CodegenError::OffsetTooLarge { offset: params.positional_only_count })?,
	);
	put_u32(
		&mut bytes,
		PARAM_SPEC_POS_OFFSET,
		u32::try_from(params.positional_count)
			.map_err(|_| CodegenError::OffsetTooLarge { offset: params.positional_count })?,
	);
	put_u32(
		&mut bytes,
		PARAM_SPEC_KWONLY_OFFSET,
		u32::try_from(params.keyword_only_count)
			.map_err(|_| CodegenError::OffsetTooLarge { offset: params.keyword_only_count })?,
	);
	put_u32(
		&mut bytes,
		PARAM_SPEC_VARARGS_OFFSET,
		params
			.vararg_name
			.as_deref()
			.map_or(0, pon_runtime::intern::intern),
	);
	put_u32(
		&mut bytes,
		PARAM_SPEC_VARKW_OFFSET,
		params
			.kwarg_name
			.as_deref()
			.map_or(0, pon_runtime::intern::intern),
...
fn build_closure_array(
	...
	for (index, cell) in closure.iter().enumerate() {
		let value =
			super::name::cell_object(builder, helpers, state, cell.0, ptr_ty, exception_exit)?;
		builder
			.ins()
			.stack_store(value, slot, offset_i32(index * ptr_bytes)?);
	}
...
fn build_kw_name_array(
	...
	for (index, (name, _)) in kwargs.iter().enumerate() {
		...
		builder
			.ins()
			.stack_store(runtime_name, slot, offset_i32(index * 4)?);
	}
...
mod tests {
	...
	fn code_info_layout_offsets_match_runtime_abi() {
		...
		assert_eq!(
			core::mem::size_of::<pon_runtime::abi::CodeInfo>(),
			core::mem::size_of::<*const u8>() * 2 + 16
		);

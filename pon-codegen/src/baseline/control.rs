...
use cranelift_codegen::ir::{
	self, AbiParam, FuncRef, InstBuilder, StackSlotData, StackSlotKind, TrapCode, condcodes::IntCC,
};
...
pub(crate) fn lower_terminator_with_blocks(
	...
	match term {
		...
		Terminator::Branch { cond, then_blk, else_blk } => lower_conditional_branch(
			builder,
			state,
			helpers,
			ptr_ty,
		...
		Terminator::CondBranch { cond, then_, else_ } => lower_conditional_branch(
			builder,
			state,
			helpers,
			ptr_ty,
		...
		Terminator::ForLoop { iter: _, body, done } => {
			...
			let stop_value =
				call_pyobject_helper(builder, helpers.gen_stop_value, &[], ptr_ty, exception_exit);
...
fn emit_backedge_instrumentation(
	builder: &mut FunctionBuilder<'_>,
	state: &LowerState,
	helpers: &HelperFuncRefs,
	ptr_ty: ir::Type,
...
fn emit_osr_poll_and_transfer(
	...
	for index in 0..pon_jit_osr_max_live() {
		builder
			.ins()
			.stack_store(null, slot, offset_i32(8 + index * ptr_bytes)?);
	}
	for local in 0..function.n_locals {
		...
		builder
			.ins()
			.stack_store(value, slot, offset_i32(8 + local * ptr_bytes)?);
	}
	for (index, value) in live_values.iter().enumerate() {
		...
		builder.ins().stack_store(
			boxed,
			slot,
			offset_i32(8 + (function.n_locals + index) * ptr_bytes)?,
		);
	}
...
fn lower_conditional_branch(
	...
	let then_route = if then_backedge {
		builder.create_block()
	} else {
		then_target
	};
	let else_route = if else_backedge {
		builder.create_block()
	} else {
		else_target
	};
	builder.ins().brif(truth, then_route, &[], else_route, &[]);
...
fn call_is_true(
	...
	builder
		.ins()
		.brif(failed, exception_exit, &[], continue_block, &[]);

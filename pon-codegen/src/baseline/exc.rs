...
// surrounding IR terminator decides whether control enters a handler or escapes.
...
/// Lower active-exception match test.  The runtime returns the current exception
/// on match and `None` on miss, preserving NULL as the error sentinel.
...
/// Lower representative `except*` split.  The runtime returns a matched group or
/// `None` when the active exception is not an exception group match.

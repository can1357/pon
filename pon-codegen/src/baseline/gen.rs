...
//!
//! 1. a **stub** at the function's declared `FuncId` with the normal
//!    `(argv, argc) -> obj` ABI that allocates a [`GenFrame`], stores the
//!    bound arguments into the frame's parameter slots, and wraps the frame
//!    in a generator/coroutine object without running any body code; and
//! 2. an anonymous **body** with the single-argument resume ABI
//!    `(frame) -> obj` (`GenResumeBodyFn`): the entry block loads
//!    `resume_state`, stores `RESUME_RUNNING`, reloads every local from its
//!    frame slot, and `br_table`-dispatches over the dense suspend states to
//!    IR block 0 (`RESUME_START`) or a resume block.  `Terminator::Suspend`
//!    spills all locals, stores its state number, and returns the yielded
//!    value; `Terminator::Return` routes through `pon_gen_finish`; the
//!    function-level exception exit routes through `pon_gen_unwind`
//!    (PEP 479 + slot zeroing live in the runtime helpers).
//!
...
fn store_frame_slot(
    ...
    builder.ins().store(MemFlagsData::new(), value, frame, offset);
...
pub(crate) fn compile_generator_function<M: Module>(
    ...
    compile_generator_body(module, helpers, func_ids, functions, names, ir_function, &mut body_ctx, fctx)?;
...
fn compile_generator_stub<M: Module>(
    ...
    let kind = builder.ins().iconst(ir::types::I8, i64::from(generator_kind_flag(ir_function)));
...
fn compile_generator_body<M: Module>(
    ...
    table_calls.push(builder.func.dfg.block_call(start_block, &[] as &[ir::BlockArg]));
    for target in &resume_targets {
        let target = target.ok_or(CodegenError::Unsupported("generator suspend states are not dense"))?;
        table_calls.push(builder.func.dfg.block_call(target, &[] as &[ir::BlockArg]));
    }
    let default_call = builder.func.dfg.block_call(exception_exit, &[] as &[ir::BlockArg]);
...
pub(crate) fn lower_suspend(
    ...
    let state_value = builder.ins().iconst(ir::types::I32, i64::from(suspend_state as i32));
    builder
        .ins()
...
pub(crate) fn lower_gen_resume_payload(
    ...
    Ok(call_pyobject_helper(builder, helpers.gen_consume_payload, &[ctx.frame], ptr_ty, exception_exit))
}
...
pub(crate) fn lower_gen_delegate_step(
    ...
    let call = builder.ins().call(helpers.gen_delegate_step, &[ctx.frame, delegate]);

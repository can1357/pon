...
impl TempSpillPlan {
    ...
    pub(crate) fn compute(
        ...
        let has_sites = function.blocks.iter().any(|block| {
            term_spill_point(&block.term) || block.insts.iter().any(|inst| inst_spill_point(&inst.kind))
        });
        if !has_sites {
            return Ok(None);
        }
...
/// - constant materialization and container/function/cell construction
///   from already-evaluated operands only allocate, and allocation never
///   collects in pon;
/// - local/cell/global moves touch frame variables or module dicts with
///   interned-string keys (no user hooks); `LoadName` is NOT excluded
///   because class bodies execute against user `__prepare__` mappings;
/// - `Is` is pointer identity; exact-list append/copy run no user code;
/// - exception-state bookkeeping (push/pop/reraise/`except*` phase tags)
///   moves already-constructed objects; `Raise`/match/check arms are spill
///   points (user exception `__init__`, metaclass `__instancecheck__`/
///   `__subclasscheck__`);
/// - `Yield`/`YieldFrom` are generator-transform inputs that codegen
///   rejects before any window could be emitted.

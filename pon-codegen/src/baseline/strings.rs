...
fn build_fstring_parts(
	...
	for (index, part) in parts.iter().enumerate() {
		...
		match part {
			FStrPart::Literal(_) => {
				return Err(CodegenError::Unsupported("BuildString literal ConstId data"));
			},
			FStrPart::Interp { value, conversion, format_spec } => {
...
fn build_template_parts<M: Module>(
	...
	for (index, part) in parts.iter().enumerate() {
		...
		match part {
			TStrPart::Literal(_) => {
				return Err(CodegenError::Unsupported("BuildTemplate literal ConstId data"));
			},
			TStrPart::Interp { value, expression, conversion, format_spec } => {
				let value = state.value(*value)?;
				let format_spec = match format_spec {
					Some(format_spec) => state.value(*format_spec)?,
					None => null,
				...
				let header = builder
					.ins()
					.iconst(ir::types::I64, i64::from(*conversion) << 32);
				stack_store(builder, header, slot, base + 24)?;
				stack_store(builder, format_spec, slot, base + 32)?;
			},
		}

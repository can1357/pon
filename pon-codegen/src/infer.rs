...
/// Run local annotation seeding and type inference over every function in
/// `module`.
pub fn infer_module_types(module: &mut Module, annotations: &ModuleAnnotations) {
	...
	for (index, function) in module.functions.iter_mut().enumerate() {
		let function_annotations = annotations
			.function(index)
			.filter(|ann| ann.name == function.name);
...
fn infer_inst(
	...
	match kind {
		...
		InstKind::Call { callee, args } => {
			call_type(result, *callee, args, values, value_names, range_values)
		},
...
fn arithmetic_type(op: BinOp, lhs: Type, rhs: Type) -> Type {
	match (op, lhs, rhs) {
		...
		(
			...
			lhs,
			rhs,
		) if numeric(lhs) && numeric(rhs) && (lhs == Type::Float || rhs == Type::Float) => Type::Float,
		_ => Type::Object,
	}
}
	...
	match value_names.get(&callee).map(String::as_str) {
		...
		Some("range")
			if (1..=3).contains(&args.len())
				&& args
					.iter()
					.all(|arg| value_type(values, *arg) == Type::IntI64) =>
		{

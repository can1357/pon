...
use cranelift_codegen::{
	Context,
	ir::{self, AbiParam, InstBuilder, MemFlagsData, condcodes::IntCC, types},
};
...
use pon_ir::{
	Type,
	ir::{
		BinOp, Block as IrBlock, BlockId, CmpOp, Function, Inst, InstKind, PyConst, Terminator, UnOp,
		Value as IrValue,
	},
};
...
use crate::{
	baseline::{
		self, CodegenError, HelperFuncRefs, LowerState, NameMap, control, spill::TempSpillPlan,
	},
	helpers::HelperRefs,
	region::{self, TypedInput, TypedRegion, TypedValue},
};
...
/// Build an optimizing plan for the largest currently typed region in
/// `function`.
///
...
pub fn can_compile_plan(plan: &OptimizingPlan) -> bool {
	plan
		...
		.values
		.iter()
		.all(|value| value.ty == Type::IntI64)
		&& plan
			.entry_guards
			.iter()
			.all(|guard| guard.expected == Type::IntI64)
		&& plan
			.fast_path
...
pub fn lowering_steps(plan: &OptimizingPlan) -> Vec<LoweringStep> {
	...
	steps.extend(
		plan
			...
			.iter()
			.copied()
			.map(LoweringStep::BoxedColdTwinCall),
	);
	steps.extend(
		plan
			.cold_twin
			...
			.iter()
			.cloned()
			.map(LoweringStep::StackMap),
	);
	steps
}

/// Lower the boxed cold twin for `region` by delegating every instruction to
/// the baseline sub-region hook.
///
/// This is not called by the public baseline path.  It exists so the future
/// optimizing entry point can share the exact boxed semantics with tier-0
/// codegen instead of forking helper calls, name remapping, NULL checks, or
/// local-slot rules.
#[allow(
	dead_code,
	clippy::too_many_arguments,
	reason = "Phase-D cold-twin lowering is reserved until typed tier entry wiring lands"
)]
...
/// guards and divide-by-zero guards side-exit to the cold twin after reboxing
/// any dirty unboxed locals, so unsupported shapes preserve boxed semantics
/// instead of trapping in the optimized path.
...
impl FastLowerState {
	...
	fn define_unboxed(&mut self, ir_value: IrValue, ty: Type, value: ir::Value) {
		self
			.unboxed_values
			.insert(ir_value, FastValue { ty, value });
	}
...
fn define_trusted_region_live_ins(
	...
	for input in &region.live_ins {
		if input.ty != Type::IntI64
			|| state.unboxed(input.value).is_some()
			|| !is_for_next_result(function, input.value)
		{
			continue;
		}
...
fn range_spec_from_args(
	args: &[IrValue],
	const_ints: &HashMap<IrValue, i64>,
) -> Option<RangeIterSpec> {
	match args {
		[stop] => Some(RangeIterSpec { start: 0, stop: *const_ints.get(stop)?, step: 1 }),
...
fn lower_cold_terminator_with_region_args(
	...
	match term {
		...
		Terminator::ForLoop { body, done, .. } if *body == region.entry => {
			...
			let stop_value = baseline::call_pyobject_helper(
				builder,
				helpers.gen_stop_value,
				&[],
				ptr_ty,
				exception_exit,
			);
	...
	match &inst.kind {
			...
			if region::inst_unboxed_type(inst) == Some(Type::IntI64) =>
		{
			let Some(value) = state
				.unboxed(*value)
				.filter(|value| value.ty == Type::IntI64)
...
fn ensure_fast_range_iter(
	...
	state
		.range_iters
		.insert(iter, FastRangeIter { current, initialized: false });
}
...
fn successor_flows_to_region_entry(
	function: &Function,
	successor: BlockId,
	...
	block_by_id(function, successor).is_some_and(
		|block| matches!(&block.term, Terminator::ForLoop { body, .. } if *body == region_entry),
	)
}
...
fn sync_for_baseline_inst(
	...
	match kind {
		InstKind::LoadLocal(slot) => {
			sync_dirty_local(builder, helpers, state, ptr_ty, exception_exit, slot.0)
		},
...
fn sync_locals_leaving_region(
	...
	if successors
		.iter()
		.any(|successor| !region.blocks.contains(successor))
	{
...
fn entry_guard(input: TypedInput) -> EntryGuard {
	EntryGuard { value: input.value, expected: input.ty, failure: GuardFailure::ColdTwin }
}
	...
	for block in &function.blocks {
		...
		for (inst_index, inst) in block.insts.iter().enumerate() {
			if boxed_cold_path_call(&inst.kind) {
				calls.push(ColdCallSite { block: block.id, inst_index, result: inst.result });
			}
		}
	}
	calls
	...
	calls
		...
		.filter_map(|call| {
			...
			for value in region.live_ins.iter().map(|input| input.value) {
				if last_uses
					.get(&value)
					.is_some_and(|last_use| *last_use >= call_position)
				{
			...
			Some(StackMapDecl { call: *call, boxed_values })
		})
		.collect()
}
	...
	for block in &function.blocks {
		...
		for (inst_index, inst) in block.insts.iter().enumerate() {
			let position = positions
				.get(&(block.id, inst_index))
				.copied()
				.unwrap_or(terminator_position);
...
fn flatten_region(function: &Function, region: &TypedRegion) -> Vec<FlatInst> {
	...
	for block in &function.blocks {
		...
		for (inst_index, _inst) in block.insts.iter().enumerate() {
			flattened.push(FlatInst { block: block.id, inst_index });
		}
	}
	flattened
}
...
"#,
		...
		let srem_result = clif
			...
			.find_map(|line| {
				line
					.trim_start()
					.split_once(" = srem")
					.map(|(result, _)| result.trim().to_owned())
			})
		...
		assert!(
			...
			"expected the loop backedge to carry the updated unboxed accumulator {srem_result}, \
			 got:\n{clif}"
		);
	}
	...
	fn optimized_load_local_reloads_boxed_local_for_cold_guard_path() {
		...
		let payload_offset =
			pylong_value_offset_i32(jit_module().target_config().pointer_type().bytes() as usize)
	...
	fn typed_region_exit_reboxes_stored_local() {
		...
		let (call_index, boxed_n, unboxed_n) = lines
			...
			.find_map(|(index, line)| {
				unboxed_n_values.iter().find_map(|unboxed| {
					line
						.split_once(&format!(" = call fn0({unboxed})"))
						.map(|(boxed, _)| (index, boxed.to_owned(), unboxed.to_owned()))
				})
			})
			.unwrap_or_else(|| {
				panic!(
					"expected the n < 2 arm to box the loaded local {payload_load} before the ternary \
					 merge, got:\n{clif}"
				)
			});
		...
		assert!(
			lines[..call_index].iter().any(
				|line| line.starts_with("brif ") && line.contains(&format!(", {true_side_block},"))
			),
			"expected the boxing call fn0({unboxed_n}) to be in the true successor of the n < 2 \
			 branch, got:\n{clif}"
		);
		...
		let guard_successor = lines[call_index + 1..]
			...
			.find_map(|line| {
				...
				successors
					.split_once(',')
					.map(|(successor, _)| successor.to_owned())
			})
		...
		assert!(
			...
			"expected the base-case arm to carry boxed n {boxed_n} from call fn0({unboxed_n}) to the \
			 ternary merge, not a bare iconst.i64 0, got:\n{clif}"
		);
	}
}

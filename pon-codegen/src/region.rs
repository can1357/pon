...
//! instructions already carry unboxable type metadata (or literal constants
//! whose unboxed representation is obvious) and whose CFG can be entered only
//! through the selected entry block.  That gives later tier-up/AoT work a
//! stable region contract without changing today's baseline codegen path.
...
use pon_ir::{
	Type,
	ir::{
		BinOp, BlockId, CmpOp, FStrPart, Function, Inst, InstKind, PyConst, TStrPart, Terminator,
		UnOp, Value as IrValue,
	},
};
...
///
/// Ties are resolved by the function's existing block layout, which keeps
/// region selection deterministic and makes the entry block stable for later
/// feedback plumbing.
#[must_use]
pub fn find_maximal_typed_region(function: &Function) -> Option<TypedRegion> {
	let compatible: Vec<bool> = function
		.blocks
		.iter()
		.map(|block| is_typed_block(block))
		.collect();
...
/// Return true when `kind` is in the Phase-D skeleton's unboxed fast-path
/// subset.
#[must_use]
pub fn is_fast_path_kind(kind: &InstKind) -> bool {
	matches!(
		...
		InstKind::Const(PyConst::Int(_) | PyConst::Float(_))
			...
			| InstKind::Not { .. }
			| InstKind::UnaryOp { op: UnOp::Neg | UnOp::Pos, .. }
	)
}
...
fn collect_compatible_reachable(
	function: &Function,
	compatible: &[bool],
	entry: BlockId,
) -> HashSet<BlockId> {
	...
	for block in &function.blocks {
		...
		for (inst_index, inst) in block.insts.iter().enumerate() {
			if let Some(ty) = inst_unboxed_type(inst) {
				values.push(TypedValue { value: inst.result, ty, block: block.id, inst_index });
			}
		}
	}
	values
	...
	for block in &function.blocks {
		...
		for (target, kind) in successor_edges(&block.term) {
			if !region_blocks.contains(&target) {
				let exit = RegionExit { from: block.id, to: target, kind };
...
fn fast_path_result_type(kind: &InstKind) -> Option<Type> {
	match kind {
		InstKind::Compare { .. } | InstKind::BoolTest { .. } | InstKind::Not { .. } => {
			Some(Type::IntI64)
		},
		_ => None,
	}
...
fn successor_edges(term: &Terminator) -> Vec<(BlockId, RegionExitKind)> {
	match term {
		...
		Terminator::Branch { then_blk, else_blk, .. } => {
			vec![(*then_blk, RegionExitKind::Branch), (*else_blk, RegionExitKind::Branch)]
		},
		Terminator::CondBranch { then_, else_, .. } => {
			vec![(*then_, RegionExitKind::Branch), (*else_, RegionExitKind::Branch)]
		},
		Terminator::ForLoop { body, done, .. } => {
			vec![(*body, RegionExitKind::ForLoopBody), (*done, RegionExitKind::ForLoopDone)]
		},
...
fn push_inst_operands(kind: &InstKind, operands: &mut Vec<IrValue>) {
	match kind {
		...
		InstKind::BuildTuple { elts }
		| InstKind::BuildList { elts }
		| InstKind::BuildSet { elts } => {
		...
		InstKind::CallEx { callee, args, star, kwargs, dstar } => {
			operands.push(*callee);
			operands.extend(args.iter().copied());
		...
		InstKind::MakeFunctionFull { defaults, kwdefaults, closure: _, annotations, .. } => {
...
mod tests {
	use pon_ir::{
		Type,
		ir::{BinOp, Block, BlockId, Function, Inst, InstKind, NameId, PyConst, Terminator, Value},
	};
	...
	fn finds_region_across_noncontiguous_successor_block_ids() {
		...
		assert_eq!(region.exits, vec![RegionExit {
			from: BlockId(30),
			to:   BlockId(40),

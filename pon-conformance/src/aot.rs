...
/// Static scripts that can be built as closed AoT units: no eval/exec, no
/// dynamic imports, and no extension-module dependency. Increase
/// `AOT_MIN_PASS_COUNT` and populate `AOT_FLOOR_MODULES` when the orchestrated
/// exit gate establishes a new green floor.
...
impl AotParityReport {
	...
	fn push(&mut self, record: AotParityRecord) {
		self.scoreboard.push(
			record.module.clone(),
			record.status.scoreboard_status(),
			record.detail.clone(),
		);
		...
		for record in &self.records {
			if record.status == AotParityStatus::Refused {
				let message = record
					.detail
					.as_deref()
					.unwrap_or("AoT compiler refused")
					.to_owned();
				buckets
					.entry(message)
					.or_default()
					.push(record.module.clone());
			}
		}
	...
	pub fn to_json(&self) -> String {
		...
		json.push_str("{\n");
		write!(json, "  \"suite\": \"{}\"", escape_json(&self.scoreboard.suite))
		...
		for (index, (message, modules)) in refusal_buckets.iter().enumerate() {
			let comma = if index + 1 == refusal_buckets.len() {
				""
			} else {
				","
			};
			...
			for (module_index, module) in modules.iter().enumerate() {
				let module_comma = if module_index + 1 == modules.len() {
					""
				} else {
					", "
				};
				write!(json, "\"{}\"{module_comma}", escape_json(module))
					.expect("write to String cannot fail");
			}
			write!(json, "] }}{comma}\n").expect("write to String cannot fail");
		}
	...
	for (index, (label, script)) in modules.into_iter().enumerate() {
		let Some(script) = script else {
			scoreboard.push(
				label,
				Status::Unsupported,
				Some("AoT subset script is not present".to_owned()),
			);
			continue;
		};
		...
		let reference = match suite::run_python314(root, &script) {
			...
			Err(error) => {
				scoreboard.push(
					label,
					Status::Fail,
					Some(format!("failed to run python3.14: {error:#}")),
				);
				continue;
			},
		};
		...
		let exe =
			run_dir.join(format!("{}-{}{}", index, safe_exe_stem(&label), env::consts::EXE_SUFFIX));
...
pub fn check_floor(scoreboard: &Scoreboard) -> ratchet::FloorCheck {
	let floor = ratchet::Floor {
		...
		passing_modules: AOT_FLOOR_MODULES
			.iter()
...
fn classify_aot_parity_module(
	...
	let Some(script) = script else {
		return AotParityRecord::new(
			...
			Some("manifest entry is not present".to_owned()),
		);
	};
	...
	let build = match run_build(root, pon_binary, &script, &exe, timeout) {
		...
		Err(error) => {
			return AotParityRecord::new(
				label,
				AotParityStatus::Error,
				Some(format!("failed to spawn pon build: {error:#}")),
			);
		},
	};
	if build.exit != 0 {
		if suite::is_unsupported_pon_output(build.exit, &build.stderr) {
			return AotParityRecord::new(
				label,
				AotParityStatus::Refused,
				Some(refusal_message(&build)),
			);
		}
		return AotParityRecord::new(
			label,
			AotParityStatus::Error,
			Some(build_failure_report(&exe, &build)),
		);
	}
	if !exe.is_file() {
		return AotParityRecord::new(
...
fn aot_modules(root: &Path, requested_modules: &[PathBuf]) -> Vec<(String, Option<PathBuf>)> {
	if requested_modules.is_empty() {
		return AOT_SUBSET
			.iter()
			.map(|module| resolve_aot_module(root, Path::new(module)))
			.collect();
	}
	requested_modules
		.iter()
...
fn resolve_aot_module(root: &Path, module: &Path) -> (String, Option<PathBuf>) {
	...
	if module.extension().is_none() {
		let cpython_test = PathBuf::from("Lib")
			.join("test")
			.join(format!("{}.py", module.to_string_lossy()));
...
"#;
	...
	fn no_class_module_executes_aot_body_not_build_class() {
		...
		let exe = fixture_dir
			.path()
			.join(format!("hello_no_class{}", std::env::consts::EXE_SUFFIX));
		let pon_binary = suite::ensure_pon(&root).expect("build pon for AoT regression");

		let build = match run_build(&root, &pon_binary, &script, &exe, aot_timeout())
			.expect("spawn pon build")
		{
			AotRun::Completed(build) => build,
		...
		let aot = match run_executable_without_pon(&root, &exe, aot_timeout())
			.expect("run no-class AoT executable")
		{

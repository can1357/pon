...
pub fn run_full_suite(root: &Path, opts: &FullSuiteOptions) -> Result<Scoreboard> {
    ...
    fs::create_dir_all(&scratch_base)
        .with_context(|| format!("failed to create scratch directory `{}`", scratch_base.display()))?;
    let ctx = ExecContext {
        ...
        for _ in 0..jobs {
            scope.spawn(|| {
                loop {
                    ...
                    let Some(unit) = selected.get(index) else { break };
    ...
    for (unit, slot) in selected.iter().zip(slots) {
        ...
        let (status, detail) = outcome.with_context(|| format!("failed to execute unit `{}`", unit.dotted))?;
...
fn select_units(units: &[Unit], selectors: &[String]) -> Result<Vec<Unit>> {
    ...
    Ok(chosen.into_iter().map(|index| units[index].clone()).collect())
}
...
fn check_ledger_staleness(ledger: &Ledger, units: &[Unit]) -> Result<()> {
    let patterns = ledger
        ...
        .chain(ledger.divergences.iter().map(|entry| ("divergence-ledger.toml", &entry.pattern)));
...
fn execute_unit(ctx: &ExecContext<'_>, unit: &Unit, unit_scratch: &Path) -> Result<(Status, Option<String>)> {
    ...
    let oracle = match run_with_timeout(&mut oracle_cmd, ctx.timeout) {
        ...
        Ok(RunOutcome::TimedOut) => {
            return Ok((Status::Unsupported, Some(format!("oracle: timed out after {timeout_secs}s"))));
        }
    ...
    let (oracle_vector, oracle_exit) = match parse_side(&oracle, &unit.dotted) {
        Err(why) => {
            return Ok((
                Status::Unsupported,
                Some(format!("oracle: outcome block malformed: {why}")),
            ));
        }
        Ok(parsed) => parsed,
    };
...
fn configure_side(cmd: &mut Command, ctx: &ExecContext<'_>, side_dir: &Path, path_var: &str) -> Result<()> {
...
fn run_with_timeout(cmd: &mut Command, timeout: Duration) -> std::io::Result<RunOutcome> {
    ...
    Ok(RunOutcome::Completed(SideResult {
        exit: status.code(),
        stdout,
        stderr,
    }))
...
fn classify_vectors(
    ...
    if let Some(entry) = matched_divergence {
        let covered = if entry.test_ids.is_empty() {
            ...
            differing.iter().all(|(id, _, _)| entry.test_ids.iter().any(|t| t == id))
        };
        if covered {
            return (
    ...
    let differ_count = differing.iter().filter(|(_, o, p)| *o != ABSENT && *p != ABSENT).count();
    ...
    let first = matched_divergence
        ...
        .and_then(|entry| {
            differing
                .iter()
                .find(|(id, _, _)| !entry.test_ids.iter().any(|t| t == id))
        })
    ...
    (
        Status::Fail,
        Some(format!(
            "outcome mismatch: {differ_count} id(s) differ, {missing_count} missing on pon, {extra_count} extra on pon; first: {} oracle={} pon={}",
...
mod tests {
    ...
    fn unit(dotted: &str) -> Unit {
        let key = dotted.strip_prefix("test.").expect("dotted names start with test.").to_owned();
    ...
    fn discovery_finds_vendored_units_sorted_and_named() {
        ...
        let test_root = root.join(crate::suite::CPYTHON_VENDOR_DIR).join("Lib").join("test");
        ...
        let grammar = units.iter().find(|unit| unit.dotted == "test.test_grammar").expect("top-level unit");
    ...
    fn selectors_accept_pinned_forms_and_reject_typos() {
        ...
        let dotted = select_units(&units, &["test.test_json.test_decode".to_owned()]).expect("dotted form");
        ...
        let union = select_units(&units, &["test_abc".to_owned(), "test_json*".to_owned()]).expect("union");
        ...
        assert!(error.to_string().contains("matches no vendored test module"));
    }

    #[test]
    fn shards_partition_the_unit_list() {
        let units = (0..10).map(|i| unit(&format!("test.test_{i:02}"))).collect::<Vec<_>>();
    ...
    fn outcome_block_parser_takes_last_block_and_validates() {
        ...
        assert!(parse_outcome_block(b"PONTEST BEGIN test.test_x\nPONTEST ok a\n", "test.test_x").is_err());
        assert!(parse_outcome_block(b"PONTEST BEGIN test.test_x\nPONTEST ok a\nPONTEST END 2\n", "test.test_x").is_err());
        assert!(parse_outcome_block(b"PONTEST BEGIN test.test_x\ngarbage\nPONTEST END 0\n", "test.test_x").is_err());
        assert!(parse_outcome_block(b"PONTEST BEGIN test.test_x\nPONTEST bogus a\nPONTEST END 1\n", "test.test_x").is_err());
        // Mismatched module name in BEGIN.
        assert!(parse_outcome_block(b"PONTEST BEGIN test.test_y\nPONTEST END 0\n", "test.test_x").is_err());
    }
    ...
    fn classification_follows_the_decision_table() {
        ...
        assert_eq!(classify_vectors(&base, 0, &base.clone(), 0, &target, &empty), (Status::Pass, None));
        ...
        assert_eq!(
            detail.as_deref(),
            Some("outcome mismatch: 1 id(s) differ, 1 missing on pon, 1 extra on pon; first: test.test_x.T.a oracle=ok pon=ERROR")
        );
        ...
        // Row 10 via §6.3: an uncovered differing id defeats the entry and is named first.

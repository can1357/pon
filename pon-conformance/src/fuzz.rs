...
//! `pon run` using the same subprocess hygiene as the CPython-full runner:
...
pub fn run_fuzz_suite(root: &Path, opts: &FuzzOptions) -> Result<FuzzSummary> {
	...
	fs::create_dir_all(&repro_dir)
		.with_context(|| format!("failed to create `{}`", repro_dir.display()))?;

	let cases = (0..opts.count)
		.map(|index| GeneratedCase { index, source: generate_program(opts.seed, index) })
		.collect::<Vec<_>>();
	...
	for worker in workers {
		worker
			.join()
			.map_err(|_| anyhow::anyhow!("fuzz worker panicked"))?;
	}
	...
	Ok(FuzzSummary { generated: opts.count, ran: opts.count, divergences })
}
...
fn observe_program(ctx: &ExecContext, source: &str, scratch: &Path) -> Result<Comparison> {
	...
	fs::create_dir_all(&oracle_dir)
		.with_context(|| format!("failed to create `{}`", oracle_dir.display()))?;
	fs::create_dir_all(&pon_dir)
		.with_context(|| format!("failed to create `{}`", pon_dir.display()))?;

	let oracle_script = oracle_dir.join(CASE_FILE);
	let pon_script = pon_dir.join(CASE_FILE);
	fs::write(&oracle_script, source)
		.with_context(|| format!("failed to write `{}`", oracle_script.display()))?;
	fs::write(&pon_script, source)
		.with_context(|| format!("failed to write `{}`", pon_script.display()))?;
	...
	let oracle = run_with_timeout(&mut oracle_cmd, ctx.timeout)
		.context("failed to run python3.14 fuzz case")?;
...
impl Observation {
	fn from_process(output: ProcessOutput) -> Self {
		...
		Self { stdout: output.stdout, stderr: output.stderr, stderr_class, exit: output.exit }
	}
}
...
fn save_repro(
	...
	fs::write(&minimized_path, minimized)
		.with_context(|| format!("failed to write `{}`", minimized_path.display()))?;
	...
	fs::write(ctx.repro_dir.join(format!("{stem}.summary.txt")), mismatch_summary(comparison))?;
	Ok(SavedRepro { index, original_path, minimized_path })
}
...
fn minimize_divergence(ctx: &ExecContext, index: usize, source: &str) -> Result<String> {
	...
	minimize_source(source, |candidate| {
		...
		let scratch = ctx
			.temp_dir
			.join(format!("case-{index:04}-shrink-{attempt:03}"));
...
fn emit_descriptors(rng: &mut SplitMix64) -> String {
	...
	format!(
		"# PONFUZZ BEGIN descriptors\nclass FuzzDesc:\n    def __init__(self, bias):\n        \
		 ...
		 __init__(self, value):\n        self.value = value\n    def bump(self):\n        self.item \
		 = self.item + 3\n        return self.item\nfbox = FuzzBox({start})\nprint('desc', \
		 fbox.item, fbox.bump(), fbox.value)\n# PONFUZZ END descriptors\n"
	)
}
...
fn emit_match(rng: &mut SplitMix64) -> String {
	...
	format!(
		"# PONFUZZ BEGIN match\nmsubj = {{'tag': {tag}, 'items': [{item}, {item} + 1, {item} + \
...
fn run_with_timeout(cmd: &mut Command, timeout: Duration) -> std::io::Result<ProcessOutput> {
	...
	loop {
		...
		if Instant::now() >= deadline {
			...
			let stdout = finish_reader(stdout_reader, &stdout_buf);
			let stderr = finish_reader(stderr_reader, &stderr_buf);
			return Ok(ProcessOutput { stdout, stderr, exit: TIMEOUT_EXIT, timed_out: true });
		}
...
mod tests {
	...
	fn generated_programs_include_marked_printing_feature_chunks() {
		...
		assert!(
			begin_markers >= 2,
			"generated source should carry multiple marked feature chunks:\n{source}"
		);
		assert_eq!(begin_markers, end_markers, "feature chunk markers should be paired:\n{source}");
		assert!(
			source.contains("print("),
			"generated source should emit observable output:\n{source}"
		);
	}
...
";
		...
		let minimized =
			minimize_source(&joined, |candidate| Ok(candidate.contains("print('try', result)")))
				.expect("predicate is infallible");

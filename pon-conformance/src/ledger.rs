...
impl Ledger {
	...
	pub fn exclusion_for(&self, key: &str, stem: &str) -> Option<&ExcludeEntry> {
		self
			.exclusions
			.iter()
			.find(|entry| pattern_matches_unit(&entry.pattern, key, stem))
	}
...
pub fn load_ledger(root: &Path) -> Result<Ledger> {
	...
	let divergences = parse_divergences(&divergence_text)
		.with_context(|| format!("failed to load `{}`", divergence_path.display()))?;
	...
	let exclusions = parse_exclusions(&exclusions_text)
		.with_context(|| format!("failed to load `{}`", exclusions_path.display()))?;
...
/// A pattern matches a unit iff it matches its match key *or* its stem (pin
/// §7.3).
	...
	for (index, raw_line) in text.lines().enumerate() {
		...
		if line.starts_with("[[") {
			if line != expected_header {
				bail!(
					"line {line_no}: unsupported table header `{line}` (only `{expected_header}` is \
					 allowed)"
				);
			}
			tables.push(TomlTable { line: line_no, fields: Vec::new() });
			continue;
		}
...
/// Parses one `"..."` with `\"` and `\\` escapes only; returns (text, bytes
/// consumed).
	...
	while let Some((index, character)) = chars.next() {
		match character {
			...
			'\\' => match chars.next() {
				...
				Some((_, '\\')) => parsed.push('\\'),
				Some((_, other)) => {
					bail!("line {line_no}: unsupported escape `\\{other}` (only \\\" and \\\\)")
				},
...
/// Parses one single-line `["a", "b"]` string array; returns (items, bytes
/// consumed).
fn parse_toml_string_array(text: &str, line_no: usize) -> Result<(Vec<String>, usize)> {
	...
	loop {
		...
		match bytes.get(index) {
			...
			_ => bail!(
				"line {line_no}: string array must contain only double-quoted strings on one line"
			),
		}
	}
}
...
fn parse_divergences(text: &str) -> Result<Vec<DivergenceEntry>> {
	...
	const KNOWN_KEYS: &[&str] =
		&["pattern", "reason", "cpython", "pon", "test_ids", "approved_by", "note"];
	tables
		.iter()
		.map(|table| {
			...
			let Some(reason) = DivergenceReason::parse(&reason_text) else {
				bail!(
					"line {}: reason `{reason_text}` is outside the closed taxonomy \
	...
	tables
		...
		.map(|table| {
			...
			let Some(reason) = ExcludeReason::parse(&reason_text) else {
				bail!(
					"line {}: reason `{reason_text}` is outside the closed taxonomy (c-abi-boundary | \
					 platform-inapplicable | interpreter-internals)",
					table.line
				);
			};
			Ok(ExcludeEntry {
...
"#;
	...
	fn checked_in_ledgers_load_and_carry_seed_exclusions() {
		...
		let patterns = ledger
			...
			.collect::<Vec<_>>();
		// The four J0-pin seed entries stay first, in file order; wave entries append
		// after them.
		assert!(
		...
		assert!(
			ledger
				.exclusion_for("test_capi.test_abstract", "test_abstract")
				.is_some()
		);
		// Wave-6 (owner-approved 2026-07-03) rows are carried too.
		assert!(
			ledger
				.exclusion_for("test_xxlimited", "test_xxlimited")
				.is_some()
		);
		assert!(
			ledger
				.exclusion_for("test_grammar", "test_grammar")
				.is_none()
		);
	}
}

...
/// Floor file for `--suite cpython-full` (workspace root, sibling of
...
impl FloorCheck {
	...
	pub fn message(&self) -> String {
		...
		if self.pass_count < self.min_pass_count {
			parts.push(format!("pass count {} below floor {}", self.pass_count, self.min_pass_count));
		}
		if parts.is_empty() {
...
pub fn write_floor_from_scoreboard(
	root: &Path,
	cpython_tag: &str,
...
/// `write_floor_from_scoreboard` twin for an arbitrary floor file name under
/// `root`.
pub fn write_floor_from_scoreboard_at(
	root: &Path,
	file: &str,
	cpython_tag: &str,
	scoreboard: &Scoreboard,
...
fn parse_floor(text: &str) -> Result<Floor> {
	...
	Ok(Floor { cpython_tag, passing_modules, min_pass_count })
}
...
fn field_value<'a>(text: &'a str, key: &str) -> Result<&'a str> {
	...
	let colon = after_key
		.find(':')
		.with_context(|| format!("missing colon after field `{key}`"))?;
			...
			b'\\' => {
				...
				match bytes.get(index).copied() {
					...
					Some(b'u') => {
						...
						let character =
							char::from_u32(code).context("unicode escape is not a scalar value")?;
...
mod tests {
	...
	fn synthetic_floor_regression_catches_module_and_count_drops() {
		...
		scoreboard.push(
			...
			Some("different stdout".to_owned()),
		);

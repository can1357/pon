...
pub fn run_slice_suite(root: &Path, requested_modules: &[PathBuf]) -> Result<Scoreboard> {
	...
	for script in scripts {
		let pon = run_pon(root, &pon_binary, &script)
			.with_context(|| format!("failed to run pon for `{}`", script.display()))?;
		let reference = match mode {
			ReferenceMode::Python314 => run_python314(root, &script)
				.with_context(|| format!("failed to run python3.14 for `{}`", script.display()))?,
		...
		requested_modules
			...
			.map(|module| {
				...
				(
					...
					script,
					Status::Unsupported,
					"vendored CPython module is not present".to_owned(),
				)
			})
			.collect::<Vec<_>>()
	};
	...
	for (label, script, missing_status, missing_detail) in modules {
		...
		if pon == reference {
			...
			scoreboard.push(
				label.clone(),
				Status::Unsupported,
				Some(mismatch_report(&label, &pon, &reference)),
			);
		} else {
			scoreboard.push(
				label.clone(),
...
pub(crate) fn ensure_pon(root: &Path) -> Result<PathBuf> {
	let output = Command::new("cargo")
		...
		.current_dir(root)
		.output()
		.context("failed to spawn cargo build for pon")?;
...
fn built_in_golden(script: &Path) -> Result<RunResult> {
	...
	Ok(RunResult { stdout, stderr: Vec::new(), exit: 0 })
}
...
/// The pin's `is_unsupported_pon_result` predicate on raw exit/stderr: a
/// non-zero exit whose stderr mentions "unsupported" (ASCII
/// case-insensitively).

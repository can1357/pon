...
///
/// Blocks are stored in layout order; `blocks[0]` is the entry block.
/// Positional parameters occupy the leading local slots; [`ParamLayout`]
/// records the full call-binding order for keyword-only, `*args`, and
/// `**kwargs` slots.
#[derive(Clone, Debug, PartialEq)]
pub struct Function {
	...
	pub arity:              usize,
	/// True when this function was produced from `async def` and calls must
	...
	/// body (PEP 525): calls must return an async-generator object driven
	/// through `__anext__`/`asend` awaitables rather than an awaitable
	/// coroutine.
...
impl ParamLayout {
	...
	pub fn total_slot_count(&self) -> usize {
		self.names.len()
			+ usize::from(self.vararg_name.is_some())
			+ usize::from(self.kwarg_name.is_some())
	}
...
pub enum InstKind {
	...
	/// Update a set with values produced by an iterable for `*` set-display
	/// unpacking.
	...
	/// Produce the stashed `StopIteration.value` of the last finished
	/// delegation.
	GenLastStopValue,
	...
	PushExcInfo {
		/// Handler block used by the boxed codegen NULL-sentinel edge while this
		/// handler is active.
		target:      BlockId,
	...
	/// Legacy representative `except*` split; retained until helper-table
	/// consumers migrate.
	...
	/// Attach a synthesized PEP 649 `__annotate__` function to a function
	/// object.
	FunctionSetAnnotate {
...
/// `Branch`, and `Unreachable` reserve the CFG shape used by later phases and
/// by internal placeholder blocks before lowering is finalized.
#[non_exhaustive]
#[derive(Clone, Debug, PartialEq)]
pub enum Terminator {

...
//! `lower_module`) in place while introducing a real driver boundary.  The
//! driver performs scope analysis first, then routes every statement/expression
//! family through a named module.  Most Phase-B families intentionally return
//! precise `LowerError::Unsupported` values today; the routing exists so future
//! semantic work lands in the right family instead of expanding a single
//! monolithic match.
...
use ruff_python_ast::{
	Expr, ExprContext, ModModule, Number, Parameters, Stmt, StmtAssign, StmtFunctionDef,
...
impl LowerError {
	...
	pub fn unsupported(feature: impl Into<String>) -> Self {
		Self::Unsupported { feature: feature.into(), span: None }
	}

	fn unsupported_at(feature: impl Into<String>, span: SourceSpan) -> Self {
		Self::Unsupported { feature: feature.into(), span: Some(span) }
	}
...
/// Lower a Ruff module AST into IR while preserving the Phase-A executable
/// slice.
///
...
/// form (`eval(...)`, `exec(...)`, `compile(...)`) that lowers through a
/// builtin name load followed by `Call`. Indirect access through `getattr`,
/// rebinding, or imports remains a runtime AoT-boundary check.
#[must_use]
		...
		if let Expr::Call(call) = expr {
			if let Expr::Name(callee) = call.func.as_ref() {
				if let Some(kind) = DynamicCodeKind::from_name(callee.id.as_str()) {
					self
						.sinks
						.push(DynamicSink { kind, span: span_expr(call.func.as_ref()) });
				}
			}
		}
...
impl LoweringDriver {
	...
	fn lower_module(mut self, module: &ModModule) -> Result<Module, LowerError> {
		...
		if let Some((annotate_info, entries)) =
			synth::claim_namespace_annotate(&mut body, &module.body)?
		{
			let annotate =
				synth::synthesize_annotate_scope(&mut self, &mut body, annotate_info, &entries)?;
		...
		Ok(Module { functions: self.functions, main, names: self.names.names })
	}
	...
	fn lower_stmt_with_loop(
		...
		match stmt {
			...
			Stmt::IpyEscapeCommand(stmt) => unsupported_at(
				"IPython escape command",
	...
	fn lower_for_stmt(
		...
		let nested_targets =
			control::LoopTargets { break_block: done_block, continue_block: header_block };
		...
		match expr {
				...
				if scope.is_class() && !scope.is_global_name(raw_name) {
					...
					match scope.name_class(raw_name) {
						Some(NameClass::Local { slot }) => {
							scope.emit(InstKind::LoadLocal(LocalId(*slot)))
						},
			...
			Expr::NumberLiteral(literal) => match &literal.value {
				Number::Int(value) => match value.as_i64() {
					...
					None => {
						scope.emit(InstKind::Const(PyConst::BigInt(value.to_string().into_boxed_str())))
					},
				},
				Number::Float(value) => scope.emit(InstKind::Const(PyConst::Float(*value))),
				Number::Complex { real, imag } => {
					scope.emit(InstKind::Const(PyConst::Complex { real: *real, imag: *imag }))
				},
			},
			...
			Expr::Starred(_) => {
				unsupported_expr("starred expression outside container literal or call", expr)
			},
	...
	fn lower_sequence_store_target(
		...
		if let Some(starred_index) = starred_index {
			...
			let unpacked = scope.emit(InstKind::UnpackSeq { val: value, n: elts.len() })?;
			for (index, elt) in elts.iter().enumerate() {
				let item = self.lower_sequence_item(scope, unpacked, index as i64)?;
				self.lower_store_target(scope, elt, item)?;
			}
...
impl BodyScope {
	...
	fn emit_cell_prologue(&mut self) {
		for index in 0..self.info.cell_vars.len() {
			...
			let (local_slot, is_parameter) = match known_local {
				...
				None if self.info.needs_class_closure && self.info.cell_vars[index] == "__class__" => {
			...
			if !is_parameter {
				...
				self
					.insts
					.push(Inst::new(none, InstKind::Const(PyConst::None)));
			...
			self
				.insts
				.push(Inst::new(result, InstKind::MakeCell(LocalId(local_slot))));
		}
	}
	...
	fn cell_slot_for_local(&self, local: LocalId) -> Option<CellId> {
		for index in 0..self.info.cell_vars.len() {
			...
			if let Some(NameClass::Cell { local_slot, cell_slot }) = self.name_class(name) {
	...
	fn switch_to(&mut self, id: BlockId) -> Result<(), LowerError> {
		...
		self.blocks.push(Block { id: self.current_id, insts, term });
		self.current_id = id;
		Ok(())
	}
	...
	fn reserve_feedback_slot(
		&mut self,
		kind: &InstKind,
	) -> Result<Option<FeedbackSlot>, LowerError> {
	...
	fn finish(mut self) -> Result<Function, LowerError> {
		...
		self
			.blocks
			.push(Block { id: self.current_id, insts: self.insts, term });
...
fn span_expr(expr: &Expr) -> SourceSpan {
	match expr {
		...
		Expr::StringLiteral(node) => {
			span_bounds(node.range.start().to_u32(), node.range.end().to_u32())
		},
...
"#,
		...
		assert_eq!(main_block.insts[3].kind, InstKind::Const(PyConst::Str("hello".to_owned())));
		...
		assert_eq!(main_block.insts[7].kind, InstKind::Call {
			callee: Value(4),
			args:   vec![Value(5), Value(6)],
		});
		assert_eq!(main_block.insts[8].kind, InstKind::Call {
			callee: Value(2),
		...
		assert_eq!(add_block.insts[2].kind, InstKind::BinaryOp {
			op:  BinOp::Add,
			lhs: Value(0),
			rhs: Value(1),
		});
...
"#,
		...
		let InstKind::MakeFunctionFull { code, defaults, kwdefaults, closure, annotations } =
		...
		assert!(defaults.is_empty(), "plain closure should not synthesize positional defaults");
		assert!(kwdefaults.is_empty(), "plain closure should not synthesize keyword defaults");
		assert!(annotations.is_empty(), "plain closure should not synthesize annotations");
...
"#,
		...
		assert!(main.blocks[0].insts.iter().any(|inst| match inst.kind {
			InstKind::MakeFunctionFull { code, .. } =>
				module.functions[code.0 as usize].name == "<genexpr>",
			InstKind::MakeFunction { func_index, .. } =>
				module.functions[func_index as usize].name == "<genexpr>",
			_ => false,
		}));
		assert!(
			main.blocks[0]
...
"#,
		...
		assert!(function.is_generator, "async bodies must be resumable state machines");
		assert!(
			function.blocks.first().is_some_and(|block| matches!(
				block.insts.first().map(|inst| &inst.kind),
				Some(InstKind::GenResumePayload)
			)),
...
"#,
		...
		assert_eq!(sinks.iter().map(|sink| sink.kind).collect::<Vec<_>>(), vec![
			DynamicCodeKind::Eval,
			DynamicCodeKind::Exec,
			DynamicCodeKind::Compile
		]);
...
"#,
		...
		assert!(listcomp.is_coroutine, "an async comprehension body must run as a coroutine");
		assert!(
		...
		assert!(
			...
			"an async first clause must acquire the outer iterator with GetAIter in the enclosing \
			 scope"
		);
		assert!(
...
"#,
	...
	fn rejects_async_constructs_in_unsupported_scopes_with_exact_features() {
		...
		for (source, expected_feature) in cases {
			let err = lower_source(source).expect_err("source should be rejected during lowering");
			match err {
...
"#,
		...
		for genexpr in genexprs {
			assert!(genexpr.is_async_generator, "an async genexpr child must be an async generator");
...
"#,
		...
		let listcomps: Vec<_> = module
			...
			.collect();
		assert_eq!(listcomps.len(), 2, "both comprehension levels should synthesize child functions");

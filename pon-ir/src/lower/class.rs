...
pub(super) fn lower_class_def(
	...
	if let Some((annotate_info, entries)) = namespace_annotate {
		if !body.is_terminated() {
			let annotate =
				synth::synthesize_annotate_scope(driver, &mut body, annotate_info, &entries)?;
	...
	if let Some(arguments) = stmt.arguments.as_deref() {
		let has_starred_base = arguments
			.args
			.iter()
			.any(|arg| matches!(arg, Expr::Starred(_)));
		let has_dstar = arguments
			.keywords
			.iter()
...
mod tests {
	...
	/// Every instruction kind of `func` across all of its blocks, in layout
	/// order.
	fn inst_kinds(func: &Function) -> impl Iterator<Item = &InstKind> {
		func
			.blocks
			.iter()
			.flat_map(|block| block.insts.iter().map(|inst| &inst.kind))
	}
...
"#,
		...
		if let InstKind::BuildClass { bases, bases_seq, keywords, dstar, .. } =
			&main.blocks[0].insts[6].kind
		{
...
"#,
		...
		assert!(
			insts[..at]
				.iter()
				.any(|inst| matches!(inst.kind, InstKind::ListExtend { .. })),
...
"#,
		...
		assert_ne!(
			index, main_index,
			"UnpackSeq belongs to synthesized __annotate__, not the module body"
		);
		assert!(

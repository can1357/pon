...
pub(super) fn lower_function_def_stmt(
	...
	let function_info =
		scope.next_child_scope(ScopeKind::Function, name, Some(scope::span_key(def.range)))?;
	...
	for decorator in def.decorator_list.iter().rev() {
		...
		value = scope.emit(InstKind::Call { callee, args: vec![value] })?;
	}
...
pub(super) fn lower_call(
	...
	if star.is_none() && kwargs.is_empty() && dstar.is_none() {
		...
		scope.emit(InstKind::CallEx { callee, args, star, kwargs, dstar })
	}
}
...
pub(super) fn lower_lambda(
	...
	let lambda_info = scope.next_child_scope(
		ScopeKind::Function,
		"<lambda>",
		Some(scope::span_key(lambda.range)),
	)?;
...
"#,
		...
		assert!(main.blocks[0].insts.iter().any(
			|inst| matches!(inst.kind, InstKind::MakeFunctionFull { ref defaults, ref kwdefaults, .. }
                if defaults.len() == 1 && kwdefaults.len() == 1)
		));
		let decorator_call = main.blocks[0]
			.insts
			.iter()
		...
		assert!(matches!(target.blocks[0].insts[2].kind, InstKind::BinaryOp { op: BinOp::Add, .. }));
	}

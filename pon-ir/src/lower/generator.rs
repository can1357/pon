...
pub(super) fn emit_async_for_step(
	...
	let method = scope.emit(InstKind::LoadAttr { obj: aiter, name: anext })?;
	let awaitable = scope.emit(InstKind::Call { callee: method, args: Vec::new() })?;
	let iter = scope.emit(InstKind::Await { awaitable })?;
	...
	scope.set_term(Terminator::CondBranch { cond, then_: exhausted, else_: miss_block })?;
...
fn block_entry_handler_stacks(
	...
	fn merge(
		...
		if let Some(existing) = entries.get(&block) {
			...
			return Err(LowerError::internal("inconsistent handler stack at generator block join"));
		}
...
/// - Every SSA value used outside its defining block is spilled to a fresh temp
///   local (store after def, load before use), so codegen's whole-frame
		...
		match suspend_inst.kind {
			InstKind::Yield { val } => {
				...
				result.push(Block {
					id,
					insts: head_insts,
					term: Terminator::Suspend { state, val, resume: resume_blk },
				});

				let mut resume_insts = Vec::with_capacity(active_handlers.len() + 1 + tail_insts.len());
				for spec in &active_handlers {
					resume_insts.push(Inst::new(ids.value()?, InstKind::PushExcInfo {
						target:      spec.target,
				...
				pending.push_front(Block { id: resume_blk, insts: resume_insts, term });
			},
			InstKind::YieldFrom { iter } => {
				...
				head_insts.push(Inst::new(ids.value()?, InstKind::StoreLocal(delegate_slot, iter)));
				result.push(Block { id, insts: head_insts, term: Terminator::Jump(loop_blk) });
				...
				let loop_block = Block {
					...
					term:  Terminator::ForLoop { iter: delegate, body: yield_blk, done: done_blk },
				};
				...
				let yield_block = Block {
					id:    yield_blk,
					insts: yield_insts,
					term:  Terminator::Suspend { state, val: step, resume: resume_blk },
				};
				...
				let resume_block =
					Block { id: resume_blk, insts: resume_insts, term: Terminator::Jump(loop_blk) };

				let mut done_insts = vec![Inst::new(suspend_inst.result, InstKind::GenLastStopValue)];
				done_insts.extend(tail_insts);
				let done_block = Block { id: done_blk, insts: done_insts, term };
...
fn prepend_entry_payload(function: &mut Function, ids: &mut GenIds) -> Result<(), LowerError> {
	...
	entry
		.insts
		.insert(0, Inst::new(result, InstKind::GenResumePayload));
	Ok(())
}
...
fn localize_cross_block_values(
...
fn debug_assert_suspend_numbering(function: &Function) {
	if cfg!(debug_assertions) {
		...
		for (index, state) in states.iter().enumerate() {
			assert_eq!(*state, index as u32 + 1, "generator suspend states must be dense 1..=N");
		}
	}
}
...
fn rewrite_operands(kind: &mut InstKind, f: &mut impl FnMut(&mut Value)) {
	match kind {
		...
		InstKind::BuildClass { bases, bases_seq, keywords, dstar, decorators, .. } => {
			for base in bases {
				f(base);
			}
		...
		InstKind::MakeFunctionFull { defaults, kwdefaults, annotations, .. } => {

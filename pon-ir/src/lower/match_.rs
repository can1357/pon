...
fn branch_on(scope: &mut BodyScope, cond: Value, fail_block: BlockId) -> Result<(), LowerError> {
	...
	scope.set_term(Terminator::CondBranch { cond: truth, then_: pass_block, else_: fail_block })?;
...
fn lower_pattern(
	...
	match pattern {
		Pattern::MatchValue(pattern) => {
			...
			let cond =
				scope.emit(InstKind::Compare { op: CmpOp::Eq, lhs: subject, rhs: expected })?;
		...
		Pattern::MatchSingleton(pattern) => {
			...
			let cond =
				scope.emit(InstKind::Is { lhs: subject, rhs: expected, negate: false })?;
			branch_on(scope, cond, fail_block)
		},
		...
		Pattern::MatchMapping(pattern) => {
			lower_mapping_pattern(driver, scope, subject, pattern, fail_block)
		},
		Pattern::MatchClass(pattern) => {
			lower_class_pattern(driver, scope, subject, pattern, fail_block)
		},
		Pattern::MatchStar(_) => {
			Err(LowerError::internal("star pattern outside sequence-pattern lowering"))
		},
	}
}
		...
		match &expected_names {
			Some(expected) if *expected != names => {
				return Err(LowerError::unsupported_at(
					...
					SourceSpan::from_bounds(
						pattern.range.start().to_u32(),
						pattern.range.end().to_u32(),
					),
				));
			},
			Some(_) => {},
...
fn lower_sequence_pattern(
	...
	if patterns
		...
		> 1
	{
		return Err(LowerError::unsupported("multiple starred subpatterns in sequence pattern"));
	}
		...
		Some(star_index) => {
			...
			if let Pattern::MatchStar(star) = &patterns[star_index] {
				if let Some(name) = star.name.as_ref() {
					let rest =
						driver.lower_sequence_rest(scope, subject, before as i64, after as i64)?;
...
fn lower_mapping_pattern(
	...
	if !keys.is_empty() {
		let extracted = scope.emit(InstKind::MatchKeys { subj: subject, keys: keys.clone() })?;
		let none = scope.emit(InstKind::Const(PyConst::None))?;
		let found = scope.emit(InstKind::Is { lhs: extracted, rhs: none, negate: true })?;
	...
	if let Some(rest) = pattern.rest.as_ref() {
		...
		scope.emit(InstKind::DictMerge { map: rest_map, other: subject })?;
		for key in &keys {
			scope.emit(InstKind::SubscriptDel { obj: rest_map, index: *key })?;
		}
...
fn lower_class_pattern(
	...
	let none = scope.emit(InstKind::Const(PyConst::None))?;
	let matched = scope.emit(InstKind::Is { lhs: extracted, rhs: none, negate: true })?;

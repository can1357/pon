...
impl NameClass {
	...
	/// Return the closure-cell slot if this classification provides a cell
	/// object.
...
pub struct ScopeInfo {
	...
	/// True if the function contains `yield` or `yield from` directly in its
	/// body.
...
impl ScopeInfo {
	...
	pub fn local_slot(&self, name: &str) -> Option<u32> {
		self
			.symbol(name)
			.and_then(|symbol| symbol.class.local_slot())
	}
	...
	pub fn closure_slot(&self, name: &str) -> Option<u32> {
		self
			.symbol(name)
			.and_then(|symbol| symbol.class.closure_slot())
	}
...
struct ScopeBuilder {
	...
	name:               String,
	span:               Option<(u32, u32)>,
	/// Positional-only + positional-or-keyword parameters for
	/// `ParameterSummary`.
	params:             Vec<String>,
	/// All formal parameters in local-slot order (`pos`, `*args`, kw-only,
	/// `**kwargs`).
...
impl ScopeBuilder {
	...
	fn annotate_child(&mut self) -> &mut ScopeBuilder {
		...
		self
			.annotate
			.as_mut()
			.expect("annotate child was just created")
	}
}
...
fn finalize_scope(mut builder: ScopeBuilder, enclosing_locals: &BTreeSet<String>) -> ScopeInfo {
	...
	let needs_class_closure =
		matches!(builder.kind, ScopeKind::Class) && names_needed_by_children.contains("__class__");
	...
	for name in all_names {
		let class = if let Some(local_slot) = slot_by_name.get(&name) {
			if let Some(cell_slot) = cell_slot_by_name.get(&name) {
				NameClass::Cell { local_slot: *local_slot, cell_slot: *cell_slot }
			} else {
...
fn parameter_summary(
	builder: &ScopeBuilder,
	slot_by_name: &BTreeMap<String, u32>,
) -> ParameterSummary {
...
"#,
		...
		let outer = analysis
			...
			.child(ScopeKind::Function, "outer")
			.expect("outer function should be discovered");
		assert_eq!(outer.cell_vars, vec!["args".to_owned(), "flag".to_owned(), "kwargs".to_owned()]);
		for (name, local_slot, cell_slot) in [("args", 0, 0), ("flag", 1, 1), ("kwargs", 2, 2)] {
			let symbol = outer
				.symbol(name)
				.expect("captured parameter should be classified");
...
"#,
	...
	fn records_child_scope_spans_for_span_pairing() {
		...
		let expect_span = |snippet: &str| {
			let start = source
				.find(snippet)
				.expect("snippet should appear in the fixture");

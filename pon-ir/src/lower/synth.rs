...
pub(crate) fn synthesize_scope_function(
	...
	synthesize_scope_function_with_annotate(
		driver, enclosing, child_info, parameters, None, lower_body,
	)
}
...
pub(crate) fn synthesize_scope_function_with_annotate(
	...
	let value = if let Some(name_interned) = name_interned {
		...
		enclosing.emit(InstKind::MakeFunction { func_index: function.0, name_interned, arity })?
	} else {
	...
	if let Some(annotate) = annotate {
		enclosing.emit(InstKind::FunctionSetAnnotate { function: value, annotate })?;
	}
	Ok(value)
}
...
pub(crate) fn claim_namespace_annotate<'a>(
	...
	let child_info =
		scope.next_child_scope(ScopeKind::Function, scope::ANNOTATE_SCOPE_NAME, None)?;
...
fn emit_format_guard(driver: &mut LoweringDriver, body: &mut BodyScope) -> Result<(), LowerError> {
	...
	let gt = body.emit(InstKind::Compare { op: CmpOp::Gt, lhs: format, rhs: two })?;
	let cond = body.emit(InstKind::BoolTest { val: gt })?;
	let raise_block = body.alloc_block()?;
	let cont_block = body.alloc_block()?;
	body.set_term(Terminator::CondBranch { cond, then_: raise_block, else_: cont_block })?;
	...
	body.emit(InstKind::Raise { exc: Some(exc), cause: None })?;
...
pub(super) fn closure_cells(
	scope: &BodyScope,
	info: &ScopeInfo,
) -> Result<Vec<CellId>, LowerError> {
	info
		.free_vars
		.iter()
		.map(|name| {

...
pub(super) fn redirect_raise_terms(
	scope: &mut BodyScope,
	start_block: usize,
	...
	match final_term {
		...
		None => {
			if let Some(saved) = saved_exception {
				scope.emit(InstKind::Raise { exc: Some(saved), cause: None })?;
			}
...
fn lower_exc_star_clause(
	...
	let cond = scope.emit(InstKind::BoolTest { val: matched })?;
	scope.set_term(Terminator::CondBranch { cond, then_: body_block, else_: next_head })?;
...
/// - the dynamic path (a helper call inside the body raising) routes through a
///   handler record (`PushExcInfo` targeting a finally-exception block) that
///   runs a second finally copy and re-raises the pending exception.
///

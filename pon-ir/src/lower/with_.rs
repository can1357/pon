...
fn lower_enter_call(
	...
	let called = scope.emit(InstKind::Call { callee: method, args: Vec::new() })?;
	if is_async {
...
fn lower_exit_call(
	...
	let name = driver.names.intern(name)?;
	let method = scope.emit(InstKind::LoadAttr { obj: manager, name })?;
	let called = scope.emit(InstKind::Call { callee: method, args: args.to_vec() })?;
	if is_async {
		scope.emit(InstKind::Await { awaitable: called })
	} else {
		Ok(called)
...
/// skeleton: evaluate managers left-to-right, call enter left-to-right, lower
/// the body inside a `PushExcInfo` protected region, then call exits in reverse
/// order.
///
...
pub(super) fn lower_with_stmt(
	...
	let exit_name = if stmt.is_async {
		"__aexit__"
	} else {
		"__exit__"
	};
	...
	let exc_type = scope.emit(InstKind::Call { callee: type_fn, args: vec![exc] })?;
	let tb = scope.emit(InstKind::Const(PyConst::None))?;
	for (index, manager) in managers.iter().rev().copied().enumerate() {
		let result =
			lower_exit_call(driver, scope, manager, exit_name, stmt.is_async, [exc_type, exc, tb])?;
		let cond = scope.emit(InstKind::BoolTest { val: result })?;
		let suppress_block = scope.alloc_block()?;
		let next_block = scope.alloc_block()?;
		scope.set_term(Terminator::CondBranch { cond, then_: suppress_block, else_: next_block })?;
	...
	for (tramp, outer) in [
		...
		(continue_tramp, loop_targets.map(|targets| targets.continue_block)),
	] {

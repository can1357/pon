#![doc = "Phase-A Cranelift JIT driver."]
...
use pon_codegen::baseline::{CodegenError, NameMap, compile_function};
...
/// scratch state.
///
/// The engine must stay alive while compiled code runs: Phase-A string constants
/// can be emitted as JIT module data and borrowed by runtime objects.
...
impl JitEngine {
    ...
    pub fn compile(&mut self, ir_module: &IrModule) -> Result<MainFn, JitError> {
        ...
        for (index, function) in ir_module.functions.iter().enumerate() {
            ...
            self.module.define_function(func_ids[index], &mut self.ctx)?;
        }
    ...
    /// Initialize the runtime, compile the module, and invoke `__main__(NULL, 0)`.
    ///
    /// A NULL return from compiled code is the Phase-A runtime-error sentinel and
    /// is reported as [`JitError::Runtime`].
    ...
    fn declare_ir_functions(&mut self, ir_module: &IrModule) -> Result<Vec<cranelift_module::FuncId>, JitError> {

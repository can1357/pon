...
pub fn install_local_python_package(env: &EnvLayout, resolved_record: &ResolvedRecord, path: &Path) -> Result<InstallReport> {
    ...
    env.create_dirs()?;
    if source.is_dir() {
        copy_tree(&source, &env.site_packages.join(&manifest.import_name), Path::new(&manifest.import_name), &mut record_entries)?;
    } else {
...
fn copy_tree(source: &Path, destination: &Path, record_prefix: &Path, record_entries: &mut Vec<String>) -> Result<()> {

...
pub struct EntryPoint {
    ...
    pub module: String,
    /// Attribute path invoked by the generated wrapper; dotted attributes are preserved.
    pub attr: String,
}
...
pub fn parse_entry_points(text: &str, label: &str) -> Result<EntryPoints> {
    ...
    for (line_index, raw_line) in text.lines().enumerate() {
        ...
        match section {
            Section::ConsoleScripts => entry_points.console_scripts.push(parse_script(line, label, line_number)?),
            Section::GuiScripts => entry_points.gui_scripts.push(parse_script(line, label, line_number)?),
            Section::Other => {}
        }
    }

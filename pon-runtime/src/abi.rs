...
impl HandlerInfo {
	...
	pub const fn new(frame: *mut PyFrame, target: u32, stack_depth: u32, kind: u8) -> Self {
		Self { frame, target, stack_depth, kind, reserved: [0; 3] }
	}
}
...
const PARAMS_OP_OBJ_OBJ: &[AbiTy] =
	&[AbiTy::U8, AbiTy::PyObjectPtr, AbiTy::PyObjectPtr, AbiTy::FeedbackCellPtr];
...
const PARAMS_OBJ_OBJ_FEEDBACK: &[AbiTy] =
	&[AbiTy::PyObjectPtr, AbiTy::PyObjectPtr, AbiTy::FeedbackCellPtr];
...
const PARAMS_BUILD_CLASS: &[AbiTy] =
	&[AbiTy::PyObjectPtr, AbiTy::U32, AbiTy::PyObjectPtrPtr, AbiTy::Usize];
...
const PARAMS_STR_METHOD: &[AbiTy] =
	&[AbiTy::U16, AbiTy::PyObjectPtr, AbiTy::PyObjectPtrPtr, AbiTy::Usize];
...
const PARAMS_CALL_FEEDBACK: &[AbiTy] =
	&[AbiTy::PyObjectPtr, AbiTy::PyObjectPtrPtr, AbiTy::Usize, AbiTy::FeedbackCellPtr];
...
const PARAMS_MATCH_CLASS: &[AbiTy] =
	&[AbiTy::PyObjectPtr, AbiTy::PyObjectPtr, AbiTy::Usize, AbiTy::ConstU32Ptr, AbiTy::Usize];
...
pub(crate) fn alloc_heap_instance(
	...
	with_runtime(|runtime| {
		...
		unsafe {
			ptr::write(object, type_::PyHeapInstance {
...
fn perform_runtime_init() -> Result<(), String> {
	let should_register_native = {
		...
		if slot.is_some() {
			...
			let long_type =
				Box::into_raw(Box::new(PyType::new(type_type, "int", mem::size_of::<PyLong>())));
			unsafe {
				(*long_type).tp_getattro = Some(int_getattro);
			}
			let unicode_type =
				Box::into_raw(Box::new(PyType::new(type_type, "str", mem::size_of::<PyUnicode>())));
			let mut function_type =
				Box::new(PyType::new(type_type, "function", mem::size_of::<PyFunction>()));
			...
			let none_type =
				Box::into_raw(Box::new(PyType::new(type_type, "NoneType", mem::size_of::<PyNone>())));
			...
			let ellipsis_type =
				Box::into_raw(Box::new(PyType::new(type_type, "ellipsis", mem::size_of::<PyNone>())));
			...
			unsafe {
				ptr::write(none, PyNone { ob_base: PyObjectHeader::new(none_type) });
			}
			...
			// right size.
			unsafe {
				ptr::write(not_implemented, PyNone {
			...
			// size.
			unsafe {
				ptr::write(ellipsis, PyNone { ob_base: PyObjectHeader::new(ellipsis_type) });
			}
...
fn register_gc_types(heap: &Heap) {
	heap.register_type(TYPE_ID_TYPE, GcTypeInfo {
	...
	heap.register_type(TYPE_ID_FUNCTION, GcTypeInfo {
		size:     mem::size_of::<PyFunction>(),
		trace:    trace_function,
		finalize: Some(finalize_function),
	});
	heap.register_type(TYPE_ID_NONE, GcTypeInfo {
		size:     mem::size_of::<PyNone>(),
		trace:    trace_no_refs,
		finalize: None,
	});
	heap.register_type(TYPE_ID_NOT_IMPLEMENTED, GcTypeInfo {
		size:     mem::size_of::<PyNone>(),
		trace:    trace_no_refs,
		finalize: None,
	});
	heap.register_type(TYPE_ID_ELLIPSIS, GcTypeInfo {
		size:     mem::size_of::<PyNone>(),
		trace:    trace_no_refs,
		finalize: None,
	});
	heap.register_type(TYPE_ID_EXCEPTION, GcTypeInfo {
		size:     mem::size_of::<PyBaseException>(),
		trace:    trace_base_exception,
		finalize: Some(crate::types::exc::finalize_base_exception),
	});
	heap.register_type(TYPE_ID_EXCEPTION_GROUP, GcTypeInfo {
		size:     mem::size_of::<crate::types::exc::PyExceptionGroup>(),
		trace:    trace_exception_group,
		finalize: Some(crate::types::exc::finalize_base_exception),
	});
	heap.register_type(type_::TYPE_ID_HEAP_INSTANCE, GcTypeInfo {
	...
	heap.register_type(type_::TYPE_ID_PAYLOAD_SUBCLASS_INSTANCE, GcTypeInfo {
	...
	heap.register_type(crate::types::weakref::TYPE_ID_WEAKREF, GcTypeInfo {
...
unsafe extern "C" fn object_dunder_format_native(
	...
	if !spec.is_empty() {
		let message = format!("unsupported format string passed to {}.__format__", unsafe {
			object_type_name_for_error(receiver)
		},);
		return unsafe { exc::pon_raise_type_error(message.as_ptr(), message.len()) };
	}
...
fn classmethod_builtin_type() -> *mut PyType {
	static TYPE: LazyLock<usize> = LazyLock::new(|| {
		let mut ty =
			PyType::new(ptr::null(), "classmethod", mem::size_of::<classmethod::PyClassMethod>());
...
pub(crate) fn staticmethod_builtin_type() -> *mut PyType {
	static TYPE: LazyLock<usize> = LazyLock::new(|| {
		let mut ty =
			PyType::new(ptr::null(), "staticmethod", mem::size_of::<classmethod::PyStaticMethod>());
...
unsafe extern "C" fn builtin_type_new(
	...
	unsafe {
		call_builtin_type_constructor(_cls, args, kwargs, crate::native::builtins_mod::builtin_type)
	}
}
...
unsafe extern "C" fn builtin_object_new(
	...
	unsafe {
		call_builtin_type_constructor(_cls, args, kwargs, crate::native::builtins_mod::builtin_object)
	}
}
...
unsafe extern "C" fn builtin_bool_new(
	...
	unsafe {
		call_builtin_type_constructor(_cls, args, kwargs, crate::native::builtins_mod::builtin_bool)
	}
}
...
unsafe extern "C" fn builtin_float_new(
	...
	unsafe {
		call_builtin_type_constructor(_cls, args, kwargs, crate::native::builtins_mod::builtin_float)
	}
}
...
unsafe extern "C" fn builtin_bytes_new(
	...
	unsafe {
		call_builtin_type_constructor(_cls, args, kwargs, crate::native::builtins_mod::builtin_bytes)
	}
}
...
unsafe extern "C" fn builtin_list_new(
	...
	unsafe {
		call_builtin_type_constructor(_cls, args, kwargs, crate::native::builtins_mod::builtin_list)
	}
}
...
unsafe extern "C" fn builtin_tuple_new(
	...
	unsafe {
		call_builtin_type_constructor(_cls, args, kwargs, crate::native::builtins_mod::builtin_tuple)
	}
}
...
unsafe extern "C" fn builtin_range_new(
	...
	unsafe {
		call_builtin_type_constructor(_cls, args, kwargs, crate::native::builtins_mod::builtin_range)
	}
}
...
unsafe extern "C" fn builtin_filter_new(
	...
	unsafe {
		call_builtin_type_constructor(_cls, args, kwargs, crate::native::builtins_mod::builtin_filter)
	}
}
...
unsafe extern "C" fn builtin_super_new(
	...
	unsafe {
		call_builtin_type_constructor(_cls, args, kwargs, crate::native::builtins_mod::builtin_super)
	}
}
...
fn alloc_long(runtime: &Runtime, value: i64) -> Result<*mut PyObject, String> {
	...
	// size.
	unsafe {
		ptr::write(object, PyLong { ob_base: PyObjectHeader::new(runtime.long_type), value });
	}
	Ok(as_object_ptr(object))
}
...
unsafe fn build_class_with_body(
	...
	if !frame.orig_bases.is_null() {
		if frame.mapping.is_null() {
			unsafe {
				(&mut *frame.namespace).set(crate::intern::intern("__orig_bases__"), frame.orig_bases)
			};
		} else {
...
pub unsafe extern "C" fn pon_load_global(
	...
	catch_object_helper(|| {
		...
		match resolved {
			Some(value) => {
				if let Some(cell) = unsafe { feedback.as_ref() } {
					cell.record_global(namespace_identity(), GlobalIC {
...
pub(crate) unsafe fn alloc_payload_subclass_instance(
	...
	with_runtime(|runtime| {
		...
		unsafe {
			ptr::write(object, type_::PyPayloadSubclassInstance {
...
unsafe extern "C" fn bytes_dunder_new_native(
	...
	unsafe {
		data_type_dunder_new_common("bytes", crate::native::builtins_mod::builtin_bytes, argv, argc)
	}
}
...
fn install_data_type_dunders(runtime: &mut Runtime) {
	...
	for (ty, new_entry, with_str) in entries {
		...
		let new_name = crate::intern::intern("__new__");
		let Ok(function) =
			alloc_function(runtime, new_entry, crate::builtins::variadic_arity(), new_name)
		else {
			continue;
		};
		crate::types::function::mark_native_function(function);

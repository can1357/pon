...
use crate::{
	...
	types::{
		exc::{
			EXC_GROUP_METHOD_DERIVE, EXC_GROUP_METHOD_SPLIT, EXC_GROUP_METHOD_SUBGROUP, ExceptionKind,
			ExceptionTypeSet, PyBaseException, PyExceptionGroup, as_exception_group,
...
fn alloc_exception_group_object(
	...
	let context = active_context();
	unsafe {
		ptr::write(object, PyExceptionGroup {
...
fn attach_current_traceback(runtime: &Runtime, exception: *mut PyObject) {
	...
	runtime.heap.register_type(TYPE_ID_FRAME, GcTypeInfo {
...
pub(crate) fn raise_kind_error_no_args(kind: ExceptionKind) -> *mut PyObject {
	match ensure_runtime_for_exc() {
		Ok(()) => match super::with_runtime(|runtime| {
			raise_builtin_value(runtime, kind, ptr::null_mut(), exception_kind_name(kind).to_owned())
		}) {
			Some(result) => result,
			None => super::return_null_with_error("runtime is not initialized"),
		},
...
fn raise_value_exception(
	...
	match ensure_runtime_for_exc() {
		Ok(()) => {
			match super::with_runtime(|runtime| raise_builtin_value(runtime, kind, value, diagnostic))
			{
				Some(result) => result,
				None => super::return_null_with_error("runtime is not initialized"),
			}
	...
	super::with_runtime(|runtime| {
		...
		if type_name_is(types, "tuple") || type_name_is(types, "list") {
			let values = match unsafe { crate::types::type_::positional_args_from_object(types) } {
				Ok(values) => values,
				Err(message) => {
					return Err(raise_builtin_text(runtime, ExceptionKind::TypeError, &message));
				},
			};
...
fn split_exception(
	...
	if condition_matches(cond, node)? {
		return Ok(SplitOutcome { matched: node, rest: ptr::null_mut() });
	}
	if unsafe { !is_exception_group_instance(node) } {
		return Ok(SplitOutcome { matched: ptr::null_mut(), rest: node });
	}
...
fn exc_star_split_current(
	...
	if rest.is_null() {
		return Ok(SplitOutcome { matched: ptr::null_mut(), rest: ptr::null_mut() });
	}
...
pub unsafe extern "C" fn pon_build_exc_group(
	...
	super::catch_object_helper(|| {
		...
		match super::with_runtime(|runtime| {
			...
			build_exception_group_checked(runtime, runtime.exception_types.exception_group, &[
				message,

...
static TEMPLATE_NUMBER_METHODS: LazyLock<usize> = LazyLock::new(|| {
	let methods = PyNumberMethods { nb_add: Some(template_add), ..PyNumberMethods::EMPTY };
...
static INTERPOLATION_TYPE: LazyLock<usize> = LazyLock::new(|| {
	let mut ty =
		PyType::new(runtime_type_type(), "Interpolation", core::mem::size_of::<PyInterpolation>());
...
pub(crate) unsafe fn format_template(
	...
	let mut state =
		TemplateFormatState { args, mapping, auto_index: 0, numbering: NumberingMode::None };
...
unsafe fn resolve_field(
	...
	let mut value = if head.is_empty() {
		if state.numbering == NumberingMode::Manual {
			return Err(
				"cannot switch from manual field specification to automatic field numbering".to_owned(),
			);
		}
	...
	while i < template.len() {
		...
		let rendered = match ty {
			...
			b'd' | b'i' | b'u' => {
				...
				render_percent_int(&value, 10, false, false, plus, space, zero, left, width, precision)
					.into_bytes()
			},
			b'o' | b'x' | b'X' => {

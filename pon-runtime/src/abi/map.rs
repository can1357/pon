...
fn register_map_types(runtime: &super::Runtime) {
	...
	runtime
		.heap
		.register_type(TYPE_ID_DICT_SUBCLASS_INSTANCE, GcTypeInfo {
...
fn alloc_dict_view(
	...
	unsafe {
		dict::init_dict_view(object, dict::dict_view_type(runtime._type_type, kind), source, kind)
	};
	as_object_ptr(object)
}
...
fn alloc_frozenset(runtime: &super::Runtime, entries: Vec<*mut PyObject>) -> *mut PyObject {
	...
	unsafe {
		frozenset::init_frozenset(object, frozenset::frozenset_type(runtime._type_type), entries)
	};
	as_object_ptr(object)
}
	...
	super::catch_object_helper(|| {
		if unsafe { dict::is_dict(object) } {
			...
			if let Some(slot) = unsafe {
				...
				let setitem =
					unsafe { crate::descr::lookup_in_type(ty, crate::intern::intern("__setitem__")) };
				if setitem.is_null() {
					let message =
						format!("'{}' object does not support item assignment", unsafe { (*ty).name() });
...
pub(crate) fn alloc_dict_subclass_instance(
	...
	super::with_runtime(|runtime| {
		...
		unsafe {
			ptr::write(object, dict::PyDictSubclassInstance {
				...
				storage: dict::PyDictStorage { entries: Vec::new(), buckets: Vec::new() },
			});
		}
...
pub(crate) unsafe fn builtin_set_type_method(ty: *mut PyType, name_id: u32) -> *mut PyObject {
	...
	let function = unsafe {
		super::pon_make_function(entry as *const u8, crate::builtins::variadic_arity(), name_id)
	};
	if !function.is_null() {

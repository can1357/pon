...
fn register_seq_gc_types(runtime: &Runtime) {
	runtime.heap.register_type(TYPE_ID_LIST, GcTypeInfo {
		size:     mem::size_of::<PyList>(),
		trace:    list::trace_list,
		finalize: Some(list::finalize_list),
	});
	runtime.heap.register_type(TYPE_ID_TUPLE, GcTypeInfo {
		size:     mem::size_of::<PyTuple>(),
		trace:    tuple::trace_tuple,
		finalize: Some(tuple::finalize_tuple),
	});
	runtime.heap.register_type(TYPE_ID_RANGE, GcTypeInfo {
		size:     mem::size_of::<PyRange>(),
		trace:    range_::trace_range,
		finalize: None,
	});
	runtime.heap.register_type(TYPE_ID_SLICE, GcTypeInfo {
		size:     mem::size_of::<PySlice>(),
		trace:    slice_::trace_slice,
		finalize: None,
	});
	runtime.heap.register_type(TYPE_ID_SEQ_ITER, GcTypeInfo {
	...
	runtime
		.heap
		.register_type(TYPE_ID_LIST_SUBCLASS_INSTANCE, GcTypeInfo {
	...
	runtime
		.heap
		.register_type(TYPE_ID_TUPLE_SUBCLASS_INSTANCE, GcTypeInfo {
...
pub(crate) fn alloc_list_subclass_instance(
	...
	with_runtime(|runtime| {
		...
		unsafe {
			ptr::write(object, list::PyListSubclassInstance {
				...
				storage: list::PyListStorage { len: 0, cap: 0, items: ptr::null_mut() },
			});
		}
		Ok(as_object_ptr(object))
	})
...
pub(crate) fn alloc_tuple_subclass_instance(
	...
	with_runtime(|runtime| {
		...
		unsafe {
			ptr::write(object, tuple::PyTupleSubclassInstance {
				base:    type_::PyHeapInstance {
					ob_base: PyObjectHeader::new(cls),
					dict: instance_dict,
					slots,
				...
				storage: tuple::PyTupleStorage { len: values.len(), items },
			});
		}
		Ok(as_object_ptr(object))
	})
...
pub(crate) fn normalize_slice(slice: &PySlice, len: usize) -> Result<SliceIndices, String> {
	...
	Ok(SliceIndices { start, stop, step, len: slice_len })
}
...
fn alloc_native_seq_function(
	...
	with_runtime(|runtime| {
		super::alloc_function(runtime, entry as *const u8, native_method_arity(), name_interned)
	})
	.unwrap_or_else(|| Err("runtime is not initialized".to_owned()))
}
...
pub unsafe extern "C" fn pon_seq_len(object: *mut PyObject) -> isize {
	...
	match catch_unwind(AssertUnwindSafe(|| sequence_len_raw(object))) {
		Ok(Ok(len)) => isize::try_from(len)
			.unwrap_or_else(|_| return_minus_one_with_error("sequence length exceeds isize") as isize),
...
pub unsafe extern "C" fn pon_seq_get_item(
	...
	catch_object_helper(|| {
		if is_slice(index_or_slice) {
			...
			match sequence_item_raw(object, match index_value(index_or_slice) {
				Ok(index) => index,
				Err(message) => return return_null_with_error(message),
			}) {
				Ok(value) => value,
...
mod tests {
	...
	fn list_full_reverse_slice_preserves_length_and_order() {
		...
		unsafe {
			assert_eq!(pon_runtime_init(), 0);
			let mut items =
				[pon_const_int(10), pon_const_int(20), pon_const_int(30), pon_const_int(40)];
			let list = pon_build_list(items.as_mut_ptr(), items.len());
			assert!(!list.is_null());
	...
	unsafe fn abi_range(start: i64, stop: i64, step: i64) -> *mut PyObject {
		let object =
			unsafe { pon_build_range(pon_const_int(start), pon_const_int(stop), pon_const_int(step)) };
		assert!(!object.is_null());
		object
	}
	...
	fn range_hash_crosses_representations() {
		...
		unsafe {
			...
			let native_hash =
				crate::native::builtins_mod::range_hash_value(native).expect("native range must hash");

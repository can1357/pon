...
pub unsafe fn is_true(object: *mut PyObject) -> i32 {
	...
	if let Some(slot) = unsafe { (*ty).tp_bool } {
		return unsafe {
			normalize_inquiry(slot(object), "__bool__ returned an error without setting an exception")
		};
	}
	...
	if let Some(slot) = unsafe {
		...
		return unsafe {
			normalize_inquiry(slot(object), "nb_bool returned an error without setting an exception")
		};
	}
...
unsafe fn is_true_len_value(result: *mut PyObject) -> i64 {
	...
	match value.to_i64() {
		...
		None => {
			raise_type_error_status("cannot fit '__len__' result into an index-sized integer").into()
		},
	}
}
...
pub unsafe fn del_attr(object: *mut PyObject, name: u32) -> i32 {
	unsafe {
		set_or_del_attr(object, name, ptr::null_mut(), "object does not support attribute deletion")
	}
}
	...
	match unsafe { call_unary_slot(slot, object) } {
		...
		SlotOutcome::Missing | SlotOutcome::NotImplemented => {
			...
			if hook.is_null() {
				...
				if sq_item.is_some()
					|| !unsafe { crate::descr::lookup_in_type(ty, crate::intern::intern("__getitem__")) }
						.is_null()
				{
...
pub unsafe fn subscript_get(object: *mut PyObject, key: *mut PyObject) -> *mut PyObject {
	...
	if unsafe { crate::types::type_::is_type_object(object) } {
		...
		if !hook.is_null() {
			let bound =
				unsafe { crate::descr::descriptor_get(hook, ptr::null_mut(), object.cast::<PyType>()) };
			if bound.is_null() {
				return ptr::null_mut();
			}
			let mut argv = [key];

...
pub(crate) struct PyPonCapiContainers {
    ...
    dict_next: unsafe extern "C" fn(*mut PyObject, *mut isize, *mut *mut PyObject, *mut *mut PyObject) -> c_int,
...
fn list_storage(object: *mut PyObject) -> Option<&'static list::PyListStorage> {
    ...
    if unsafe { (*object).ob_type } == abi::seq::list_type().cast_const() {
        let storage = unsafe { object.cast::<u8>().add(core::mem::offset_of!(list::PyList, len)) };
...
fn checked_index(index: isize, len: usize, kind: &str) -> Result<usize, *mut PyObject> {
    ...
    let index = usize::try_from(index).map_err(|_| index_error(&format!("{kind} index out of range")))?;
    if index >= len {
        return Err(index_error(&format!("{kind} index out of range")));
    }
    Ok(index)
...
unsafe extern "C" fn capi_tuple_get_slice(tuple: *mut PyObject, start: isize, stop: isize) -> *mut PyObject {
    catch_object(|| {
        let tuple = crate::tag::untag_arg(tuple);
        let Some(items) = tuple_slice(tuple) else {
            return type_error("expected tuple object");
        ...
        let (start, stop) = if stop < start { (start, start) } else { (start, stop) };
...
unsafe extern "C" fn capi_list_insert(list: *mut PyObject, index: isize, item: *mut PyObject) -> c_int {
    catch_status(|| {
        ...
        if storage.len == storage.cap {
            let new_cap = if storage.cap == 0 { 4 } else { storage.cap.saturating_mul(2) };
            if new_cap <= storage.cap {
...
unsafe extern "C" fn capi_dict_set_item_string(dict: *mut PyObject, key: *const c_char, value: *mut PyObject) -> c_int {
    catch_status(|| {
...
unsafe extern "C" fn capi_dict_next(
    ...
    catch_status(|| {
        ...
        if cursor == 0 {
            ...
            snapshots.insert(
                snapshot_key,
                entries.into_iter().map(|entry| (entry.key as usize, entry.value as usize)).collect(),
            );
        }
...
unsafe extern "C" fn capi_dict_merge(dict: *mut PyObject, other: *mut PyObject, override_flag: c_int) -> c_int {
    catch_status(|| {
        let dict = crate::tag::untag_arg(dict);
...
fn merge_entries(dict: *mut PyObject, entries: Result<Vec<DictEntry>, String>, override_existing: bool) -> c_int {
...
unsafe extern "C" fn capi_set_size(set: *mut PyObject) -> isize {
    catch_size(|| {
        ...
        if unsafe { set_::is_set(set) } {
            return checked_len(unsafe { set_::set_ref(set) }.map(|set| set.entries.len()).unwrap_or(0));
        }
        if unsafe { frozenset::is_frozenset(set) } {
            return checked_len(unsafe { frozenset::entries_snapshot(set) }.map(|entries| entries.len()).unwrap_or(0));
        }
...
unsafe extern "C" fn capi_slice_unpack(slice: *mut PyObject, start: *mut isize, stop: *mut isize, step: *mut isize) -> c_int {
    catch_status(|| {
...
unsafe extern "C" fn capi_slice_adjust_indices(length: isize, start: *mut isize, stop: *mut isize, step: isize) -> isize {
    catch_size(|| {
...
unsafe extern "C" fn capi_sequence_fast(object: *mut PyObject, message: *const c_char) -> *mut PyObject {
    catch_object(|| {
        let object = crate::tag::untag_arg(object);
        if list_storage(object).is_some() || tuple_slice(object).is_some() {
            return object;
...
unsafe extern "C" fn capi_sequence_fast_items(object: *mut PyObject, len_out: *mut isize) -> *mut *mut PyObject {
    let object = crate::tag::untag_arg(object);
    if !len_out.is_null() {
        unsafe { *len_out = 0 };
    }
...
unsafe extern "C" fn capi_mapping_get_item_string(object: *mut PyObject, key: *const c_char) -> *mut PyObject {
    catch_object(|| {
        let object = crate::tag::untag_arg(object);
        let Some(key) = string_key(key) else {
            return ptr::null_mut();
...
fn slice_index_saturating(object: *mut PyObject) -> Result<isize, c_int> {
    ...
    let Some(value) = (unsafe { crate::types::int::to_bigint_including_bool(object) }) else {
        return Err(status_type_error("slice indices must be integers or None or have an __index__ method"));
    };
...
"#,
        ...
        let method = module_attr(module_name, intern("exercise_containers")).expect("exercise_containers registered");

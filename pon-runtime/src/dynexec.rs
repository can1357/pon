...
pub(crate) fn module_namespace_dict(module_name: u32) -> Result<*mut PyObject, String> {
	...
	registry
		.by_dict
		.insert(dict_object as usize, GlobalsBinding {
			module_name,
...
fn compile_source(
	...
	hook(DynCompileRequest { source: &source, filename: &filename, mode })
...
fn ensure_dynexec_globals_binding(globals: *mut PyObject) -> Result<GlobalsBinding, String> {
	...
	let binding = GlobalsBinding { module_name, kind: GlobalsBindingKind::DynExecGlobals };
	let mut registry = GLOBALS_REGISTRY
		.lock()
		.unwrap_or_else(|poison| poison.into_inner());
...
pub unsafe extern "C" fn builtin_dunder_import(
	...
	unsafe {
		crate::import::pon_import_name(name_id, fromlist_names.as_ptr(), fromlist_names.len(), level)
	}
}

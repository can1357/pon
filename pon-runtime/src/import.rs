...
pub unsafe extern "C" fn pon_aot_register_module(
	...
	let mut modules = EMBEDDED_MODULES
		.lock()
		.unwrap_or_else(|poison| poison.into_inner());
	modules.insert(name.to_owned(), EmbeddedModule { is_package: is_package != 0, body });
}
...
mod tests {
	...
	fn install_module_stamps_only_native_function_carriers() {
		...
		install_module(&installed_module, [
	...
	fn pep562_module_attr_miss_calls_module_getattr() {
		...
		let value =
			unsafe { crate::abi::object::pon_get_attr(module, intern("lazy_attr"), ptr::null_mut()) };
	...
	fn pep562_import_from_getattr_attribute_error_becomes_import_error() {
		...
		assert!(
			message.as_deref().is_some_and(
				|text| text.contains(&format!("cannot import name 'missing' from '{module_name}'"))
			),
	...
	fn pon_import_path_root_loads_curated_source_module() {
		...
		assert_eq!(format_object_for_print(marker).as_deref(), Ok("loaded-via-pon-import-path"));
	...
	fn namespace_package_import_composes_roots() {
		...
		let pkg_name =
			format!("pon_ns_pkg_{}_{}", process::id(), NEXT_TEMP_ID.fetch_add(1, Ordering::Relaxed));
		...
		assert!(!package.is_null(), "importing namespace package failed: {:?}", pon_err_message());
...
pub(crate) fn module_namespace_for_object(
	object: *mut PyObject,

...
//! `readinto`), slicing, ordering comparisons, `byteswap`, `array_reconstructor`.
...
fn checked_signed(value: &BigInt, min: i64, max: i64, what: &str) -> Result<i64, *mut PyObject> {
    match value.to_i64() {
        ...
        Some(value) if value < min => Err(raise_overflow_error(&format!("{what} is less than minimum"))),
        Some(_) => Err(raise_overflow_error(&format!("{what} is greater than maximum"))),
        None if value.is_negative() => Err(raise_overflow_error(&format!("{what} is less than minimum"))),
    ...
    match typecode {
        b'f' | b'd' => {
            ...
            let float_value = if let Some(float_value) = unsafe { crate::types::float::to_f64(value) } {
                float_value
            } else if let Some(int_value) = unsafe { crate::types::int::to_bigint_including_bool(value) } {
        ...
        _ => {
            // SAFETY: `value` is a live untagged object.
            let Some(int_value) = (unsafe { crate::types::int::to_bigint_including_bool(value) }) else {
                return Err(raise_type_error(&format!(
            ...
            match typecode {
                b'b' => checked_signed(&int_value, i64::from(i8::MIN), i64::from(i8::MAX), "signed char")
                ...
                b'h' => checked_signed(&int_value, i64::from(i16::MIN), i64::from(i16::MAX), "signed short integer")
                ...
                b'i' => checked_signed(&int_value, i64::from(i32::MIN), i64::from(i32::MAX), "signed integer")
                    .map(|value| ItemBytes::new(&(value as i32).to_ne_bytes())),
                b'I' => checked_unsigned(&int_value, u64::from(u32::MAX), "unsigned int is greater than maximum")
                ...
                b'q' => int_value
                    .to_i64()
                    .ok_or_else(|| raise_overflow_error("Python int too large to convert to C long long"))
                    .map(|value| ItemBytes::new(&value.to_ne_bytes())),
                b'L' => checked_unsigned(&int_value, u64::MAX, "Python int too large to convert to C unsigned long")
                    .map(|value| ItemBytes::new(&value.to_ne_bytes())),
                _ => checked_unsigned(&int_value, u64::MAX, "Python int too large to convert to C unsigned long long")
                    .map(|value| ItemBytes::new(&value.to_ne_bytes())),
            }
        }
    }
...
unsafe extern "C" fn array_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    ...
    if positional.len() > 2 {
        return raise_type_error(&format!("array() takes at most 2 arguments ({} given)", positional.len()));
    }
    ...
    if !code_char.is_ascii() || !is_typecode(code_char as u8) {
        return raise_value_error("bad typecode (must be b, B, u, w, h, H, i, I, l, L, q, Q, f or d)");
    }
...
unsafe extern "C" fn array_ass_item_slot(object: *mut PyObject, index: isize, value: *mut PyObject) -> c_int {
    let Some(array) = (unsafe { as_array(object) }) else {
    ...
    if value.is_null() {
        ...
        let index = match normalize_index(index, element_count(array), "array assignment index out of range") {
            Ok(index) => index,
...
unsafe extern "C" fn array_richcmp_slot(left: *mut PyObject, right: *mut PyObject, op: c_int) -> *mut PyObject {
...
unsafe extern "C" fn array_insert_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let index = if clamped < 0 { (clamped + len) as usize } else { clamped as usize };
...
unsafe extern "C" fn array_frombytes_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn array_index_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if rest.is_empty() || rest.len() > 3 {
        return raise_type_error(&format!("array.index() takes 1 to 3 arguments ({} given)", rest.len()));
    }
...
unsafe extern "C" fn array_pop_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if rest.len() > 1 {
        return raise_type_error(&format!("array.pop() takes at most 1 argument ({} given)", rest.len()));
    }

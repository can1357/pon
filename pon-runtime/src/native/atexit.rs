...
use crate::abi::{pon_call, pon_make_function};
...
pub(super) fn make_module() -> Result<*mut PyObject, String> {
    ...
    for (function_name, entry) in functions {
        ...
        let function = unsafe { pon_make_function(entry as *const u8, VARIADIC_ARITY, intern(function_name)) };
        if function.is_null() {
...
pub fn run_exit_callbacks() {
    let callbacks = std::mem::take(&mut *CALLBACKS.lock().unwrap_or_else(|poison| poison.into_inner()));
    for callback in callbacks.into_iter().rev() {
        let callable = callback.callable as *mut PyObject;
        let mut args = callback.args.iter().map(|&arg| arg as *mut PyObject).collect::<Vec<_>>();
...
fn report_ignored_exception() {
    ...
    let detail = crate::abi::exc::pending_exception_object().map(|value| {
        ...
        if message.is_empty() { type_name.to_owned() } else { format!("{type_name}: {message}") }
    });
...
pub(crate) fn gc_held_roots() -> Vec<*mut PyObject> {
    let callbacks = CALLBACKS.lock().unwrap_or_else(|poison| poison.into_inner());
...
unsafe extern "C" fn atexit_register(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let callable = args[0];
    CALLBACKS.lock().unwrap_or_else(|poison| poison.into_inner()).push(ExitCallback {
...
unsafe extern "C" fn atexit_clear(_argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    CALLBACKS.lock().unwrap_or_else(|poison| poison.into_inner()).clear();
    // SAFETY: Singleton accessor.
    unsafe { crate::abi::pon_none() }
}
...
unsafe extern "C" fn atexit_ncallbacks(_argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let count = CALLBACKS.lock().unwrap_or_else(|poison| poison.into_inner()).len();

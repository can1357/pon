...
use rusqlite::types::{Value as SqlValueRaw, ValueRef};
use rusqlite::{ffi, params_from_iter, Connection as RusqliteConnection, OpenFlags};
...
use crate::types::{bytearray_ as bytearray_type, bytes_ as bytes_type, memoryview as memoryview_type, type_};

use super::builtins_mod::{alloc_list, alloc_tuple, VARIADIC_ARITY};
...
fn args_or_type_error<'a>(argv: *mut *mut PyObject, argc: usize, function: &str) -> Result<&'a [*mut PyObject], *mut PyObject> {
    unsafe { argv_slice(argv, argc) }.ok_or_else(|| type_error(&format!("{function}() received a null argument vector")))
}
...
fn function_attr(attr: &str, function_name: &str, entry: BuiltinFn) -> Result<(u32, *mut PyObject), String> {
    ...
    let function = unsafe { abi::pon_make_function(entry as *const u8, VARIADIC_ARITY, intern(function_name)) };
    (!function.is_null())
...
fn optional_str_arg(args: &[*mut PyObject], index: usize, default: &str, name: &str) -> Result<String, *mut PyObject> {
...
fn int_arg(object: *mut PyObject, name: &str) -> Result<i64, *mut PyObject> {
    ...
    value.to_i64().ok_or_else(|| value_error(&format!("{name} is too large")))
}

fn optional_int_arg(args: &[*mut PyObject], index: usize, default: i64, name: &str) -> Result<i64, *mut PyObject> {
    match args.get(index).copied() {
...
fn bool_arg(object: *mut PyObject) -> Option<bool> {
    ...
    unsafe { crate::types::int::to_bigint_including_bool(object) }.map(|value| value != num_bigint::BigInt::from(0))
}
...
fn bytes_or_text_arg(object: *mut PyObject, name: &str) -> Result<Vec<u8>, *mut PyObject> {
    ...
    Err(type_error(&format!("{name} must be a bytes-like object or str, not '{}'", type_name(object))))
}
...
fn sequence_items(object: *mut PyObject, name: &str) -> Result<Vec<*mut PyObject>, *mut PyObject> {
    ...
    match unsafe { crate::types::dict::type_name(object) } {
        Some("list") => Ok(unsafe { (*object.cast::<crate::types::list::PyList>()).as_slice() }.to_vec()),
        Some("tuple") => Ok(unsafe { (*object.cast::<crate::types::tuple::PyTuple>()).as_slice() }.to_vec()),
...
fn exception_class_from_base(module: &str, name: &str, base: *mut PyObject) -> Result<*mut PyObject, String> {
    if base.is_null() {
...
fn sqlite_adapter_key(type_object: *mut PyObject, protocol: *mut PyObject) -> Result<*mut PyObject, *mut PyObject> {
...
fn sqlite_adapter_for_type(type_object: *mut PyObject, protocol: *mut PyObject) -> Result<Option<*mut PyObject>, *mut PyObject> {
...
fn sqlite_call_unary(callable: *mut PyObject, arg: *mut PyObject) -> Result<*mut PyObject, *mut PyObject> {
    let mut argv = [arg];
...
fn sqlite_conform(object: *mut PyObject, protocol: *mut PyObject) -> Result<Option<*mut PyObject>, *mut PyObject> {
    let object = crate::tag::untag_arg(object);
    if object.is_null() {
        return Ok(None);
    }
...
static SQLITE_ERROR_CLASS: LazyLock<usize> = LazyLock::new(|| exception_class("sqlite3", "Error", "Exception").map_or(0, |class| class as usize));
static SQLITE_WARNING_CLASS: LazyLock<usize> = LazyLock::new(|| exception_class("sqlite3", "Warning", "Exception").map_or(0, |class| class as usize));
...
fn alloc_sqlite_cursor(connection: *mut PySqliteConnection, rows: Vec<Vec<SqlValue>>, rowcount: isize) -> *mut PyObject {
...
unsafe fn sqlite_connection_receiver<'a>(object: *mut PyObject) -> Option<&'a mut PySqliteConnection> {
    let object = crate::tag::untag_arg(object);
...
fn sqlite_py_to_value_direct(object: *mut PyObject) -> Result<SqlValueRaw, *mut PyObject> {
    ...
    if let Some(value) = unsafe { crate::types::int::to_bigint(object) }.and_then(|value| value.to_i64()) {
...
fn sqlite_params_from_object(object: Option<*mut PyObject>) -> Result<Vec<SqlValueRaw>, *mut PyObject> {
...
fn sqlite_execute_core(conn: &mut RusqliteConnection, sql: &str, params: Vec<SqlValueRaw>) -> Result<(Vec<Vec<SqlValue>>, isize), String> {
    ...
    let mut rows = stmt.query(params_from_iter(params.iter())).map_err(|error| error.to_string())?;
    let mut out = Vec::new();
...
unsafe extern "C" fn sqlite_prepare_protocol_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    if !kwargs.is_null() {
    ...
    if !positional.is_empty() {
        return type_error(&format!("PrepareProtocol() takes no arguments, got {}", positional.len()));
    }
...
unsafe extern "C" fn sqlite_register_adapter_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn sqlite_register_converter_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn sqlite_complete_statement_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn sqlite_enable_callback_tracebacks_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "enable_callback_tracebacks") {
        Ok(args) => args,
        Err(error) => return error,
    };
    if args.len() != 1 {
        return type_error(&format!("enable_callback_tracebacks() expected 1 argument, got {}", args.len()));
    }
...
unsafe extern "C" fn sqlite_connect_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.is_empty() || args.len() > 9 {
        return type_error(&format!("connect() expected database and optional arguments, got {}", args.len()));
    }
    let database = match str_arg(args[0], "database") {
        Ok(database) => database,
        Err(error) => return error,
    };
    let uri = args.get(7).copied().and_then(bool_arg).unwrap_or_else(|| database.starts_with("file:"));
...
unsafe extern "C" fn sqlite_connection_cursor_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "cursor") {
        Ok(args) => args,
        Err(error) => return error,
    };
    if args.len() != 1 {
        return type_error(&format!("cursor() expected no arguments, got {}", args.len().saturating_sub(1)));
    }
...
unsafe extern "C" fn sqlite_connection_execute_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "execute") {
        Ok(args) => args,
        Err(error) => return error,
    };
    if args.len() < 2 || args.len() > 3 {
        return type_error(&format!("execute() expected sql and optional parameters, got {}", args.len().saturating_sub(1)));
    }
    let Some(connection) = (unsafe { sqlite_connection_receiver(args[0]) }) else {
...
unsafe extern "C" fn sqlite_connection_close_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn sqlite_connection_commit_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn sqlite_connection_rollback_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn sqlite_context_enter_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn sqlite_connection_exit_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn sqlite_cursor_execute_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "execute") {
        Ok(args) => args,
        Err(error) => return error,
    };
...
unsafe extern "C" fn sqlite_cursor_fetchone_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn sqlite_cursor_fetchall_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn sqlite_cursor_close_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "close") {
        Ok(args) => args,
        Err(error) => return error,
    };
...
unsafe extern "C" fn sqlite_cursor_exit_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "__exit__") {
        Ok(args) => args,
        Err(error) => return error,
    };
...
unsafe extern "C" fn sqlite_connection_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
...
unsafe extern "C" fn sqlite_cursor_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    let name = match attr_name(name) {
        Ok(name) => name,
        Err(error) => return error,
    };
...
fn sqlite_attrs(name: &str) -> Result<Vec<(u32, *mut PyObject)>, String> {
    ...
    let mut attrs = vec![
        ...
        function_attr("enable_callback_tracebacks", "enable_callback_tracebacks", sqlite_enable_callback_tracebacks_entry)?,
        ...
        (intern("Row"), if tuple_type.is_null() { sqlite_cursor_type().cast::<PyObject>() } else { tuple_type }),
    ...
    push_sqlite_constants!(
        SQLITE_ABORT, SQLITE_ABORT_ROLLBACK, SQLITE_ALTER_TABLE, SQLITE_ANALYZE,
        SQLITE_ATTACH, SQLITE_AUTH, SQLITE_AUTH_USER, SQLITE_BUSY,
        SQLITE_BUSY_RECOVERY, SQLITE_BUSY_SNAPSHOT, SQLITE_BUSY_TIMEOUT, SQLITE_CANTOPEN,
        SQLITE_CANTOPEN_CONVPATH, SQLITE_CANTOPEN_DIRTYWAL, SQLITE_CANTOPEN_FULLPATH, SQLITE_CANTOPEN_ISDIR,
        SQLITE_CANTOPEN_NOTEMPDIR, SQLITE_CANTOPEN_SYMLINK, SQLITE_CONSTRAINT_CHECK, SQLITE_CONSTRAINT_COMMITHOOK,
        SQLITE_CONSTRAINT_FOREIGNKEY, SQLITE_CONSTRAINT_FUNCTION, SQLITE_CONSTRAINT_NOTNULL, SQLITE_CONSTRAINT_PINNED,
        SQLITE_CONSTRAINT_PRIMARYKEY, SQLITE_CONSTRAINT_ROWID, SQLITE_CONSTRAINT_TRIGGER, SQLITE_CONSTRAINT_UNIQUE,
        SQLITE_CONSTRAINT_VTAB, SQLITE_CORRUPT, SQLITE_CORRUPT_INDEX, SQLITE_CORRUPT_SEQUENCE,
        SQLITE_CORRUPT_VTAB, SQLITE_CREATE_INDEX, SQLITE_CREATE_TABLE, SQLITE_CREATE_TEMP_INDEX,
        SQLITE_CREATE_TEMP_TABLE, SQLITE_CREATE_TEMP_TRIGGER, SQLITE_CREATE_TEMP_VIEW, SQLITE_CREATE_TRIGGER,
        SQLITE_CREATE_VIEW, SQLITE_CREATE_VTABLE, SQLITE_DBCONFIG_DEFENSIVE, SQLITE_DBCONFIG_DQS_DDL,
        ...
        SQLITE_DBCONFIG_ENABLE_QPSG, SQLITE_DBCONFIG_ENABLE_TRIGGER, SQLITE_DBCONFIG_ENABLE_VIEW, SQLITE_DBCONFIG_LEGACY_ALTER_TABLE,
        SQLITE_DBCONFIG_LEGACY_FILE_FORMAT, SQLITE_DBCONFIG_NO_CKPT_ON_CLOSE, SQLITE_DBCONFIG_RESET_DATABASE, SQLITE_DBCONFIG_TRIGGER_EQP,
        SQLITE_DBCONFIG_TRUSTED_SCHEMA, SQLITE_DBCONFIG_WRITABLE_SCHEMA, SQLITE_DELETE, SQLITE_DENY,
        SQLITE_DETACH, SQLITE_DONE, SQLITE_DROP_INDEX, SQLITE_DROP_TABLE,
        SQLITE_DROP_TEMP_INDEX, SQLITE_DROP_TEMP_TABLE, SQLITE_DROP_TEMP_TRIGGER, SQLITE_DROP_TEMP_VIEW,
        SQLITE_DROP_TRIGGER, SQLITE_DROP_VIEW, SQLITE_DROP_VTABLE, SQLITE_EMPTY,
        SQLITE_ERROR_MISSING_COLLSEQ, SQLITE_ERROR_RETRY, SQLITE_ERROR_SNAPSHOT, SQLITE_FORMAT,
        SQLITE_FULL, SQLITE_FUNCTION, SQLITE_IGNORE, SQLITE_INSERT,
        SQLITE_INTERNAL, SQLITE_INTERRUPT, SQLITE_IOERR, SQLITE_IOERR_ACCESS,
        SQLITE_IOERR_AUTH, SQLITE_IOERR_BEGIN_ATOMIC, SQLITE_IOERR_BLOCKED, SQLITE_IOERR_CHECKRESERVEDLOCK,
        SQLITE_IOERR_CLOSE, SQLITE_IOERR_COMMIT_ATOMIC, SQLITE_IOERR_CONVPATH, SQLITE_IOERR_CORRUPTFS,
        SQLITE_IOERR_DATA, SQLITE_IOERR_DELETE, SQLITE_IOERR_DELETE_NOENT, SQLITE_IOERR_DIR_CLOSE,
        SQLITE_IOERR_DIR_FSYNC, SQLITE_IOERR_FSTAT, SQLITE_IOERR_FSYNC, SQLITE_IOERR_GETTEMPPATH,
        SQLITE_IOERR_LOCK, SQLITE_IOERR_MMAP, SQLITE_IOERR_NOMEM, SQLITE_IOERR_RDLOCK,
        SQLITE_IOERR_READ, SQLITE_IOERR_ROLLBACK_ATOMIC, SQLITE_IOERR_SEEK, SQLITE_IOERR_SHMLOCK,
        SQLITE_IOERR_SHMMAP, SQLITE_IOERR_SHMOPEN, SQLITE_IOERR_SHMSIZE, SQLITE_IOERR_SHORT_READ,
        SQLITE_IOERR_TRUNCATE, SQLITE_IOERR_UNLOCK, SQLITE_IOERR_VNODE, SQLITE_IOERR_WRITE,
        SQLITE_LIMIT_ATTACHED, SQLITE_LIMIT_COLUMN, SQLITE_LIMIT_COMPOUND_SELECT, SQLITE_LIMIT_EXPR_DEPTH,
        SQLITE_LIMIT_FUNCTION_ARG, SQLITE_LIMIT_LENGTH, SQLITE_LIMIT_LIKE_PATTERN_LENGTH, SQLITE_LIMIT_SQL_LENGTH,
        SQLITE_LIMIT_TRIGGER_DEPTH, SQLITE_LIMIT_VARIABLE_NUMBER, SQLITE_LIMIT_VDBE_OP, SQLITE_LIMIT_WORKER_THREADS,
        SQLITE_LOCKED, SQLITE_LOCKED_SHAREDCACHE, SQLITE_LOCKED_VTAB, SQLITE_MISMATCH,
        SQLITE_MISUSE, SQLITE_NOLFS, SQLITE_NOMEM, SQLITE_NOTADB,
        SQLITE_NOTFOUND, SQLITE_NOTICE, SQLITE_NOTICE_RECOVER_ROLLBACK, SQLITE_NOTICE_RECOVER_WAL,
        SQLITE_OK_LOAD_PERMANENTLY, SQLITE_OK_SYMLINK, SQLITE_PERM, SQLITE_PRAGMA,
        SQLITE_PROTOCOL, SQLITE_RANGE, SQLITE_READ, SQLITE_READONLY,
        SQLITE_READONLY_CANTINIT, SQLITE_READONLY_CANTLOCK, SQLITE_READONLY_DBMOVED, SQLITE_READONLY_DIRECTORY,
        SQLITE_READONLY_RECOVERY, SQLITE_READONLY_ROLLBACK, SQLITE_RECURSIVE, SQLITE_REINDEX,
        SQLITE_ROW, SQLITE_SAVEPOINT, SQLITE_SCHEMA, SQLITE_SELECT,
        SQLITE_TOOBIG, SQLITE_TRANSACTION, SQLITE_UPDATE, SQLITE_WARNING,
...
static DBM_ERROR_CLASS: LazyLock<usize> = LazyLock::new(|| exception_class("dbm", "error", "OSError").map_or(0, |class| class as usize));
...
static DBM_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "dbm.dbm", core::mem::size_of::<PyDbm>());
    ty.tp_base = object_type();
...
unsafe extern "C" fn dbm_open_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.is_empty() || args.len() > 3 {
        return type_error(&format!("open() expected file, flag='r', mode=0o666; got {} arguments", args.len()));
    }
...
unsafe extern "C" fn dbm_subscript_slot(object: *mut PyObject, key: *mut PyObject) -> *mut PyObject {
...
unsafe extern "C" fn dbm_ass_subscript_slot(object: *mut PyObject, key: *mut PyObject, value: *mut PyObject) -> c_int {
    let dbm = match unsafe { dbm_live(object) } {
        Ok(dbm) => dbm,
        Err(_) => return -1,
    };
    ...
    let rc = unsafe { dbm_store(dbm.db, datum_from_slice(&key), datum_from_slice(&value), DBM_REPLACE) };
    if rc != 0 {
...
static SSL_CONTEXT_STATES: LazyLock<Mutex<HashMap<usize, SslContextState>>> = LazyLock::new(|| Mutex::new(HashMap::new()));

static SSL_ERROR_CLASS: LazyLock<usize> = LazyLock::new(|| exception_class("ssl", "SSLError", "OSError").map_or(0, |class| class as usize));
...
static SSL_CERT_VERIFICATION_ERROR_CLASS: LazyLock<usize> = LazyLock::new(|| {
    let base = *SSL_ERROR_CLASS as *mut PyObject;
    exception_class_from_base("ssl", "SSLCertVerificationError", base).map_or(0, |class| class as usize)
});
...
static SSL_CONTEXT_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "_ssl._SSLContext", core::mem::size_of::<PySslContext>());
    ty.tp_base = object_type();
...
static MEMORY_BIO_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "ssl.MemoryBIO", core::mem::size_of::<PyMemoryBio>());
    ty.tp_base = object_type();
...
static SSL_SESSION_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "ssl.SSLSession", core::mem::size_of::<PySslSession>());
    ty.tp_base = object_type();
    Box::into_raw(Box::new(ty)) as usize
});
...
fn build_openssl_context(_protocol: i64) -> Option<SslContext> {
    SslContextBuilder::new(SslMethod::tls()).ok().map(SslContextBuilder::build)
}
...
fn install_ssl_context_type_dict(ty: &mut PyType) {
    ...
    for (name, entry) in [
        ...
        ("__enter__", sqlite_context_enter_entry as BuiltinFn),
...
unsafe fn with_ssl_context_state<R>(object: *mut PyObject, body: impl FnOnce(&mut SslContextState) -> R) -> Option<R> {
    let object = crate::tag::untag_arg(object);
    if object.is_null() {
        return None;
    }
    ...
    if unsafe { crate::mro::is_subtype(ty, ssl_context_type()) } {
        let mut states = SSL_CONTEXT_STATES.lock().unwrap_or_else(|poison| poison.into_inner());
        return states.get_mut(&(object as usize)).map(body);
    }
    None
}

unsafe extern "C" fn ssl_context_new(cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    if !kwargs.is_null() {
    ...
    let protocol = match positional.as_slice() {
        ...
        _ => return type_error(&format!("SSLContext() expected at most 1 argument, got {}", positional.len())),
    };
...
fn ssl_context_get_member(context: &mut SslContextState, member: SslContextMember) -> *mut PyObject {
    match member {
...
fn ssl_context_set_member(context: &mut SslContextState, member: SslContextMember, value: *mut PyObject) -> c_int {
    if value.is_null() {
        ...
        SslContextMember::VerifyMode => match int_arg(value, "verify_mode") {
            Ok(mode @ (PY_SSL_CERT_NONE | PY_SSL_CERT_OPTIONAL | PY_SSL_CERT_REQUIRED)) => {
                if context.check_hostname && mode == PY_SSL_CERT_NONE {
                    let _ = value_error("Cannot set verify_mode to CERT_NONE when check_hostname is enabled.");
                    return -1;
                }
...
unsafe extern "C" fn ssl_context_descriptor_get(descr: *mut PyObject, obj: *mut PyObject, _owner: *mut PyObject) -> *mut PyObject {
    if descr.is_null() || obj.is_null() || is_none(obj) || unsafe { type_::is_type_object(crate::tag::untag_arg(obj)) } {
        return descr;
    }
...
unsafe extern "C" fn ssl_context_descriptor_set(descr: *mut PyObject, obj: *mut PyObject, value: *mut PyObject) -> c_int {
    if descr.is_null() {
        let _ = type_error("SSLContext descriptor is NULL");
        return -1;
    }
    let member = unsafe { (*descr.cast::<PySslContextDescriptor>()).member };
    match unsafe { with_ssl_context_state(obj, |context| ssl_context_set_member(context, member, value)) } {
        Some(status) => status,
        None => {
...
unsafe extern "C" fn ssl_context_descriptor_set_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn ssl_context_descriptor_get_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn ssl_context_descriptor_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    let name = match attr_name(name) {
        Ok(name) => name,
        Err(error) => return error,
    };
...
unsafe extern "C" fn ssl_context_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    let name = match attr_name(name) {
        Ok(name) => name,
        Err(error) => return error,
    };
    match name {
        "protocol" => unsafe { with_ssl_context_state(object, |context| ssl_context_get_member(context, SslContextMember::Protocol)) }
            .unwrap_or_else(|| type_error("SSLContext attribute lookup on non-context")),
        "verify_mode" => unsafe { with_ssl_context_state(object, |context| ssl_context_get_member(context, SslContextMember::VerifyMode)) }
            .unwrap_or_else(|| type_error("SSLContext attribute lookup on non-context")),
        "check_hostname" => unsafe { with_ssl_context_state(object, |context| ssl_context_get_member(context, SslContextMember::CheckHostname)) }
            .unwrap_or_else(|| type_error("SSLContext attribute lookup on non-context")),
        "options" => unsafe { with_ssl_context_state(object, |context| ssl_context_get_member(context, SslContextMember::Options)) }
            .unwrap_or_else(|| type_error("SSLContext attribute lookup on non-context")),
        "minimum_version" => unsafe { with_ssl_context_state(object, |context| ssl_context_get_member(context, SslContextMember::MinimumVersion)) }
            .unwrap_or_else(|| type_error("SSLContext attribute lookup on non-context")),
        "maximum_version" => unsafe { with_ssl_context_state(object, |context| ssl_context_get_member(context, SslContextMember::MaximumVersion)) }
            .unwrap_or_else(|| type_error("SSLContext attribute lookup on non-context")),
        "verify_flags" => unsafe { with_ssl_context_state(object, |context| ssl_context_get_member(context, SslContextMember::VerifyFlags)) }
            .unwrap_or_else(|| type_error("SSLContext attribute lookup on non-context")),
        "_host_flags" => unsafe { with_ssl_context_state(object, |context| ssl_context_get_member(context, SslContextMember::HostFlags)) }
            .unwrap_or_else(|| type_error("SSLContext attribute lookup on non-context")),
        "_msg_callback" => unsafe { with_ssl_context_state(object, |context| ssl_context_get_member(context, SslContextMember::MsgCallback)) }
            .unwrap_or_else(|| type_error("SSLContext attribute lookup on non-context")),
        "set_ciphers" => bound_method(object, "set_ciphers", ssl_context_set_ciphers_entry),
        "load_default_certs" => bound_method(object, "load_default_certs", ssl_context_load_default_certs_entry),
        "load_verify_locations" => bound_method(object, "load_verify_locations", ssl_context_load_verify_locations_entry),
        "load_cert_chain" => bound_method(object, "load_cert_chain", ssl_context_load_cert_chain_entry),
        "set_default_verify_paths" => bound_method(object, "set_default_verify_paths", ssl_context_set_default_verify_paths_entry),
        "cert_store_stats" => bound_method(object, "cert_store_stats", ssl_context_cert_store_stats_entry),
        "wrap_socket" | "_wrap_socket" | "wrap_bio" | "_wrap_bio" => bound_method(object, name, ssl_context_wrap_not_implemented_entry),
        "__enter__" => bound_method(object, "__enter__", sqlite_context_enter_entry),
...
unsafe extern "C" fn ssl_context_setattro(object: *mut PyObject, name: *mut PyObject, value: *mut PyObject) -> c_int {
    let name = match attr_name(name) {
        Ok(name) => name,
        Err(_) => return -1,
    };
    ...
    match unsafe { with_ssl_context_state(object, |context| ssl_context_set_member(context, member, value)) } {
        Some(status) => status,
        None => {
...
unsafe extern "C" fn ssl_context_set_ciphers_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn ssl_context_set_default_verify_paths_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let result = unsafe {
        with_ssl_context_state(args[0], |context| {
            ssl_rebuild_context(context, |builder| builder.set_default_verify_paths().map_err(|error| error.to_string()))
        })
    };
    match result {
        Some(Ok(())) => none(),
...
unsafe extern "C" fn ssl_context_load_default_certs_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    unsafe { ssl_context_set_default_verify_paths_entry(set_default_args.as_mut_ptr(), set_default_args.len()) }
}

unsafe extern "C" fn ssl_context_load_verify_locations_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.get(3).copied().is_some_and(|value| !value.is_null() && !is_none(value)) {
        return raise(ExceptionKind::NotImplementedError, "SSLContext.load_verify_locations(cadata=...) is not implemented");
    }
    ...
    let result = unsafe {
        with_ssl_context_state(args[0], |context| {
            ssl_rebuild_context(context, |builder| {
                builder
                    .load_verify_locations(cafile.as_deref().map(Path::new), capath.as_deref().map(Path::new))
                    .map_err(|error| error.to_string())
            })
        })
    };
...
unsafe extern "C" fn ssl_context_load_cert_chain_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "load_cert_chain") {
        Ok(args) => args,
        Err(error) => return error,
    };
    if args.is_empty() {
        return type_error("load_cert_chain() missing receiver");
    }
    if unsafe { with_ssl_context_state(args[0], |_| ()).is_none() } {
        return type_error("load_cert_chain() receiver must be SSLContext");
    }
    raise(ExceptionKind::NotImplementedError, "SSLContext.load_cert_chain() requires certificate/key loading not implemented in this runtime")
}

unsafe extern "C" fn ssl_context_cert_store_stats_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn ssl_context_wrap_not_implemented_entry(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject {
    raise(ExceptionKind::NotImplementedError, "network SSL wrapping is not implemented")
}

unsafe extern "C" fn ssl_context_exit_entry(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject {
    none()
}

unsafe extern "C" fn memory_bio_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    if !kwargs.is_null() {
...
unsafe extern "C" fn memory_bio_write_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn memory_bio_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    let name = match attr_name(name) {
        Ok(name) => name,
        Err(error) => return error,
    };
    match name {
        ...
        "pending" => unsafe { memory_bio_receiver(object) }.map_or_else(|| type_error("pending on non-MemoryBIO"), |bio| py_int(bio.buffer.len() as i64)),
        _ => unsafe { abi::exc::pon_raise_attribute_error(object, intern(name)) },
    }
}

unsafe extern "C" fn ssl_rand_status_entry(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject {
    py_bool(true)
}
...
unsafe extern "C" fn ssl_get_default_verify_paths_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn ssl_txt2obj_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let dotted = text.bytes().all(|byte| byte.is_ascii_digit() || byte == b'.');
    if !name && !dotted {
...
unsafe extern "C" fn ssl_nid2obj_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let nid = match int_arg(args[0], "nid") {
        Ok(nid) if nid >= i64::from(c_int::MIN) && nid <= i64::from(c_int::MAX) => Nid::from_raw(nid as c_int),
...
fn ssl_attrs(name: &str) -> Result<Vec<(u32, *mut PyObject)>, String> {
    ...
    Ok(vec![
        ...
        str_attr("_DEFAULT_CIPHERS", "TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA256:TLS_AES_128_GCM_SHA256")?,
        ...
        function_attr("get_default_verify_paths", "get_default_verify_paths", ssl_get_default_verify_paths_entry)?,
...
pub(super) fn make_ssl_underscore_module() -> Result<*mut PyObject, String> {
...
impl CKind {
    const fn size(self) -> usize {
        match self {
            ...
            Self::SizeT | Self::VoidP | Self::CharP | Self::WCharP | Self::PyObject => core::mem::size_of::<usize>(),
        }
    }
...
fn namespace_for_ctypes_type(kind: Option<CKind>, include_from_param: bool) -> *mut PyObject {
    ...
    unsafe {
        ...
        if include_from_param {
            let function = abi::pon_make_function(ctypes_from_param_entry as *const u8, VARIADIC_ARITY, intern("from_param"));
            if !function.is_null() {
...
static SIMPLE_CDATA_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "ctypes._SimpleCData", core::mem::size_of::<PyCData>());
    ty.tp_base = object_type();
...
fn make_c_type(kind: CKind) -> usize {
    ...
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), typename, core::mem::size_of::<PyCData>());
...
static C_LONGDOUBLE_COMPLEX_TYPE: LazyLock<usize> = LazyLock::new(|| make_c_type(CKind::LongDoubleComplex));
...
static ARRAY_TYPE: LazyLock<usize> = LazyLock::new(|| make_plain_ctypes_type("ctypes.Array", core::mem::size_of::<PyObjectHeader>()));
static STRUCTURE_TYPE: LazyLock<usize> = LazyLock::new(|| make_plain_ctypes_type("ctypes.Structure", core::mem::size_of::<PyObjectHeader>()));
static UNION_TYPE: LazyLock<usize> = LazyLock::new(|| make_plain_ctypes_type("ctypes.Union", core::mem::size_of::<PyObjectHeader>()));
static CFIELD_TYPE: LazyLock<usize> = LazyLock::new(|| make_plain_ctypes_type("ctypes.CField", core::mem::size_of::<PyObjectHeader>()));

static POINTER_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "ctypes._Pointer", core::mem::size_of::<PyPointer>());
    ty.tp_base = object_type();
...
static CDLL_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "ctypes.CDLL", core::mem::size_of::<PyCDll>());
    ty.tp_base = object_type();
...
static CFUNC_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "ctypes._CFuncPtr", core::mem::size_of::<PyCFunc>());
    ty.tp_base = object_type();
...
fn one_char_codepoint(object: *mut PyObject, name: &str) -> Result<u32, *mut PyObject> {
    ...
    if let Some(text) = unsafe { type_::unicode_text(object) } {
        ...
        let Some(ch) = chars.next() else { return Err(value_error(&format!("{name} must not be empty"))); };
        if chars.next().is_some() {
            return Err(value_error(&format!("{name} must be a single character")));
        }
        return Ok(u32::from(ch));
    }
    u32::try_from(int_arg(object, name)?).map_err(|_| value_error(&format!("{name} is out of range")))
}

fn cdata_value_from_py(kind: CKind, object: *mut PyObject) -> Result<(i128, Option<Vec<u8>>), *mut PyObject> {
    let object = crate::tag::untag_arg(object);
    if object.is_null() || is_none(object) {
        return Ok((0, None));
    }
    match kind {
        ...
        CKind::Double | CKind::LongDouble => Ok((number_to_f64_arg(object, "value")?.to_bits() as i128, None)),
        CKind::FloatComplex | CKind::DoubleComplex | CKind::LongDoubleComplex => Err(type_error("complex ctypes scalars are not implemented")),
...
unsafe extern "C" fn cdata_new(cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    if !kwargs.is_null() {
    ...
    if positional.len() > 1 {
        return type_error(&format!("{}() expected at most 1 argument, got {}", kind.public_name(), positional.len()));
    }
    cdata_new_for_type(kind, cls, positional.first().copied())
}

unsafe extern "C" fn ctypes_from_param_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    match args {
        [value] => *value,
        [_cls, value] => *value,
        _ => type_error(&format!("from_param() expected 1 argument, got {}", args.len().saturating_sub(1))),
    }
}
...
unsafe extern "C" fn cdata_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    ...
    match name {
        "value" => match cdata.kind {
            ...
            CKind::WChar => char::from_u32(cdata.value as u32).map_or_else(none, |ch| py_str(&ch.to_string())),
            ...
            CKind::VoidP => {
                if cdata.value == 0 { none() } else { py_int(cdata.value as i64) }
            }
            CKind::PyObject => {
                if cdata.value == 0 { none() } else { cdata.value as usize as *mut PyObject }
            }
...
unsafe extern "C" fn cdata_setattro(object: *mut PyObject, name: *mut PyObject, value: *mut PyObject) -> c_int {
    let name = match attr_name(name) {
        Ok(name) => name,
        Err(_) => return -1,
    };
...
unsafe extern "C" fn cdata_repr(object: *mut PyObject) -> *mut PyObject {
    ...
    py_str(&format!("{}({})", cdata.kind.public_name(), match cdata.kind { CKind::Float | CKind::Double | CKind::LongDouble => cdata_float_value(cdata).to_string(), _ => cdata.value.to_string() }))
}
...
unsafe extern "C" fn pointer_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    if !kwargs.is_null() {
...
unsafe extern "C" fn pointer_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    ...
    match name {
        ...
        "value" => {
            if pointer.address == 0 { none() } else { py_int(pointer.address as i64) }
        }
        _ => unsafe { abi::exc::pon_raise_attribute_error(object, intern(name)) },
    }
}
...
unsafe extern "C" fn ctypes_pointer_type_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "POINTER") {
        Ok(args) => args,
        Err(error) => return error,
    };
...
unsafe extern "C" fn ctypes_alignment_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    unsafe { ctypes_sizeof_entry(argv, argc) }
}

unsafe extern "C" fn ctypes_addressof_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn ctypes_get_errno_entry(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject {
    py_int(0)
}

unsafe extern "C" fn ctypes_set_errno_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn cdll_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    if !kwargs.is_null() {
    ...
    let handle = if let Some(handle) = positional.get(2).copied().filter(|object| !is_none(*object)) {
...
fn cfunc_restype_from_object(object: *mut PyObject) -> Result<(Option<CKind>, *mut PyObject), *mut PyObject> {
    let object = crate::tag::untag_arg(object);
    if object.is_null() || is_none(object) {
        return Ok((None, none()));
    }
...
fn cfunc_defaults_from_class(cls: *mut PyType) -> Result<(Option<CKind>, *mut PyObject, *mut PyObject), *mut PyObject> {
    let cls_object = cls.cast::<PyObject>();
    let restype_object = optional_attr(cls_object, "_restype_").unwrap_or_else(|| c_type_for_kind(CKind::Int).cast::<PyObject>());
...
fn cfunc_from_address(cls: *mut PyType, name: String, address: *mut c_void, restype: Option<CKind>, restype_object: *mut PyObject, argtypes: *mut PyObject) -> *mut PyObject {
    Box::into_raw(Box::new(PyCFunc {
...
unsafe extern "C" fn cfunc_new(cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    if !kwargs.is_null() {
    ...
    if positional.len() != 1 {
        return type_error(&format!("CFuncPtr() expected exactly 1 argument, got {}", positional.len()));
    }
    ...
    if let Ok(address) = int_arg(target, "address") {
        return cfunc_from_address(cls, format!("0x{:x}", address as usize), address as usize as *mut c_void, restype, restype_object, argtypes);
    }
...
fn cdata_word(cdata: &PyCData) -> usize {
    match cdata.kind {
        CKind::CharP | CKind::WCharP => cdata.bytes.as_ref().map_or(0, |bytes| bytes.as_ptr() as usize),
...
fn c_arg_word(object: *mut PyObject, keepalive: &mut Vec<Vec<u8>>) -> Result<usize, *mut PyObject> {
    ...
    if let Some(value) = unsafe { crate::types::int::to_bigint_including_bool(object) }.and_then(|value| value.to_i64()) {
...
unsafe fn call_c_address(address: *mut c_void, words: &[usize]) -> usize {
    match words {
        [] => unsafe { core::mem::transmute::<*mut c_void, unsafe extern "C" fn() -> usize>(address)() },
        [a] => unsafe { core::mem::transmute::<*mut c_void, unsafe extern "C" fn(usize) -> usize>(address)(*a) },
        [a, b] => unsafe { core::mem::transmute::<*mut c_void, unsafe extern "C" fn(usize, usize) -> usize>(address)(*a, *b) },
        [a, b, c] => unsafe { core::mem::transmute::<*mut c_void, unsafe extern "C" fn(usize, usize, usize) -> usize>(address)(*a, *b, *c) },
        [a, b, c, d] => unsafe { core::mem::transmute::<*mut c_void, unsafe extern "C" fn(usize, usize, usize, usize) -> usize>(address)(*a, *b, *c, *d) },
        [a, b, c, d, e] => unsafe { core::mem::transmute::<*mut c_void, unsafe extern "C" fn(usize, usize, usize, usize, usize) -> usize>(address)(*a, *b, *c, *d, *e) },
        [a, b, c, d, e, f] => unsafe { core::mem::transmute::<*mut c_void, unsafe extern "C" fn(usize, usize, usize, usize, usize, usize) -> usize>(address)(*a, *b, *c, *d, *e, *f) },
        _ => 0,
    }
}
...
fn c_result_to_py(kind: Option<CKind>, result: usize) -> *mut PyObject {
    match kind {
        ...
        Some(CKind::VoidP) => {
            if result == 0 { none() } else { py_int(result as i64) }
        }
        Some(CKind::PyObject) => {
            if result == 0 { none() } else { result as *mut PyObject }
        }
...
fn c_arg_word_for_kind(object: *mut PyObject, expected: Option<CKind>, keepalive: &mut Vec<Vec<u8>>) -> Result<usize, *mut PyObject> {
...
unsafe extern "C" fn cfunc_call(callee: *mut PyObject, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    if !kwargs.is_null() {
...
unsafe extern "C" fn cfunc_setattro(object: *mut PyObject, name: *mut PyObject, value: *mut PyObject) -> c_int {
    let name = match attr_name(name) {
        Ok(name) => name,
        Err(_) => return -1,
    };
...
unsafe extern "C" fn ctypes_load_library_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn ctypes_buffer_info_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "buffer_info") {
        Ok([typ]) => [*typ],
        Ok(args) => return type_error(&format!("buffer_info() expected 1 argument, got {}", args.len())),
        Err(error) => return error,
    };
...
unsafe extern "C" fn ctypes_memoryview_at_raw(ptr_value: usize, size: isize, _readonly: usize) -> *mut PyObject {
    if ptr_value == 0 {
        return value_error("NULL pointer access");
    }
    if size < 0 {
    ...
    let bytes = py_bytes(unsafe { core::slice::from_raw_parts(ptr_value as *const u8, size as usize) });
...
unsafe extern "C" fn ctypes_cast_raw(address: usize, _source: *mut PyObject, target: *mut PyObject) -> *mut PyObject {

...
use crate::types::type_::{self as type_mod, unicode_text};
...
static ERROR_CLASS: LazyLock<usize> = LazyLock::new(|| {
    exception_class("Error", "ValueError").map_or(0, |class| class as usize)
});

static INCOMPLETE_CLASS: LazyLock<usize> = LazyLock::new(|| {
    exception_class("Incomplete", "Exception").map_or(0, |class| class as usize)
});
...
fn b2a_uu_core(data: &[u8], backtick: bool) -> Result<Vec<u8>, String> {
    ...
    let encode = |value: u8| -> u8 {
        if backtick && value == 0 { 0x60 } else { value + b' ' }
    };
...
static CRC_HQX_TABLE: [u16; 256] = {
    ...
    while i < 256 {
        ...
        let mut bit = 0;
        while bit < 8 {
            value = if value & 0x8000 != 0 { (value << 1) ^ 0x1021 } else { value << 1 };
            bit += 1;
        }
        table[i] = value;
        i += 1;
...
unsafe extern "C" fn a2b_base64_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.len() > 2 {
        return raise_type_error(&format!("a2b_base64() takes at most 2 arguments ({} given)", args.len()));
    }
...
unsafe extern "C" fn b2a_base64_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.len() > 2 {
        return raise_type_error(&format!("b2a_base64() takes at most 2 arguments ({} given)", args.len()));
    }
...
unsafe fn hexlify_shared(argv: *mut *mut PyObject, argc: usize, name: &str) -> *mut PyObject {
    ...
    if args.len() > 3 {
        return raise_type_error(&format!("{name}() takes at most 3 arguments ({} given)", args.len()));
    }
...
unsafe fn unhexlify_shared(argv: *mut *mut PyObject, argc: usize, name: &str) -> *mut PyObject {
    ...
    if args.len() > 1 {
        return raise_type_error(&format!("{name}() takes exactly 1 argument ({} given)", args.len()));
    }
...
unsafe extern "C" fn a2b_qp_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.len() > 2 {
        return raise_type_error(&format!("a2b_qp() takes at most 2 arguments ({} given)", args.len()));
    }
...
unsafe extern "C" fn b2a_qp_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.len() > 4 {
        return raise_type_error(&format!("b2a_qp() takes at most 4 arguments ({} given)", args.len()));
    }
...
unsafe extern "C" fn a2b_uu_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.len() > 1 {
        return raise_type_error(&format!("a2b_uu() takes exactly 1 argument ({} given)", args.len()));
    }
...
unsafe extern "C" fn b2a_uu_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.len() > 2 {
        return raise_type_error(&format!("b2a_uu() takes at most 2 arguments ({} given)", args.len()));
    }
...
unsafe extern "C" fn crc32_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.len() > 2 {
        return raise_type_error(&format!("crc32() takes at most 2 arguments ({} given)", args.len()));
    }
...
unsafe extern "C" fn crc_hqx_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.len() != 2 {
        return raise_type_error(&format!("crc_hqx() takes exactly 2 arguments ({} given)", args.len()));
    }

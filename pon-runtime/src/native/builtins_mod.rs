...
use crate::{
	...
	object::{
		PyFunction, PyMappingMethods, PyObject, PyObjectHeader, PySequenceMethods, PyType, UnaryFunc,
	},
...
pub(crate) fn make_module() -> Result<*mut PyObject, String> {
	...
	for (constant_name, value) in [
		...
		("NotImplemented", unsafe {
			abi::pon_load_global(crate::intern::intern("NotImplemented"), core::ptr::null_mut())
		}),
		("Ellipsis", unsafe {
...
static SUPER_TYPE: LazyLock<usize> = LazyLock::new(|| {
	let mut ty =
		PyType::new(ptr::null(), "super", std::mem::size_of::<crate::types::super_::PySuper>());
	crate::types::super_::install_super_slots(&mut ty);
	Box::into_raw(Box::new(ty)) as usize
});
static CALL_SENTINEL_ITER_TYPE: LazyLock<usize> = LazyLock::new(|| {
	let mut ty =
		Box::new(PyType::new(ptr::null(), "callable_iterator", std::mem::size_of::<NativeObject>()));
...
static LONGRANGE_ITER_TYPE: LazyLock<usize> = LazyLock::new(|| {
	let mut ty =
		Box::new(PyType::new(ptr::null(), "longrange_iterator", std::mem::size_of::<NativeObject>()));
	ty.tp_iter = Some(identity_iter_slot);
	ty.tp_iternext = Some(native_next_slot);
	ty.tp_getattro = Some(crate::abstract_op::iterator_dunder_getattro);
	ty.tp_bool = Some(native_bool_slot);
...
fn alloc_native(payload: NativePayload, ty: *mut PyType) -> *mut PyObject {
	...
	let object = Box::into_raw(Box::new(NativeObject { ob_base: PyObjectHeader::new(ty), payload }))
		.cast::<PyObject>();
	if holds_refs {
...
unsafe extern "C" fn native_next_slot(object: *mut PyObject) -> *mut PyObject {
	...
	match &mut native.payload {
		...
		NativePayload::Enumerate { iter, index } => {
			...
			let pair =
				alloc_sequence(SequenceKind::Tuple, vec![unsafe { abi::pon_const_int(*index) }, value]);
			*index += 1;
			pair
		},
		NativePayload::Zip { iters } => {
...
pub(crate) fn dict_fromkeys_function() -> *mut PyObject {
	unsafe {
		abi::pon_make_function(builtin_dict_fromkeys as *const u8, VARIADIC_ARITY, intern("fromkeys"))
	}
}
...
mod tests {
	...
	fn sorted_reverse_equal_keys_preserves_order_and_calls_key_once() {
		...
		let key = unsafe {
			abi::pon_make_function(constant_zero_key as *const u8, 1, intern("constant_zero_key"))
		};
		assert!(!key.is_null());
	...
	fn slot_test_object(tp_repr: Option<UnaryFunc>, tp_str: Option<UnaryFunc>) -> *mut PyObject {
		let mut ty =
			Box::new(PyType::new(ptr::null(), "slottest", std::mem::size_of::<PyObjectHeader>()));
		ty.tp_repr = tp_repr;
		ty.tp_str = tp_str;

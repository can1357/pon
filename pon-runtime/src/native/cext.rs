...
use crate::object::{PyFunction, PyObject, PyObjectHeader, PyType};
...
use super::builtins_mod::{alloc_tuple, VARIADIC_ARITY};
...
fn bytes_arg(object: *mut PyObject, name: &str) -> Result<Vec<u8>, *mut PyObject> {
    bytes_like(object)
        .map(<[u8]>::to_vec)
        .ok_or_else(|| type_error(&format!("a bytes-like object is required for {name}, not '{}'", type_name(crate::tag::untag_arg(object)))))
}
...
fn int_arg(object: *mut PyObject, name: &str) -> Result<i64, *mut PyObject> {
    ...
    value.to_i64().ok_or_else(|| overflow_error(&format!("{name} is too large")))
}
...
fn hashlib_hash_type_slot(name: &'static str) -> usize {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), name, core::mem::size_of::<PyOpenSslHash>());
...
static HMAC_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "_hashlib.HMAC", core::mem::size_of::<PyHmac>());
    ty.tp_getattro = Some(hmac_getattro);
    Box::into_raw(Box::new(ty)) as usize
});
static HMAC_HACL_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "_hmac.HMAC", core::mem::size_of::<PyHmac>());
    ty.tp_getattro = Some(hmac_getattro);
    Box::into_raw(Box::new(ty)) as usize
});
static UNSUPPORTED_DIGESTMOD_ERROR: LazyLock<usize> = LazyLock::new(|| {
    exception_class("_hashlib", "UnsupportedDigestmodError", "ValueError").map_or(0, |class| class as usize)
});
...
fn hmac_digest_bytes(kind: DigestKind, key: &[u8], data: &[u8]) -> Vec<u8> {
    ...
    let mut key_block = if key.len() > block { digest_bytes(kind, key) } else { key.to_vec() };
...
fn alloc_hash(kind: DigestKind, data: Vec<u8>) -> *mut PyObject {
    let ty = if kind.is_xof() { *HASHXOF_TYPE } else { *HASH_TYPE } as *mut PyType;
...
unsafe fn hash_receiver<'a>(object: *mut PyObject) -> Option<&'a mut PyOpenSslHash> {
    ...
    let ty = unsafe { (*object).ob_type };
    if ty == (*HASH_TYPE as *mut PyType).cast_const() || ty == (*HASHXOF_TYPE as *mut PyType).cast_const() {
...
unsafe fn hmac_receiver<'a>(object: *mut PyObject) -> Option<&'a mut PyHmac> {
    ...
    let ty = unsafe { (*object).ob_type };
    if ty == (*HMAC_TYPE as *mut PyType).cast_const() || ty == (*HMAC_HACL_TYPE as *mut PyType).cast_const() {
...
fn parse_hash_new_args(args: &[*mut PyObject], function_name: &str) -> Result<(DigestKind, Vec<u8>), *mut PyObject> {
    if args.is_empty() {
        return Err(type_error(&format!("{function_name}() missing required argument 'name'")));
    }
    if args.len() > 3 {
        return Err(type_error(&format!("{function_name}() expected at most 3 arguments, got {}", args.len())));
    }
...
fn parse_constructor_data(args: &[*mut PyObject], kind: DigestKind) -> Result<Vec<u8>, *mut PyObject> {
    if args.len() > 2 {
        return Err(type_error(&format!("{}() expected at most 2 arguments, got {}", kind.openssl_name(), args.len())));
    }
...
fn digest_kind_arg(
    ...
    if let Some(name) = unsafe { type_::unicode_text(object) } {
        return DigestKind::from_name(name)
            ...
            .ok_or_else(|| raise_class(error_class, ExceptionKind::ValueError, &format!("unsupported hash type {name}")));
    }
    if unsafe { crate::types::int::type_name_is(object, "function") } {
        ...
        if let Some(name) = crate::intern::resolve(name_id) {
            ...
            if let Some(kind) = DigestKind::from_name(normalized).filter(|kind| DigestKind::HMAC.contains(kind)) {
                return Ok(kind);
            }
        }
...
fn parse_hmac_args(
    ...
    let kind = match args.get(2).copied() {
        Some(object) if !object.is_null() && !is_none(object) => digest_kind_arg(object, "digestmod", error_class)?,
        _ => return Err(type_error("Missing required argument 'digestmod'.")),
    };
    Ok((key, msg, kind))
}

unsafe extern "C" fn hashlib_hmac_new_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn hmac_compute_digest_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
macro_rules! fixed_hmac_compute {
    ($entry:ident, $kind:expr) => {
        unsafe extern "C" fn $entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
            ...
            if args.len() != 2 {
                return type_error(&format!("{}() expected 2 arguments, got {}", stringify!($entry), args.len()));
            }
...
unsafe extern "C" fn compare_digest_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let (Some(left_bytes), Some(right_bytes)) = (bytes_like(left), bytes_like(right)) else {
        let (Some(left_text), Some(right_text)) = (unsafe { type_::unicode_text(left) }, unsafe { type_::unicode_text(right) }) else {
...
unsafe extern "C" fn pbkdf2_hmac_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let Some(kind) = DigestKind::from_name(&digest_name).filter(|kind| DigestKind::HMAC.contains(kind)) else {
...
fn pbkdf2_hmac(kind: DigestKind, password: &[u8], salt: &[u8], iterations: usize, dklen: usize) -> Vec<u8> {
...
unsafe extern "C" fn scrypt_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.len() < 6 || args.len() > 7 {
        return type_error(&format!("scrypt() expected password, salt, n, r, p, maxmem, dklen; got {} arguments", args.len()));
    }
...
static BZ2_COMPRESSOR_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "_bz2.BZ2Compressor", core::mem::size_of::<PyBz2Compressor>());
...
static BZ2_DECOMPRESSOR_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "_bz2.BZ2Decompressor", core::mem::size_of::<PyBz2Decompressor>());
    ty.tp_base = abi::runtime_global(intern("object")).map_or(ptr::null_mut(), |object| object.cast::<PyType>());
...
unsafe extern "C" fn bz2_compressor_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    let positional = match unsafe { type_::positional_args_from_object(args) } {
        Ok(values) => values,
        Err(message) => return type_error(&message),
    };
    if positional.len() > 1 {
        return type_error(&format!("BZ2Compressor() expected at most 1 argument, got {}", positional.len()));
    }
    ...
    if !kwargs.is_null() {
        ...
        for entry in entries {
            ...
            match key {
                ...
                other => return type_error(&format!("BZ2Compressor() got an unexpected keyword argument '{other}'")),
            }
        }
    }
...
unsafe extern "C" fn bz2_decompressor_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    let positional = match unsafe { type_::positional_args_from_object(args) } {
        Ok(values) => values,
        Err(message) => return type_error(&message),
    };
...
unsafe fn bz2_compressor_receiver<'a>(object: *mut PyObject) -> Option<&'a mut PyBz2Compressor> {
    let object = crate::tag::untag_arg(object);
    (!object.is_null() && unsafe { (*object).ob_type == (*BZ2_COMPRESSOR_TYPE as *mut PyType).cast_const() })
        .then(|| unsafe { &mut *object.cast::<PyBz2Compressor>() })
}

unsafe fn bz2_decompressor_receiver<'a>(object: *mut PyObject) -> Option<&'a mut PyBz2Decompressor> {
    let object = crate::tag::untag_arg(object);
    (!object.is_null() && unsafe { (*object).ob_type == (*BZ2_DECOMPRESSOR_TYPE as *mut PyType).cast_const() })
...
unsafe extern "C" fn bz2_compress_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if raw.len() != 2 {
        return type_error(&format!("compress() expected 1 argument, got {}", raw.len().saturating_sub(1)));
    }
...
unsafe extern "C" fn bz2_flush_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if raw.len() != 1 {
        return type_error(&format!("flush() expected no arguments, got {}", raw.len().saturating_sub(1)));
    }
    let Some(this) = (unsafe { bz2_compressor_receiver(raw[0]) }) else {
    ...
    this.flushed = true;
    let mut encoder = ::bzip2::write::BzEncoder::new(Vec::new(), ::bzip2::Compression::new(this.level));
...
fn take_decompressed(output: &[u8], offset: &mut usize, max_length: i64) -> Vec<u8> {
    ...
    let take = if max_length < 0 { remaining } else { remaining.min(max_length as usize) };
...
unsafe extern "C" fn bz2_decompress_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if raw.len() < 2 || raw.len() > 3 {
        return type_error(&format!("decompress() expected 1 or 2 arguments, got {}", raw.len().saturating_sub(1)));
    }
...
unsafe extern "C" fn bz2_compressor_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    let name = crate::tag::untag_arg(name);
    let Some(name_text) = (unsafe { type_::unicode_text(name) }) else {
        return type_error("attribute name must be str");
    };
...
unsafe extern "C" fn bz2_decompressor_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    let name = crate::tag::untag_arg(name);
    let Some(name_text) = (unsafe { type_::unicode_text(name) }) else {
        return type_error("attribute name must be str");
    };
...
static LZMA_COMPRESSOR_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "_lzma.LZMACompressor", core::mem::size_of::<PyLzmaCompressor>());
    ty.tp_base = abi::runtime_global(intern("object")).map_or(ptr::null_mut(), |object| object.cast::<PyType>());
...
static LZMA_DECOMPRESSOR_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "_lzma.LZMADecompressor", core::mem::size_of::<PyLzmaDecompressor>());
    ty.tp_base = abi::runtime_global(intern("object")).map_or(ptr::null_mut(), |object| object.cast::<PyType>());
...
unsafe extern "C" fn lzma_compressor_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    let positional = match unsafe { type_::positional_args_from_object(args) } {
        Ok(values) => values,
        Err(message) => return type_error(&message),
    };
    ...
    if positional.len() > 4 {
        return type_error(&format!("LZMACompressor() expected at most 4 arguments, got {}", positional.len()));
    }
    if !kwargs.is_null() {
        let entries = match unsafe { crate::types::dict::dict_entries_snapshot(kwargs) } {
            Ok(entries) => entries,
        ...
        for entry in entries {
            ...
            let index = match key {
                ...
                "preset" => 2,
                "filters" => 3,
                other => return type_error(&format!("LZMACompressor() got an unexpected keyword argument '{other}'")),
            };
            slots[index] = entry.value;
        }
    }
    let format = if slots[0].is_null() || is_none(slots[0]) { 1 } else { match int_arg(slots[0], "format") { Ok(v) => v, Err(e) => return e } };
    if format != 1 {
    ...
    let preset = if slots[2].is_null() || is_none(slots[2]) { 6 } else { match int_arg(slots[2], "preset") { Ok(v) => v, Err(e) => return e } };
...
unsafe extern "C" fn lzma_decompressor_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    let positional = match unsafe { type_::positional_args_from_object(args) } {
        Ok(values) => values,
        Err(message) => return type_error(&message),
    };
    ...
    if positional.len() > 3 {
        return type_error(&format!("LZMADecompressor() expected at most 3 arguments, got {}", positional.len()));
    }
    if !kwargs.is_null() {
        let entries = match unsafe { crate::types::dict::dict_entries_snapshot(kwargs) } {
            Ok(entries) => entries,
        ...
        for entry in entries {
            ...
            let index = match key {
                ...
                "memlimit" => 1,
                "filters" => 2,
                other => return type_error(&format!("LZMADecompressor() got an unexpected keyword argument '{other}'")),
            };
            slots[index] = entry.value;
        }
    }
    let format = if slots[0].is_null() || is_none(slots[0]) { 0 } else { match int_arg(slots[0], "format") { Ok(v) => v, Err(e) => return e } };
    if format != 0 && format != 1 {
        return not_implemented("pon _lzma currently supports FORMAT_AUTO/FORMAT_XZ decompression only");
    }
...
unsafe fn lzma_compressor_receiver<'a>(object: *mut PyObject) -> Option<&'a mut PyLzmaCompressor> {
    let object = crate::tag::untag_arg(object);
    (!object.is_null() && unsafe { (*object).ob_type == (*LZMA_COMPRESSOR_TYPE as *mut PyType).cast_const() })
        .then(|| unsafe { &mut *object.cast::<PyLzmaCompressor>() })
}

unsafe fn lzma_decompressor_receiver<'a>(object: *mut PyObject) -> Option<&'a mut PyLzmaDecompressor> {
    let object = crate::tag::untag_arg(object);
    (!object.is_null() && unsafe { (*object).ob_type == (*LZMA_DECOMPRESSOR_TYPE as *mut PyType).cast_const() })
...
unsafe extern "C" fn lzma_compress_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let raw = match args_or_type_error(argv, argc, "compress") { Ok(raw) => raw, Err(error) => return error };
    if raw.len() != 2 {
        return type_error(&format!("compress() expected 1 argument, got {}", raw.len().saturating_sub(1)));
    }
    let Some(this) = (unsafe { lzma_compressor_receiver(raw[0]) }) else { return type_error("compress() receiver must be LZMACompressor") };
    if this.flushed { return value_error("Compressor has been flushed"); }
    let data = match bytes_arg(raw[1], "data") { Ok(data) => data, Err(error) => return error };
    this.input.extend_from_slice(&data);
    py_bytes(&[])
}

unsafe extern "C" fn lzma_flush_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let raw = match args_or_type_error(argv, argc, "flush") { Ok(raw) => raw, Err(error) => return error };
    if raw.len() != 1 { return type_error(&format!("flush() expected no arguments, got {}", raw.len().saturating_sub(1))); }
    let Some(this) = (unsafe { lzma_compressor_receiver(raw[0]) }) else { return type_error("flush() receiver must be LZMACompressor") };
    if this.flushed { return value_error("Repeated call to flush()"); }
    this.flushed = true;
    let mut encoder = ::xz2::write::XzEncoder::new(Vec::new(), this.preset);
    if let Err(error) = encoder.write_all(&this.input) { return raise_lzma(&format!("LZMA compression failed: {error}")); }
    match encoder.finish() { Ok(bytes) => py_bytes(&bytes), Err(error) => raise_lzma(&format!("LZMA compression failed: {error}")) }
}
...
unsafe extern "C" fn lzma_decompress_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let raw = match args_or_type_error(argv, argc, "decompress") { Ok(raw) => raw, Err(error) => return error };
    if raw.len() < 2 || raw.len() > 3 { return type_error(&format!("decompress() expected 1 or 2 arguments, got {}", raw.len().saturating_sub(1))); }
    let Some(this) = (unsafe { lzma_decompressor_receiver(raw[0]) }) else { return type_error("decompress() receiver must be LZMADecompressor") };
    let data = match bytes_arg(raw[1], "data") { Ok(data) => data, Err(error) => return error };
    let max_length = match raw.get(2).copied() { Some(object) => match int_arg(object, "max_length") { Ok(value) => value, Err(error) => return error }, None => -1 };
    if max_length < -1 { return value_error("max_length must be non-negative"); }
    if this.eof {
        if !data.is_empty() { this.unused_data.extend_from_slice(&data); }
        return py_bytes(&[]);
    }
    this.input.extend_from_slice(&data);
    match lzma_decode_xz(&this.input) {
        ...
        Err(_) if this.input.len() < 6 => { this.needs_input = true; py_bytes(&[]) }
        Err(message) => raise_lzma(&message),
    }
}

unsafe extern "C" fn lzma_compressor_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    let name = crate::tag::untag_arg(name);
    let Some(name_text) = (unsafe { type_::unicode_text(name) }) else { return type_error("attribute name must be str") };
    match name_text {
...
unsafe extern "C" fn lzma_decompressor_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    let name = crate::tag::untag_arg(name);
    let Some(name_text) = (unsafe { type_::unicode_text(name) }) else { return type_error("attribute name must be str") };
    let Some(this) = (unsafe { lzma_decompressor_receiver(object) }) else { return type_error("attribute receiver must be LZMADecompressor") };
    match name_text {
        "eof" => py_bool(this.eof),
        "needs_input" => py_bool(this.needs_input),
        "unused_data" => py_bytes(&this.unused_data),
...
unsafe extern "C" fn lzma_is_check_supported(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "is_check_supported") { Ok(args) => args, Err(error) => return error };
    if args.len() != 1 { return type_error(&format!("is_check_supported() expected 1 argument, got {}", args.len())); }
...
unsafe extern "C" fn lzma_filter_props(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject {
    not_implemented("LZMA filter property encoding/decoding requires liblzma filter-chain APIs not yet exposed by pon")
}

pub(super) fn make_lzma_module() -> Result<*mut PyObject, String> {
    let error_class = *LZMA_ERROR_CLASS;
    if error_class == 0 { return Err("failed to create _lzma.LZMAError".to_owned()); }
    let mut attrs = vec![module_name_attr("_lzma")?];
    attrs.push((intern("LZMACompressor"), (*LZMA_COMPRESSOR_TYPE as *mut PyType).cast::<PyObject>()));
    attrs.push((intern("LZMADecompressor"), (*LZMA_DECOMPRESSOR_TYPE as *mut PyType).cast::<PyObject>()));
    attrs.push((intern("LZMAError"), error_class as *mut PyObject));
    for (name, value) in [
        ("FORMAT_AUTO", 0), ("FORMAT_XZ", 1), ("FORMAT_ALONE", 2), ("FORMAT_RAW", 3),
        ("CHECK_NONE", 0), ("CHECK_CRC32", 1), ("CHECK_CRC64", 4), ("CHECK_SHA256", 10), ("CHECK_ID_MAX", 15), ("CHECK_UNKNOWN", 16),
        ("FILTER_LZMA1", 4_611_686_018_427_387_905_i64), ("FILTER_LZMA2", 33), ("FILTER_DELTA", 3), ("FILTER_X86", 4), ("FILTER_POWERPC", 5), ("FILTER_IA64", 6), ("FILTER_ARM", 7), ("FILTER_ARMTHUMB", 8), ("FILTER_SPARC", 9),
        ("MF_HC3", 3), ("MF_HC4", 4), ("MF_BT2", 18), ("MF_BT3", 19), ("MF_BT4", 20),
        ("MODE_FAST", 1), ("MODE_NORMAL", 2), ("PRESET_DEFAULT", 6), ("PRESET_EXTREME", 2_147_483_648_i64),
    ] {
        attrs.push(int_attr(name, value)?);
    }
    attrs.push(function_attr("is_check_supported", "is_check_supported", lzma_is_check_supported)?);
    attrs.push(function_attr("_encode_filter_properties", "_encode_filter_properties", lzma_filter_props)?);
    attrs.push(function_attr("_decode_filter_properties", "_decode_filter_properties", lzma_filter_props)?);
...
#[repr(C)]
struct PyZstdCompressor { ob_base: PyObjectHeader, input: Vec<u8>, level: i32, last_mode: i64 }
#[repr(C)]
struct PyZstdDecompressor { ob_base: PyObjectHeader, input: Vec<u8>, output: Vec<u8>, offset: usize, eof: bool, needs_input: bool, unused_data: Vec<u8> }
#[repr(C)]
struct PyZstdDict { ob_base: PyObjectHeader, content: Vec<u8> }

static ZSTD_ERROR_CLASS: LazyLock<usize> = LazyLock::new(|| exception_class("_zstd", "ZstdError", "Exception").map_or(0, |class| class as usize));

static ZSTD_COMPRESSOR_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "_zstd.ZstdCompressor", core::mem::size_of::<PyZstdCompressor>());
    ty.tp_base = abi::runtime_global(intern("object")).map_or(ptr::null_mut(), |object| object.cast::<PyType>());
    ty.tp_new = Some(zstd_compressor_new); ty.tp_getattro = Some(zstd_compressor_getattro);
    let namespace = type_::new_namespace();
    if !namespace.is_null() {
        unsafe { (*namespace).set(intern("CONTINUE"), py_int(0)); (*namespace).set(intern("FLUSH_BLOCK"), py_int(1)); (*namespace).set(intern("FLUSH_FRAME"), py_int(2)); }
        ty.tp_dict = namespace.cast::<PyObject>();
    }
    Box::into_raw(Box::new(ty)) as usize
});
static ZSTD_DECOMPRESSOR_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "_zstd.ZstdDecompressor", core::mem::size_of::<PyZstdDecompressor>());
    ty.tp_base = abi::runtime_global(intern("object")).map_or(ptr::null_mut(), |object| object.cast::<PyType>());
    ty.tp_new = Some(zstd_decompressor_new); ty.tp_getattro = Some(zstd_decompressor_getattro); Box::into_raw(Box::new(ty)) as usize
});
static ZSTD_DICT_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "_zstd.ZstdDict", core::mem::size_of::<PyZstdDict>());
    ty.tp_base = abi::runtime_global(intern("object")).map_or(ptr::null_mut(), |object| object.cast::<PyType>());
    ty.tp_new = Some(zstd_dict_new); ty.tp_getattro = Some(zstd_dict_getattro); Box::into_raw(Box::new(ty)) as usize
});

fn raise_zstd(text: &str) -> *mut PyObject { raise_class(&ZSTD_ERROR_CLASS, ExceptionKind::ValueError, text) }

unsafe extern "C" fn zstd_compressor_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    let positional = match unsafe { type_::positional_args_from_object(args) } { Ok(v) => v, Err(m) => return type_error(&m) };
    let mut level = 3_i64;
    if let Some(first) = positional.first().copied() { if !is_none(first) { match int_arg(first, "level") { Ok(v) => level = v, Err(e) => return e } } }
    if positional.len() > 4 { return type_error(&format!("ZstdCompressor() expected at most 4 arguments, got {}", positional.len())); }
    if !kwargs.is_null() {
        let entries = match unsafe { crate::types::dict::dict_entries_snapshot(kwargs) } { Ok(e) => e, Err(m) => return type_error(&m) };
        for entry in entries {
            let Some(key) = (unsafe { type_::unicode_text(crate::tag::untag_arg(entry.key)) }) else { return type_error("ZstdCompressor() keywords must be strings") };
            match key { "level" => if !is_none(entry.value) { match int_arg(entry.value, "level") { Ok(v) => level = v, Err(e) => return e } }, "options" | "zstd_dict" => {}, other => return type_error(&format!("ZstdCompressor() got an unexpected keyword argument '{other}'")) }
        }
    }
    Box::into_raw(Box::new(PyZstdCompressor { ob_base: PyObjectHeader::new(*ZSTD_COMPRESSOR_TYPE as *mut PyType), input: Vec::new(), level: level as i32, last_mode: -1 })).cast::<PyObject>()
}

unsafe extern "C" fn zstd_decompressor_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    let positional = match unsafe { type_::positional_args_from_object(args) } { Ok(v) => v, Err(m) => return type_error(&m) };
    if positional.len() > 2 { return type_error(&format!("ZstdDecompressor() expected at most 2 arguments, got {}", positional.len())); }
    if !kwargs.is_null() {
        let entries = match unsafe { crate::types::dict::dict_entries_snapshot(kwargs) } { Ok(e) => e, Err(m) => return type_error(&m) };
        for entry in entries {
            let Some(key) = (unsafe { type_::unicode_text(crate::tag::untag_arg(entry.key)) }) else { return type_error("ZstdDecompressor() keywords must be strings") };
            match key { "options" | "zstd_dict" => {}, other => return type_error(&format!("ZstdDecompressor() got an unexpected keyword argument '{other}'")) }
        }
    }
    Box::into_raw(Box::new(PyZstdDecompressor { ob_base: PyObjectHeader::new(*ZSTD_DECOMPRESSOR_TYPE as *mut PyType), input: Vec::new(), output: Vec::new(), offset: 0, eof: false, needs_input: true, unused_data: Vec::new() })).cast::<PyObject>()
}

unsafe extern "C" fn zstd_dict_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    let positional = match unsafe { type_::positional_args_from_object(args) } { Ok(v) => v, Err(m) => return type_error(&m) };
    if positional.len() != 1 || !kwargs.is_null() { return type_error("ZstdDict() expects exactly one bytes-like argument"); }
    let content = match bytes_arg(positional[0], "dict_content") { Ok(v) => v, Err(e) => return e };
    Box::into_raw(Box::new(PyZstdDict { ob_base: PyObjectHeader::new(*ZSTD_DICT_TYPE as *mut PyType), content })).cast::<PyObject>()
}

unsafe fn zstd_compressor_receiver<'a>(object: *mut PyObject) -> Option<&'a mut PyZstdCompressor> { let object = crate::tag::untag_arg(object); (!object.is_null() && unsafe { (*object).ob_type == (*ZSTD_COMPRESSOR_TYPE as *mut PyType).cast_const() }).then(|| unsafe { &mut *object.cast::<PyZstdCompressor>() }) }
unsafe fn zstd_decompressor_receiver<'a>(object: *mut PyObject) -> Option<&'a mut PyZstdDecompressor> { let object = crate::tag::untag_arg(object); (!object.is_null() && unsafe { (*object).ob_type == (*ZSTD_DECOMPRESSOR_TYPE as *mut PyType).cast_const() }).then(|| unsafe { &mut *object.cast::<PyZstdDecompressor>() }) }
unsafe fn zstd_dict_receiver<'a>(object: *mut PyObject) -> Option<&'a mut PyZstdDict> { let object = crate::tag::untag_arg(object); (!object.is_null() && unsafe { (*object).ob_type == (*ZSTD_DICT_TYPE as *mut PyType).cast_const() }).then(|| unsafe { &mut *object.cast::<PyZstdDict>() }) }

unsafe extern "C" fn zstd_compress_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let raw = match args_or_type_error(argv, argc, "compress") { Ok(r) => r, Err(e) => return e };
    if raw.len() < 2 || raw.len() > 3 { return type_error(&format!("compress() expected 1 or 2 arguments, got {}", raw.len().saturating_sub(1))); }
    let Some(this) = (unsafe { zstd_compressor_receiver(raw[0]) }) else { return type_error("compress() receiver must be ZstdCompressor") };
    let data = match bytes_arg(raw[1], "data") { Ok(v) => v, Err(e) => return e };
    let mode = match raw.get(2).copied() { Some(o) if !is_none(o) => match int_arg(o, "mode") { Ok(v) => v, Err(e) => return e }, _ => 0 };
    this.input.extend_from_slice(&data); this.last_mode = mode;
    if mode == 2 { match ::zstd::stream::encode_all(Cursor::new(&this.input), this.level) { Ok(bytes) => { this.input.clear(); py_bytes(&bytes) }, Err(error) => raise_zstd(&format!("zstd compress failed: {error}")) } } else { py_bytes(&[]) }
}

unsafe extern "C" fn zstd_flush_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let raw = match args_or_type_error(argv, argc, "flush") { Ok(r) => r, Err(e) => return e };
    if raw.len() > 2 { return type_error(&format!("flush() expected at most 1 argument, got {}", raw.len().saturating_sub(1))); }
    let Some(this) = (unsafe { zstd_compressor_receiver(raw[0]) }) else { return type_error("flush() receiver must be ZstdCompressor") };
    let mode = match raw.get(1).copied() { Some(o) if !is_none(o) => match int_arg(o, "mode") { Ok(v) => v, Err(e) => return e }, _ => 1 };
    this.last_mode = mode;
    if mode == 2 || mode == 1 { match ::zstd::stream::encode_all(Cursor::new(&this.input), this.level) { Ok(bytes) => { this.input.clear(); py_bytes(&bytes) }, Err(error) => raise_zstd(&format!("zstd flush failed: {error}")) } } else { value_error("Invalid mode argument") }
}

unsafe extern "C" fn zstd_set_pledged_size(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let raw = match args_or_type_error(argv, argc, "set_pledged_input_size") { Ok(r) => r, Err(e) => return e };
    if raw.is_empty() { return type_error("set_pledged_input_size() missing receiver"); }
    none()
}

unsafe extern "C" fn zstd_decompress_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let raw = match args_or_type_error(argv, argc, "decompress") { Ok(r) => r, Err(e) => return e };
    if raw.len() < 2 || raw.len() > 3 { return type_error(&format!("decompress() expected 1 or 2 arguments, got {}", raw.len().saturating_sub(1))); }
    let Some(this) = (unsafe { zstd_decompressor_receiver(raw[0]) }) else { return type_error("decompress() receiver must be ZstdDecompressor") };
    let data = match bytes_arg(raw[1], "data") { Ok(v) => v, Err(e) => return e };
    let max_length = match raw.get(2).copied() { Some(o) if !is_none(o) => match int_arg(o, "max_length") { Ok(v) => v, Err(e) => return e }, _ => -1 };
    if max_length < -1 { return value_error("max_length must be non-negative"); }
    if this.eof { if !data.is_empty() { this.unused_data.extend_from_slice(&data); } return py_bytes(&[]); }
    this.input.extend_from_slice(&data);
    match ::zstd::stream::decode_all(Cursor::new(&this.input)) {
        Ok(out) => { this.output = out; this.eof = true; this.unused_data.clear(); let chunk = take_decompressed(&this.output, &mut this.offset, max_length); this.needs_input = this.offset >= this.output.len(); py_bytes(&chunk) }
        Err(_error) if this.input.len() < 8 => { this.needs_input = true; py_bytes(&[]) }
        Err(error) => raise_zstd(&format!("zstd decompress failed: {error}")),
    }
}

unsafe extern "C" fn zstd_compressor_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    let name = crate::tag::untag_arg(name); let Some(name_text) = (unsafe { type_::unicode_text(name) }) else { return type_error("attribute name must be str") };
    let Some(this) = (unsafe { zstd_compressor_receiver(object) }) else { return type_error("attribute receiver must be ZstdCompressor") };
    match name_text { "last_mode" => py_int(this.last_mode), "compress" => bound_method(object, "compress", zstd_compress_method), "flush" => bound_method(object, "flush", zstd_flush_method), "set_pledged_input_size" => bound_method(object, "set_pledged_input_size", zstd_set_pledged_size), _ => unsafe { abi::exc::pon_raise_attribute_error(object, intern(name_text)) } }
}
unsafe extern "C" fn zstd_decompressor_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    let name = crate::tag::untag_arg(name); let Some(name_text) = (unsafe { type_::unicode_text(name) }) else { return type_error("attribute name must be str") };
    let Some(this) = (unsafe { zstd_decompressor_receiver(object) }) else { return type_error("attribute receiver must be ZstdDecompressor") };
    match name_text { "eof" => py_bool(this.eof), "needs_input" => py_bool(this.needs_input), "unused_data" => py_bytes(&this.unused_data), "decompress" => bound_method(object, "decompress", zstd_decompress_method), _ => unsafe { abi::exc::pon_raise_attribute_error(object, intern(name_text)) } }
}
unsafe extern "C" fn zstd_dict_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    let name = crate::tag::untag_arg(name); let Some(name_text) = (unsafe { type_::unicode_text(name) }) else { return type_error("attribute name must be str") };
    let Some(this) = (unsafe { zstd_dict_receiver(object) }) else { return type_error("attribute receiver must be ZstdDict") };
    match name_text { "dict_content" => py_bytes(&this.content), "is_raw" => py_bool(false), _ => unsafe { abi::exc::pon_raise_attribute_error(object, intern(name_text)) } }
}

unsafe extern "C" fn zstd_get_frame_size(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "get_frame_size") { Ok(a) => a, Err(e) => return e }; if args.len()!=1 { return type_error(&format!("get_frame_size() expected 1 argument, got {}", args.len())); }
    let data = match bytes_arg(args[0], "frame_buffer") { Ok(v) => v, Err(e) => return e };
    match ::zstd::stream::decode_all(Cursor::new(&data)) { Ok(out) => py_int(out.len() as i64), Err(error) => raise_zstd(&format!("zstd frame decode failed: {error}")) }
}
...
unsafe extern "C" fn zstd_get_param_bounds(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "get_param_bounds") { Ok(a) => a, Err(e) => return e }; if args.is_empty() { return type_error("get_param_bounds() missing parameter"); }
    let param = match int_arg(args[0], "parameter") { Ok(v) => v, Err(e) => return e };
    let (lo, hi) = if param == 100 { (-131_072, 22) } else { (0, 2_147_483_647) };
    alloc_tuple(vec![py_int(lo), py_int(hi)])
}
unsafe extern "C" fn zstd_train_dict(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject { not_implemented("zstd dictionary training requires zstd experimental dictionary APIs not yet exposed by pon") }
unsafe extern "C" fn zstd_finalize_dict(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject { not_implemented("zstd dictionary finalization requires zstd experimental dictionary APIs not yet exposed by pon") }
unsafe extern "C" fn zstd_set_parameter_types(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject { none() }

pub(super) fn make_zstd_module() -> Result<*mut PyObject, String> {
    let error_class = *ZSTD_ERROR_CLASS; if error_class == 0 { return Err("failed to create _zstd.ZstdError".to_owned()); }
    let mut attrs = vec![module_name_attr("_zstd")?];
    attrs.push((intern("ZstdCompressor"), (*ZSTD_COMPRESSOR_TYPE as *mut PyType).cast::<PyObject>())); attrs.push((intern("ZstdDecompressor"), (*ZSTD_DECOMPRESSOR_TYPE as *mut PyType).cast::<PyObject>())); attrs.push((intern("ZstdDict"), (*ZSTD_DICT_TYPE as *mut PyType).cast::<PyObject>())); attrs.push((intern("ZstdError"), error_class as *mut PyObject));
    attrs.push(int_attr("zstd_version_number", ::zstd::zstd_safe::version_number() as i64)?); attrs.push(int_attr("ZSTD_CLEVEL_DEFAULT", ::zstd::DEFAULT_COMPRESSION_LEVEL as i64)?); attrs.push(int_attr("ZSTD_DStreamOutSize", 131_072)?);
    for (name, value) in [("ZSTD_c_compressionLevel",100),("ZSTD_c_windowLog",101),("ZSTD_c_hashLog",102),("ZSTD_c_chainLog",103),("ZSTD_c_searchLog",104),("ZSTD_c_minMatch",105),("ZSTD_c_targetLength",106),("ZSTD_c_strategy",107),("ZSTD_c_enableLongDistanceMatching",160),("ZSTD_c_ldmHashLog",161),("ZSTD_c_ldmMinMatch",162),("ZSTD_c_ldmBucketSizeLog",163),("ZSTD_c_ldmHashRateLog",164),("ZSTD_c_contentSizeFlag",200),("ZSTD_c_checksumFlag",201),("ZSTD_c_dictIDFlag",202),("ZSTD_c_nbWorkers",400),("ZSTD_c_jobSize",401),("ZSTD_c_overlapLog",402),("ZSTD_d_windowLogMax",100),("ZSTD_fast",1),("ZSTD_dfast",2),("ZSTD_greedy",3),("ZSTD_lazy",4),("ZSTD_lazy2",5),("ZSTD_btlazy2",6),("ZSTD_btopt",7),("ZSTD_btultra",8),("ZSTD_btultra2",9)] { attrs.push(int_attr(name, value)?); }
    attrs.push(str_attr("zstd_version", ::zstd::zstd_safe::version_string())?); attrs.push(function_attr("get_frame_size", "get_frame_size", zstd_get_frame_size)?); attrs.push(function_attr("get_frame_info", "get_frame_info", zstd_get_frame_info)?); attrs.push(function_attr("get_param_bounds", "get_param_bounds", zstd_get_param_bounds)?); attrs.push(function_attr("train_dict", "train_dict", zstd_train_dict)?); attrs.push(function_attr("finalize_dict", "finalize_dict", zstd_finalize_dict)?); attrs.push(function_attr("set_parameter_types", "set_parameter_types", zstd_set_parameter_types)?);
...
unsafe extern "C" fn uuid_generate_time_safe(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject {
...
fn fill_random(bytes: &mut [u8]) {
    if let Ok(mut file) = std::fs::File::open("/dev/urandom") {
        if file.read_exact(bytes).is_ok() { return; }
    }
    let nanos = SystemTime::now().duration_since(UNIX_EPOCH).map_or(0, |duration| duration.as_nanos() as u64);
    let mut x = nanos ^ (bytes.as_ptr() as usize as u64);
    for byte in bytes {
        x ^= x << 13; x ^= x >> 7; x ^= x << 17; *byte = x as u8;
    }
}

fn uuid_v1_bytes() -> [u8; 16] {
    let unix_100ns = SystemTime::now().duration_since(UNIX_EPOCH).map_or(0, |duration| duration.as_nanos() / 100);
    let timestamp = unix_100ns + 0x01B2_1DD2_1381_4000_u128;
    let mut random = [0_u8; 8]; fill_random(&mut random);
    let clock_seq = (u16::from_be_bytes([random[0], random[1]]) & 0x3fff) | 0x8000;
    let mut node = [0_u8; 6]; node.copy_from_slice(&random[2..8]); node[0] |= 0x01;
    ...
    let mut out = [0_u8; 16]; out[0..4].copy_from_slice(&time_low.to_be_bytes()); out[4..6].copy_from_slice(&time_mid.to_be_bytes()); out[6..8].copy_from_slice(&time_hi.to_be_bytes()); out[8..10].copy_from_slice(&clock_seq.to_be_bytes()); out[10..16].copy_from_slice(&node); out
}

pub(super) fn make_uuid_module() -> Result<*mut PyObject, String> {
    install_module("_uuid", vec![module_name_attr("_uuid")?, bool_attr("has_stable_extractable_node", false)?, bool_attr("has_uuid_generate_time_safe", true)?, function_attr("generate_time_safe", "generate_time_safe", uuid_generate_time_safe)?])
}

...
//!
//! 1. `lookup(name)` normalizes the name (lowercase, spaces -> underscores:
//!    the C-level normalization; the cache is keyed by this form).
...
//!    `codecs.CodecInfo` — fetched from the already-imported `codecs`
//!    module — wrapping this module's own `utf_8_encode`/... function
//!    objects, exactly the pairing `encodings/utf_8.py` would produce.
//!    (`import codecs` never calls `lookup` at module scope, so the class
//!    exists by the time any lookup can run.)
//! 3. Otherwise registered search functions run in registration order; on a
//!    full miss the vendored `encodings` package is imported once (lazily,
//!    like CPython's registry init) and the search functions run again.
...
use crate::types::{bytearray_ as bytearray_type, bytes_ as bytes_type, type_ as type_mod};

use super::builtins_mod::{VARIADIC_ARITY, alloc_tuple};
...
pub(super) fn make_module() -> Result<*mut PyObject, String> {
    ...
    for (name, entry) in [
        ...
        let handler = unsafe {
            abi::pon_make_function(entry as *const u8, VARIADIC_ARITY, intern(&format!("{name}_errors")))
        };
        if handler.is_null() {
...
fn encode_error_message(codec: BuiltinCodec, text: &str, start: usize, end: usize, limit: u32) -> String {
    if end - start == 1 {
...
fn encode_charrange(codec: BuiltinCodec, text: &str, errors: &str, limit: u32) -> Result<Vec<u8>, CoreError> {
    ...
    while pos < chars.len() {
        ...
        match errors {
            "strict" => return Err(CoreError::Encode(encode_error_message(codec, text, start, end, limit))),
            "ignore" => {}
...
fn utf8_decode_error_message(bytes: &[u8], start: usize, span: usize, reason: &str) -> String {
    if span == 1 {
        format!("'utf-8' codec can't decode byte 0x{:02x} in position {}: {}", bytes[start], start, reason)
    } else {
        format!("'utf-8' codec can't decode bytes in position {}-{}: {}", start, start + span - 1, reason)
    }
}
...
pub(crate) fn utf8_decode_core(bytes: &[u8], errors: &str, final_: bool) -> Result<(String, usize), CoreError> {
...
fn run_search_functions(c_norm: &str) -> Result<*mut PyObject, ()> {
    ...
    loop {
        ...
        for function in snapshot {
            ...
            let result = unsafe { abi::pon_call(function as *mut PyObject, call_args.as_mut_ptr(), 1) };
            if result.is_null() {
                return Err(());
            }
        ...
        // SAFETY: Import entry point follows the NULL-sentinel error contract.
        let module = unsafe { crate::import::pon_import_name(intern("encodings"), ptr::null(), 0, 0) };
        if module.is_null() {
            return Err(());
        }
    }
...
unsafe extern "C" fn unregister_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    {
        let mut state = state();
        state.search_functions.retain(|&registered| registered != function);
...
unsafe extern "C" fn register_error_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    state().error_handlers.insert(name.to_owned(), handler as usize);
    none()
}
...
fn call_codec_info(info: *mut PyObject, method: &str, object: *mut PyObject, errors: &str) -> *mut PyObject {
...
macro_rules! fixed_codec_entries {
    ($encode_entry:ident, $encode_core:expr, $decode_entry:ident, $decode_core:expr, $pyname:literal) => {
        unsafe extern "C" fn $encode_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
            let Some(args) = (unsafe { arg_slice(argv, argc) }) else {
...
fn exception_end(exc: *mut PyObject, handler: &str) -> Result<i64, *mut PyObject> {
    ...
    int_of(untag(end))
        .ok_or_else(|| raise_type_error(&format!("{handler} error handler requires an int 'end' attribute")))
}
...
unsafe extern "C" fn replace_errors_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let start = {
        ...
        match int_of(untag(start)) {
            Some(start) => start,
            None => return raise_type_error("replace error handler requires an int 'start' attribute"),
        }
    };
    ...
    let replacement = match unsafe { crate::types::dict::type_name(exc) } {
        ...
        _ => {
            return raise_type_error("don't know how to handle this exception in the replace error callback");
        }
    };
...
/// [`encode_str_to_vec`].
pub(crate) fn decode_bytes_to_string(data: &[u8], encoding: &str, errors: &str) -> Result<String, ()> {
    if let Some(codec) = builtin_codec(&collapse_normalize(encoding)) {
        let decoded = match codec {
...
mod tests {
    ...
    fn utf8_decode_classifies_errors_like_cpython() {
        ...
        let CoreError::Decode(message) = error else { panic!("expected decode error") };
        assert_eq!(message, "'utf-8' codec can't decode byte 0xc3 in position 1: invalid continuation byte");

        let error = utf8_decode_core(b"a\xe2\x82", "strict", true).unwrap_err();
        let CoreError::Decode(message) = error else { panic!("expected decode error") };
        assert_eq!(message, "'utf-8' codec can't decode bytes in position 1-2: unexpected end of data");
        ...
        assert_eq!(utf8_decode_core(b"gr\xc3\xbc\xc3\x9fe", "strict", true).unwrap(), ("grüße".to_owned(), 7));
    }
    ...
    fn utf8_decode_handlers_replace_and_ignore() {
        ...
        assert_eq!(utf8_decode_core(b"a\xffb", "backslashreplace", true).unwrap().0, "a\\xffb");
    ...
    fn charrange_encode_matches_cpython_messages() {
        ...
        let CoreError::Encode(message) = error else { panic!("expected encode error") };
        assert_eq!(message, "'ascii' codec can't encode character '\\xe9' in position 1: ordinal not in range(128)");

        let error = ascii_encode_core("aħħb", "strict").unwrap_err();
        let CoreError::Encode(message) = error else { panic!("expected encode error") };
        assert_eq!(message, "'ascii' codec can't encode characters in position 1-2: ordinal not in range(128)");

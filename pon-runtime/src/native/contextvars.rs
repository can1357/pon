...
//!
//! `Lib/_py_warnings.py` binds one module-level `ContextVar('warnings_context')`
//! and calls `.get()`/`.set(value)`; `Lib/contextvars.py` re-exports `Context`,
//! `ContextVar`, `Token`, and `copy_context`.  The implementation is a correct
//! simple model, not a HAMT: each thread owns a current [`PyContext`] (an
//! insertion-ordered association list keyed by `ContextVar` identity), `set`
//! mutates the current context in place and returns a single-use [`PyToken`]
//! snapshot, and `Context.run` swaps the thread's current context around the
//! call (mirroring how CPython swaps the thread state's context stack).
//!
...
pub(super) fn make_module() -> Result<*mut PyObject, String> {
    ...
    let copy_context = unsafe {
        abi::pon_make_function(copy_context_entry as *const u8, VARIADIC_ARITY, intern("copy_context"))
    };
...
fn alloc_contextvar(name: String, name_obj: *mut PyObject, default: *mut PyObject) -> *mut PyObject {
    register(
...
unsafe extern "C" fn contextvar_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    ...
    if positional.len() > 1 {
        let message = format!("ContextVar() takes exactly 1 positional argument ({} given)", positional.len());
...
unsafe extern "C" fn contextvar_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
...
unsafe extern "C" fn var_set_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let slot = context.entries.iter_mut().find(|(entry_var, _)| *entry_var == receiver);
...
unsafe extern "C" fn var_reset_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if token.old_value.is_null() {
        context.entries.retain(|(entry_var, _)| *entry_var != receiver);
    } else {
        match context.entries.iter_mut().find(|(entry_var, _)| *entry_var == receiver) {
...
unsafe extern "C" fn token_new(_cls: *mut PyType, _args: *mut PyObject, _kwargs: *mut PyObject) -> *mut PyObject {
...
unsafe extern "C" fn context_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    let positional = match unsafe { crate::types::type_::positional_args_from_object(args) } {
        Ok(positional) => positional,
        Err(message) => return fail(message),
    };
...
unsafe extern "C" fn context_run_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if context.entered {
        let message = format!("cannot enter context: {:#x} is already entered", untag(receiver).addr());
    ...
    let result = unsafe {
        abi::pon_call(
            callable,
            if call_args.is_empty() { ptr::null_mut() } else { call_args.as_ptr().cast_mut() },
            call_args.len(),
        )
    };

...
pub(super) fn make_module() -> Result<*mut PyObject, String> {
	...
	attrs.push(phase_b_function_attr("list_dialects", list_dialects_entry, &[], 0, &mut [], None)?);
	attrs.push(phase_b_function_attr(
		"writer",
		writer_entry,
		&["fileobj", "dialect"],
...
fn alloc_dialect(config: DialectConfig) -> *mut PyObject {
	Box::into_raw(Box::new(CsvDialect { ob_base: PyObjectHeader::new(dialect_type()), config }))
		.cast::<PyObject>()
}

fn alloc_writer(file: *mut PyObject, dialect: DialectConfig) -> *mut PyObject {
	Box::into_raw(Box::new(CsvWriter { ob_base: PyObjectHeader::new(writer_type()), file, dialect }))
		.cast::<PyObject>()
}
...
fn raise_csv_error(message: &str) -> *mut PyObject {
	...
	let exception =
		unsafe { abi::pon_call(csv_error_type().cast::<PyObject>(), args.as_mut_ptr(), args.len()) };

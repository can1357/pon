...
use crate::types::{bytearray_ as bytearray_type, bytes_ as bytes_type, memoryview as memoryview_type, type_::{self, unicode_text}};
...
unsafe extern "C" {
    ...
    fn c_color_content(color: c_short, red: *mut c_short, green: *mut c_short, blue: *mut c_short) -> c_int;
...
static CURSES_ERROR: LazyLock<usize> =
    LazyLock::new(|| exception_class("_curses", "error", "Exception").map_or(0, |class| class as usize));
static PANEL_ERROR: LazyLock<usize> =
    LazyLock::new(|| exception_class("_curses_panel", "error", "Exception").map_or(0, |class| class as usize));
...
fn exception_class(module: &str, name: &str, base: &str) -> Result<*mut PyObject, String> {
    ...
    let class = unsafe { crate::types::type_::build_class_from_namespace(name, &[base_class], namespace, &[]) };
    if class.is_null() {
...
fn args_or_type_error<'a>(argv: *mut *mut PyObject, argc: usize, function: &str) -> Result<&'a [*mut PyObject], *mut PyObject> {
    unsafe { arg_slice(argv, argc) }.ok_or_else(|| type_error(&format!("{function}() received a null argument vector")))
}
...
fn c_int_arg(object: *mut PyObject, name: &str) -> Result<c_int, *mut PyObject> {
    c_int::try_from(int_arg(object, name)?).map_err(|_| overflow_error(&format!("{name} is out of range")))
}

fn c_short_arg(object: *mut PyObject, name: &str) -> Result<c_short, *mut PyObject> {
    c_short::try_from(int_arg(object, name)?).map_err(|_| overflow_error(&format!("{name} is out of range")))
}
...
fn cstring_bytes_arg(object: *mut PyObject, name: &str) -> Result<CString, *mut PyObject> {
    CString::new(bytes_or_text_arg(object, name)?).map_err(|_| value_error("embedded null character"))
}
...
fn alloc_window(window: *mut CWindow, owned: bool) -> *mut PyObject {
    ...
    WINDOW_REGISTRY.lock().unwrap_or_else(|poison| poison.into_inner()).push(object as usize);
...
fn alloc_panel(panel: *mut CPanel, window_object: *mut PyObject) -> *mut PyObject {
    ...
    PANEL_REGISTRY.lock().unwrap_or_else(|poison| poison.into_inner()).push(object as usize);
    object.cast::<PyObject>()
}
...
unsafe fn no_args_status(argv: *mut *mut PyObject, argc: usize, name: &str, f: unsafe extern "C" fn() -> c_int) -> *mut PyObject {
...
unsafe fn no_args_bool(argv: *mut *mut PyObject, argc: usize, name: &str, f: unsafe extern "C" fn() -> c_int) -> *mut PyObject {
    if let Err(error) = check_started() {
        return error;
    }
    match args_or_type_error(argv, argc, name) {
...
unsafe extern "C" fn curses_initscr(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    match args_or_type_error(argv, argc, "initscr") {
        Ok([]) => {}
        Ok(args) => return type_error(&format!("initscr() takes no arguments ({} given)", args.len())),
...
unsafe extern "C" fn curses_endwin(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    match args_or_type_error(argv, argc, "endwin") {
        Ok([]) => {}
        Ok(args) => return type_error(&format!("endwin() takes no arguments ({} given)", args.len())),
        Err(error) => return error,
    }
...
unsafe extern "C" fn curses_setupterm(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "setupterm") {
        ...
        Ok(args) => return type_error(&format!("setupterm() takes at most 2 arguments ({} given)", args.len())),
        Err(error) => return error,
    };
    ...
    let mut errret = 0;
    let term_ptr = term_storage.as_ref().map_or(ptr::null(), |term| term.as_ptr());
...
unsafe extern "C" fn curses_set_escdelay(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "set_escdelay") {
        Ok([value]) => [*value],
        Ok(args) => return type_error(&format!("set_escdelay() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
...
unsafe extern "C" fn curses_set_tabsize(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "set_tabsize") {
        Ok([value]) => [*value],
        Ok(args) => return type_error(&format!("set_tabsize() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
...
unsafe extern "C" fn curses_has_extended_color_support(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    match args_or_type_error(argv, argc, "has_extended_color_support") {
        Ok([]) => py_bool(false),
        Ok(args) => type_error(&format!("has_extended_color_support() takes no arguments ({} given)", args.len())),
        Err(error) => error,
    }
}
...
unsafe extern "C" fn curses_use_env(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "use_env") {
        Ok([flag]) => [*flag],
        Ok(args) => return type_error(&format!("use_env() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
...
unsafe extern "C" fn curses_start_color(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    match args_or_type_error(argv, argc, "start_color") {
        Ok([]) => {}
        Ok(args) => return type_error(&format!("start_color() takes no arguments ({} given)", args.len())),
        Err(error) => return error,
    }
...
unsafe extern "C" fn curses_one_int_status(argv: *mut *mut PyObject, argc: usize, name: &str, f: unsafe extern "C" fn(c_int) -> c_int) -> *mut PyObject {
    if let Err(error) = check_started() {
        return error;
    }
    let args = match args_or_type_error(argv, argc, name) {
        Ok([value]) => [*value],
        Ok(args) => return type_error(&format!("{name}() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
...
unsafe extern "C" fn curses_napms(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject { unsafe { curses_one_int_status(argv, argc, "napms", c_napms) } }
unsafe extern "C" fn curses_halfdelay(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject { unsafe { curses_one_int_status(argv, argc, "halfdelay", c_halfdelay) } }
unsafe extern "C" fn curses_delay_output(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject { unsafe { curses_one_int_status(argv, argc, "delay_output", c_delay_output) } }
unsafe extern "C" fn curses_mouseinterval(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject { unsafe { curses_one_int_status(argv, argc, "mouseinterval", c_mouseinterval) } }
unsafe extern "C" fn curses_typeahead(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject { unsafe { curses_one_int_status(argv, argc, "typeahead", c_typeahead) } }
...
unsafe extern "C" fn curses_curs_set(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "curs_set") {
        ...
        Ok(args) => return type_error(&format!("curs_set() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
...
unsafe extern "C" fn curses_intrflush(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    if let Err(error) = check_started() { return error; }
    let args = match args_or_type_error(argv, argc, "intrflush") {
        Ok([flag]) => [*flag],
        Ok(args) => return type_error(&format!("intrflush() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let flag = match bool_arg(args[0], "flag") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_meta(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "meta") {
        Ok([flag]) => [*flag],
        Ok(args) => return type_error(&format!("meta() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let flag = match bool_arg(args[0], "flag") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_color_pair(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "color_pair") {
        Ok([pair]) => [*pair],
        Ok(args) => return type_error(&format!("color_pair() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let pair = match int_arg(args[0], "pair_number") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_pair_number(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "pair_number") {
        Ok([attr]) => [*attr],
        Ok(args) => return type_error(&format!("pair_number() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let attr = match int_arg(args[0], "attr") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_init_pair(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "init_pair") {
        ...
        Ok(args) => return type_error(&format!("init_pair() takes exactly 3 arguments ({} given)", args.len())),
        Err(error) => return error,
    };
    let pair = match c_short_arg(args[0], "pair_number") { Ok(value) => value, Err(error) => return error };
    let fg = match c_short_arg(args[1], "fg") { Ok(value) => value, Err(error) => return error };
    let bg = match c_short_arg(args[2], "bg") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_pair_content(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "pair_content") {
        Ok([pair]) => [*pair],
        Ok(args) => return type_error(&format!("pair_content() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let pair = match c_short_arg(args[0], "pair_number") { Ok(value) => value, Err(error) => return error };
    let mut fg = 0;
    ...
    if items.iter().any(|item| item.is_null()) { return ptr::null_mut(); }
...
unsafe extern "C" fn curses_init_color(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "init_color") {
        ...
        Ok(args) => return type_error(&format!("init_color() takes exactly 4 arguments ({} given)", args.len())),
        Err(error) => return error,
    };
    let color = match c_short_arg(args[0], "color_number") { Ok(value) => value, Err(error) => return error };
    let red = match c_short_arg(args[1], "red") { Ok(value) => value, Err(error) => return error };
    let green = match c_short_arg(args[2], "green") { Ok(value) => value, Err(error) => return error };
    let blue = match c_short_arg(args[3], "blue") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_color_content(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "color_content") {
        Ok([color]) => [*color],
        Ok(args) => return type_error(&format!("color_content() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let color = match c_short_arg(args[0], "color_number") { Ok(value) => value, Err(error) => return error };
    let mut red = 0;
...
unsafe extern "C" fn curses_use_default_colors(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    if let Err(error) = check_started() { return error; }
    match args_or_type_error(argv, argc, "use_default_colors") {
        Ok([]) => status_none(unsafe { c_use_default_colors() }, "use_default_colors"),
        Ok(args) => type_error(&format!("use_default_colors() takes no arguments ({} given)", args.len())),
        Err(error) => error,
    }
}

unsafe extern "C" fn curses_assume_default_colors(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    if let Err(error) = check_started() { return error; }
    let args = match args_or_type_error(argv, argc, "assume_default_colors") {
        Ok([fg, bg]) => [*fg, *bg],
        Ok(args) => return type_error(&format!("assume_default_colors() takes exactly 2 arguments ({} given)", args.len())),
        Err(error) => return error,
    };
    let fg = match c_int_arg(args[0], "fg") { Ok(value) => value, Err(error) => return error };
    let bg = match c_int_arg(args[1], "bg") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_keyname(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "keyname") {
        Ok([ch]) => [*ch],
        Ok(args) => return type_error(&format!("keyname() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let ch = match c_int_arg(args[0], "ch") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_unctrl(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "unctrl") {
        Ok([ch]) => [*ch],
        Ok(args) => return type_error(&format!("unctrl() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let ch = match c_int_arg(args[0], "ch") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_setsyx(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "setsyx") {
        Ok([y, x]) => [*y, *x],
        Ok(args) => return type_error(&format!("setsyx() takes exactly 2 arguments ({} given)", args.len())),
        Err(error) => return error,
    };
    let y = match c_int_arg(args[0], "y") { Ok(value) => value, Err(error) => return error };
    let x = match c_int_arg(args[1], "x") { Ok(value) => value, Err(error) => return error };
    let window = unsafe { NEWSCR };
    if window.is_null() {
    ...
    let status = if y == -1 && x == -1 {
        ...
        if leave_status == -1 { -1 } else { unsafe { c_wmove(window, y, x) } }
    };
...
unsafe extern "C" fn curses_tparm(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "tparm") {
        ...
        Ok(args) => return type_error(&format!("tparm() expected a capability string and at most 9 parameters ({} given)", args.len())),
        Err(error) => return error,
    };
    let cap = match cstring_bytes_arg(args[0], "str") { Ok(value) => value, Err(error) => return error };
    let mut params = [0 as libc::c_long; 9];
    for (index, param) in args[1..].iter().enumerate() {
        let value = match int_arg(*param, "parameter") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_ungetch(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "ungetch") {
        Ok([ch]) => [*ch],
        Ok(args) => return type_error(&format!("ungetch() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let ch = match c_int_arg(args[0], "ch") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_unget_wch(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "unget_wch") {
        Ok([ch]) => [*ch],
        Ok(args) => return type_error(&format!("unget_wch() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let value = if let Ok(value) = int_arg(args[0], "ch") {
        ...
        let Some(ch) = chars.next() else { return value_error("unget_wch() argument must not be empty"); };
...
unsafe extern "C" fn curses_ungetmouse(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "ungetmouse") {
        Ok([event]) => [*event],
        Ok(args) => return type_error(&format!("ungetmouse() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    ...
    let id = match c_short_arg(*id, "id") { Ok(value) => value, Err(error) => return error };
    let x = match c_int_arg(*x, "x") { Ok(value) => value, Err(error) => return error };
    let y = match c_int_arg(*y, "y") { Ok(value) => value, Err(error) => return error };
    let z = match c_int_arg(*z, "z") { Ok(value) => value, Err(error) => return error };
    let bstate = match int_arg(*bstate, "bstate") { Ok(value) => value as libc::c_ulong, Err(error) => return error };
...
unsafe extern "C" fn curses_has_key(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "has_key") {
        Ok([ch]) => [*ch],
        Ok(args) => return type_error(&format!("has_key() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let ch = match c_int_arg(args[0], "ch") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_tigetflag(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "tigetflag") {
        Ok([cap]) => [*cap],
        Ok(args) => return type_error(&format!("tigetflag() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let cap = match cstring_arg(args[0], "capname") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_tigetnum(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "tigetnum") {
        Ok([cap]) => [*cap],
        Ok(args) => return type_error(&format!("tigetnum() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let cap = match cstring_arg(args[0], "capname") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_tigetstr(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "tigetstr") {
        Ok([cap]) => [*cap],
        Ok(args) => return type_error(&format!("tigetstr() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let cap = match cstring_arg(args[0], "capname") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_putp(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "putp") {
        Ok([text]) => [*text],
        Ok(args) => return type_error(&format!("putp() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let text = match cstring_arg(args[0], "text") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_newwin(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "newwin") {
        ...
        Ok(args) => return type_error(&format!("newwin() takes 2 or 4 arguments ({} given)", args.len())),
        Err(error) => return error,
    };
    let (lines, cols, y, x) = if args.len() == 2 {
        (0, 0, args[0], args[1])
    } else {
        (match c_int_arg(args[0], "nlines") { Ok(value) => value, Err(error) => return error },
         match c_int_arg(args[1], "ncols") { Ok(value) => value, Err(error) => return error },
         args[2], args[3])
    };
    let y = match c_int_arg(y, "begin_y") { Ok(value) => value, Err(error) => return error };
    let x = match c_int_arg(x, "begin_x") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_newpad(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "newpad") {
        ...
        Ok(args) => return type_error(&format!("newpad() takes exactly 2 arguments ({} given)", args.len())),
        Err(error) => return error,
    };
    let lines = match c_int_arg(args[0], "nlines") { Ok(value) => value, Err(error) => return error };
    let cols = match c_int_arg(args[1], "ncols") { Ok(value) => value, Err(error) => return error };
    alloc_window(unsafe { c_newpad(lines, cols) }, true)
}

unsafe extern "C" fn curses_update_lines_cols(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    match args_or_type_error(argv, argc, "update_lines_cols") {
        Ok([]) => { update_lines_cols_attrs(); none() }
        Ok(args) => type_error(&format!("update_lines_cols() takes no arguments ({} given)", args.len())),
        Err(error) => error,
    }
}
...
unsafe fn resize_common(argv: *mut *mut PyObject, argc: usize, name: &str, f: unsafe extern "C" fn(c_int, c_int) -> c_int) -> *mut PyObject {
    if let Err(error) = check_started() { return error; }
    let args = match args_or_type_error(argv, argc, name) {
        Ok([lines, cols]) => [*lines, *cols],
        Ok(args) => return type_error(&format!("{name}() takes exactly 2 arguments ({} given)", args.len())),
        Err(error) => return error,
    };
    let lines = match c_int_arg(args[0], "nlines") { Ok(value) => value, Err(error) => return error };
    let cols = match c_int_arg(args[1], "ncols") { Ok(value) => value, Err(error) => return error };
    let result = status_none(unsafe { f(lines, cols) }, name);
    if !result.is_null() { update_lines_cols_attrs(); }
    result
}

unsafe extern "C" fn curses_is_term_resized(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    if let Err(error) = check_started() { return error; }
    let args = match args_or_type_error(argv, argc, "is_term_resized") {
        Ok([lines, cols]) => [*lines, *cols],
        Ok(args) => return type_error(&format!("is_term_resized() takes exactly 2 arguments ({} given)", args.len())),
        Err(error) => return error,
    };
    let lines = match c_int_arg(args[0], "nlines") { Ok(value) => value, Err(error) => return error };
    let cols = match c_int_arg(args[1], "ncols") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn curses_mousemask(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let args = match args_or_type_error(argv, argc, "mousemask") {
        Ok([mask]) => [*mask],
        Ok(args) => return type_error(&format!("mousemask() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
    let mask = match int_arg(args[0], "newmask") { Ok(value) => value as u64, Err(error) => return error };
...
unsafe fn window_receiver<'a>(args: &'a [*mut PyObject], method: &str) -> Result<(&'a mut PyCursesWindow, &'a [*mut PyObject]), *mut PyObject> {
...
unsafe extern "C" fn window_keypad(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "keypad") { Ok(args) => args, Err(error) => return error };
    let (window, rest) = match unsafe { window_receiver(args, "keypad") } { Ok(value) => value, Err(error) => return error };
    let [flag] = rest else { return type_error(&format!("keypad() takes exactly 1 argument ({} given)", rest.len())); };
    let flag = match bool_arg(*flag, "flag") { Ok(value) => value, Err(error) => return error };
...
fn window_noarg_status(argv: *mut *mut PyObject, argc: usize, name: &str, f: unsafe extern "C" fn(*mut CWindow) -> c_int) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, name) { Ok(args) => args, Err(error) => return error };
    let (window, rest) = match unsafe { window_receiver(args, name) } { Ok(value) => value, Err(error) => return error };
    if !rest.is_empty() {
...
unsafe extern "C" fn window_move(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "move") { Ok(args) => args, Err(error) => return error };
    let (window, rest) = match unsafe { window_receiver(args, "move") } { Ok(value) => value, Err(error) => return error };
    let [y, x] = rest else { return type_error(&format!("move() takes exactly 2 arguments ({} given)", rest.len())); };
    let y = match c_int_arg(*y, "y") { Ok(value) => value, Err(error) => return error };
    let x = match c_int_arg(*x, "x") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn window_addstr(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "addstr") { Ok(args) => args, Err(error) => return error };
    let (window, rest) = match unsafe { window_receiver(args, "addstr") } { Ok(value) => value, Err(error) => return error };
    ...
    let text = match cstring_arg(text_obj, "str") { Ok(value) => value, Err(error) => return error };
    status_none(unsafe { c_waddnstr(window.window, text.as_ptr(), text.as_bytes().len() as c_int) }, "addstr")
}

unsafe extern "C" fn window_getmaxyx(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "getmaxyx") { Ok(args) => args, Err(error) => return error };
    let (window, rest) = match unsafe { window_receiver(args, "getmaxyx") } { Ok(value) => value, Err(error) => return error };
    if !rest.is_empty() {
        return type_error(&format!("getmaxyx() takes no arguments ({} given)", rest.len()));
    }
    let items = vec![py_int(i64::from(unsafe { c_getmaxy(window.window) })), py_int(i64::from(unsafe { c_getmaxx(window.window) }))];
    if items.iter().any(|item| item.is_null()) { return ptr::null_mut(); }
    super::builtins_mod::alloc_tuple(items)
}
...
unsafe extern "C" fn panel_new_panel(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "new_panel") {
        Ok([window]) => [*window],
        Ok(args) => return type_error(&format!("new_panel() takes exactly 1 argument ({} given)", args.len())),
        Err(error) => return error,
    };
...
unsafe extern "C" fn panel_update_panels(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    match args_or_type_error(argv, argc, "update_panels") {
        Ok([]) => { unsafe { c_update_panels() }; none() }
...
unsafe fn panel_receiver<'a>(args: &'a [*mut PyObject], method: &str) -> Result<(&'a mut PyCursesPanel, &'a [*mut PyObject]), *mut PyObject> {
    let Some((&receiver, rest)) = args.split_first() else {
        return Err(type_error(&format!("{method}() requires a receiver")));
    };
...
fn panel_noarg_status(argv: *mut *mut PyObject, argc: usize, name: &str, f: unsafe extern "C" fn(*mut CPanel) -> c_int) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, name) { Ok(args) => args, Err(error) => return error };
    let (panel, rest) = match unsafe { panel_receiver(args, name) } { Ok(value) => value, Err(error) => return error };
    if !rest.is_empty() {
        return type_error(&format!("{name}() takes no arguments ({} given)", rest.len()));
    }
...
unsafe extern "C" fn panel_hide(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject { panel_noarg_status(argv, argc, "hide", c_hide_panel) }
unsafe extern "C" fn panel_show(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject { panel_noarg_status(argv, argc, "show", c_show_panel) }
unsafe extern "C" fn panel_top(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject { panel_noarg_status(argv, argc, "top", c_top_panel) }
unsafe extern "C" fn panel_bottom(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject { panel_noarg_status(argv, argc, "bottom", c_bottom_panel) }

unsafe extern "C" fn panel_hidden(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "hidden") { Ok(args) => args, Err(error) => return error };
    let (panel, rest) = match unsafe { panel_receiver(args, "hidden") } { Ok(value) => value, Err(error) => return error };
    if !rest.is_empty() { return type_error(&format!("hidden() takes no arguments ({} given)", rest.len())); }
    py_bool(unsafe { c_panel_hidden(panel.panel) } != 0)
}

unsafe extern "C" fn panel_move(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "move") { Ok(args) => args, Err(error) => return error };
    let (panel, rest) = match unsafe { panel_receiver(args, "move") } { Ok(value) => value, Err(error) => return error };
    let [y, x] = rest else { return type_error(&format!("move() takes exactly 2 arguments ({} given)", rest.len())); };
    let y = match c_int_arg(*y, "y") { Ok(value) => value, Err(error) => return error };
    let x = match c_int_arg(*x, "x") { Ok(value) => value, Err(error) => return error };
...
unsafe extern "C" fn panel_replace(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "replace") { Ok(args) => args, Err(error) => return error };
    let (panel, rest) = match unsafe { panel_receiver(args, "replace") } { Ok(value) => value, Err(error) => return error };
    let [window_obj] = rest else { return type_error(&format!("replace() takes exactly 1 argument ({} given)", rest.len())); };
    let Some(window) = (unsafe { as_window(*window_obj) }) else { return type_error("replace() argument must be a _curses.window"); };
    if window.window.is_null() { return raise_panel_error("window is closed"); }
    let status = unsafe { c_replace_panel(panel.panel, window.window) };
    if status != -1 { panel.window_object = *window_obj; }
    status_panel_none(status, "replace")
}

unsafe extern "C" fn panel_window(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = match args_or_type_error(argv, argc, "window") { Ok(args) => args, Err(error) => return error };
    let (panel, rest) = match unsafe { panel_receiver(args, "window") } { Ok(value) => value, Err(error) => return error };
    if !rest.is_empty() { return type_error(&format!("window() takes no arguments ({} given)", rest.len())); }

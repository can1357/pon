...
use crate::abi::{self, pon_const_bool, pon_const_str, pon_make_function};
...
unsafe extern "C" fn faulthandler_is_enabled(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn faulthandler_dump_traceback(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn faulthandler_dump_c_stack(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn faulthandler_dump_traceback_later(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let generation = DUMP_LATER_GENERATION.fetch_add(1, Ordering::SeqCst).wrapping_add(1);
    std::thread::spawn(move || {
        loop {
            ...
            eprintln!("Timeout ({seconds} seconds)!\nStack (most recent call first):\n  <Pon native faulthandler dump>");
            if exit {
                std::process::abort();
            }
            if !repeat {
                let _ = DUMP_LATER_GENERATION.compare_exchange(generation, generation.wrapping_add(1), Ordering::SeqCst, Ordering::SeqCst);
                return;
            }
        }
    });
...
unsafe extern "C" fn faulthandler_cancel_dump_traceback_later(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn faulthandler_unregister(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn faulthandler_sigsegv(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn faulthandler_sigabrt(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject {
    std::process::abort();
}

unsafe extern "C" fn faulthandler_read_null(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject {
    unsafe { libc::raise(libc::SIGSEGV) };
    none()
}

unsafe extern "C" fn faulthandler_stack_overflow(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject {
    std::process::abort();
}

unsafe extern "C" fn faulthandler_fatal_error_c_thread(_argv: *mut *mut PyObject, _argc: usize) -> *mut PyObject {
...
fn write_dump(file: Option<*mut PyObject>, text: &str) -> *mut PyObject {
    if let Some(file) = file {
        let method = unsafe { crate::abstract_op::get_attr(crate::tag::untag_arg(file), intern("write")) };
        if method.is_null() {
            return ptr::null_mut();
        }
...
fn optional_args<'a>(argv: *mut *mut PyObject, argc: usize, max: usize, function: &str) -> Result<&'a [*mut PyObject], *mut PyObject> {
    let args = unsafe { arg_slice(argv, argc) }.ok_or_else(|| raise_type_error("invalid argument vector"))?;
    if args.len() > max {
        return Err(raise_type_error(&format!("{function} expected at most {max} arguments, got {argc}")));
    }
    Ok(args)
}

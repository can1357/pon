...
use super::builtins_mod::{VARIADIC_ARITY, alloc_list};
...
fn unsupported(name: &str) -> *mut PyObject {
    crate::abi::exc::raise_kind_error_text(
        ...
        &format!("gc.{name} is not supported: pon's collector does not expose CPython object-tracking introspection"),
    )
}
...
unsafe extern "C" fn gc_set_debug(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let Some(flags) = int_value(unsafe { *argv }) else {
        return crate::abi::exc::raise_kind_error_text(ExceptionKind::TypeError, "an integer is required");
    };
...
unsafe extern "C" fn gc_set_threshold(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    if argc == 0 || argc > 3 || argv.is_null() {
        return crate::abi::return_null_with_error("gc.set_threshold expected one to three arguments");
    }
    let mut thresholds = THRESHOLDS.lock().unwrap_or_else(|poison| poison.into_inner());
...
unsafe extern "C" fn gc_get_threshold(_argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let thresholds = THRESHOLDS.lock().unwrap_or_else(|poison| poison.into_inner());

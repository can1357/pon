...
fn member_list_object(members: &[String]) -> *mut PyObject {
    ...
    unsafe { abi::seq::pon_build_list(if objects.is_empty() { ptr::null_mut() } else { objects.as_mut_ptr() }, objects.len()) }
}
...
fn gid_arg(object: *mut PyObject) -> Result<libc::gid_t, *mut PyObject> {
    ...
    libc::gid_t::try_from(value).map_err(|_| raise_key_error(&format!("getgrgid(): gid not found: {value}")))
}

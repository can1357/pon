...
use ::blake2::digest::{Output, Update as _};
use ::md5::Digest as _;
...
fn hash_type_slot(name: &'static str) -> usize {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), name, core::mem::size_of::<PyHash>());
...
static MD5_TYPE: LazyLock<usize> = LazyLock::new(|| hash_type_slot(HashKind::Md5.constructor_name()));
static SHA1_TYPE: LazyLock<usize> = LazyLock::new(|| hash_type_slot(HashKind::Sha1.constructor_name()));
static SHA3_224_TYPE: LazyLock<usize> = LazyLock::new(|| hash_type_slot(HashKind::Sha3_224.constructor_name()));
static SHA3_256_TYPE: LazyLock<usize> = LazyLock::new(|| hash_type_slot(HashKind::Sha3_256.constructor_name()));
static SHA3_384_TYPE: LazyLock<usize> = LazyLock::new(|| hash_type_slot(HashKind::Sha3_384.constructor_name()));
static SHA3_512_TYPE: LazyLock<usize> = LazyLock::new(|| hash_type_slot(HashKind::Sha3_512.constructor_name()));
static SHAKE_128_TYPE: LazyLock<usize> = LazyLock::new(|| hash_type_slot(HashKind::Shake128.constructor_name()));
static SHAKE_256_TYPE: LazyLock<usize> = LazyLock::new(|| hash_type_slot(HashKind::Shake256.constructor_name()));
static BLAKE2B_TYPE: LazyLock<usize> = LazyLock::new(|| hash_type_slot(HashKind::Blake2b.constructor_name()));
static BLAKE2S_TYPE: LazyLock<usize> = LazyLock::new(|| hash_type_slot(HashKind::Blake2s.constructor_name()));
...
fn optional_bytes_arg(args: &[*mut PyObject], index: usize, default: &[u8], name: &str) -> Result<Vec<u8>, *mut PyObject> {
...
fn optional_usize_arg(args: &[*mut PyObject], index: usize, default: usize, name: &str) -> Result<usize, *mut PyObject> {
    match args.get(index).copied() {
        None => Ok(default),
        ...
        Some(value) => {
            ...
            usize::try_from(raw).map_err(|_| raise(ExceptionKind::OverflowError, &format!("{name} is too large")))
        }
    }
}

fn optional_bool_arg(args: &[*mut PyObject], index: usize, default: bool) -> Result<bool, *mut PyObject> {
    match args.get(index).copied() {
        None => Ok(default),
        Some(value) if is_none(value) => Ok(default),
        Some(value) => {
...
fn unsupported_tree_param(args: &[*mut PyObject], index: usize, default: i64, name: &str) -> Result<(), *mut PyObject> {
...
fn blake2_new(kind: HashKind, args: &[*mut PyObject]) -> *mut PyObject {
    if args.len() > 1 && args.len() != BLAKE2_ARG_SLOTS {
        return raise(
            ExceptionKind::TypeError,
            &format!("{}() takes at most 1 positional argument ({} given)", kind.constructor_name(), args.len()),
        );
    }
    ...
    if params.salt.len() > max_person {
        return raise(ExceptionKind::ValueError, &format!("maximum salt length is {max_person} bytes"));
    }
    if params.person.len() > max_person {
        return raise(ExceptionKind::ValueError, &format!("maximum person length is {max_person} bytes"));
    }
    ...
    ($entry:ident, $name:literal, $body:expr) => {
        unsafe extern "C" fn $entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
            ...
            let Some(this) = (unsafe { receiver(raw[0]) }) else {
                return raise(ExceptionKind::TypeError, concat!($name, "() receiver must be a hash object"));
            };
...
fn current_digest(this: &PyHash) -> Vec<u8> {
    match this.kind {
        HashKind::Blake2b => blake2b_digest(&this.data, this.blake2.as_ref().expect("blake2b params")),
        HashKind::Blake2s => blake2s_digest(&this.data, this.blake2.as_ref().expect("blake2s params")),
        HashKind::Shake128 | HashKind::Shake256 => Vec::new(),
        _ => fixed_digest(this.kind, &this.data),
    }
}

fn shake_length_arg(kind: HashKind, args: &[*mut PyObject], method_name: &str) -> Result<usize, *mut PyObject> {
    if args.len() != 1 {
        return Err(raise(
            ExceptionKind::TypeError,
...
fn constructor_entry(kind: HashKind) -> unsafe extern "C" fn(*mut *mut PyObject, usize) -> *mut PyObject {
    match kind {

...
pub(super) fn make_module() -> Result<*mut PyObject, String> {
	...
	attrs
		.push((intern("_tee_dataobject"), (*TEE_DATAOBJECT_TYPE as *mut PyType).cast::<PyObject>()));
...
fn make_chain_callable() -> Result<*mut PyObject, String> {
	...
	// attr dict.
	let status =
		unsafe { crate::types::function::function_setattro(chain_object, attr_name, from_iterable) };
	if status != 0 {
...
static ZIP_LONGEST_TYPE: LazyLock<usize> = LazyLock::new(|| {
	iterator_type("zip_longest", size_of::<PyItertoolsZipLongest>(), zip_longest_next)
});

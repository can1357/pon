...
use crate::abi::{self, pon_const_bool, pon_const_int, pon_const_str, pon_make_function};
use crate::intern::intern;
use crate::object::{PyObject, PyObjectHeader, PySequenceMethods, PyType};
use crate::types::bytearray_ as bytearray_type;
use crate::types::bytes_ as bytes_type;
...
static MMAP_TYPE: LazyLock<usize> = LazyLock::new(|| {
    let mut ty = PyType::new(abi::runtime_type_type().cast_const(), "mmap.mmap", std::mem::size_of::<PyMmap>());
...
pub(super) fn make_module() -> Result<*mut PyObject, String> {
    ...
    attrs.push((intern("mmap"), unsafe { pon_make_function(mmap_constructor as *const u8, VARIADIC_ARITY, intern("mmap")) }));
...
unsafe extern "C" fn mmap_constructor(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let mut flags = args.get(2).map_or(libc::MAP_SHARED, |&object| int_arg(object, "flags").unwrap_or(libc::MAP_SHARED as i64) as libc::c_int);
    let mut prot = args
        .get(3)
        .map_or((libc::PROT_READ | libc::PROT_WRITE) as i64, |&object| int_arg(object, "prot").unwrap_or((libc::PROT_READ | libc::PROT_WRITE) as i64))
        as libc::c_int;
...
unsafe extern "C" fn mmap_len_slot(object: *mut PyObject) -> isize {
    let mmap = unsafe { &*object.cast::<PyMmap>() };
    if mmap.closed { -1 } else { isize::try_from(mmap.len).unwrap_or(isize::MAX) }
}
...
unsafe extern "C" fn mmap_ass_item_slot(object: *mut PyObject, index: isize, value: *mut PyObject) -> libc::c_int {
...
unsafe extern "C" fn mmap_readline_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let rest = unsafe { std::slice::from_raw_parts(mmap.data.add(mmap.pos), mmap.len.saturating_sub(mmap.pos)) };
    let n = rest.iter().position(|&byte| byte == b'\n').map_or(rest.len(), |pos| pos + 1);
    let bytes = &rest[..n];
    mmap.pos += n;
    unsafe { abi::str_::pon_const_bytes(bytes.as_ptr(), bytes.len()) }
}
...
fn find_common(argv: *mut *mut PyObject, argc: usize, reverse: bool) -> *mut PyObject {
    let (mmap, args) = match unsafe { receiver_mut(argv, argc, if reverse { "rfind" } else { "find" }, 3) } {
        Ok(value) => value,
        Err(error) => return error,
    };
    if args.is_empty() {
    ...
    let range = if start <= end { &haystack[start..end] } else { &[] };
    let found = if needle.is_empty() {
        Some(if reverse { end } else { start })
    } else if reverse {
        range.windows(needle.len()).rposition(|window| window == needle).map(|pos| start + pos)
    } else {
        range.windows(needle.len()).position(|window| window == needle).map(|pos| start + pos)
    };
    unsafe { pon_const_int(found.map_or(-1, |pos| pos as i64)) }
}

unsafe fn receiver_mut<'a>(argv: *mut *mut PyObject, argc: usize, function: &str, max_extra: usize) -> Result<(&'a mut PyMmap, &'a [*mut PyObject]), *mut PyObject> {
...
fn byte_value(object: *mut PyObject) -> Result<u8, *mut PyObject> {
    ...
    u8::try_from(value).map_err(|_| raise_value_error("mmap assignment must be single-character bytes or an integer in range(256)"))
}

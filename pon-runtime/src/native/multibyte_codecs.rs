...
//! exposes `getcodec(name)`, returning a codec object with `encode` and `decode`
//! methods that follow the `(payload, consumed)` multibyte-codec contract used
//! by `Lib/encodings/*.py`.
//!
//! The pure-Python `_multibytecodec` fallback in the vendored Lib layer supplies
//! incremental and stream base classes.  This file deliberately does not fake
//! unsupported iconv labels: the codec object can be imported so stdlib codec
//! modules bind successfully, while the first actual conversion reports the
//! missing host label loudly.
...
unsafe extern "C" {
    ...
    fn iconv(cd: IconvT, inbuf: *mut *mut c_char, inbytesleft: *mut usize, outbuf: *mut *mut c_char, outbytesleft: *mut usize) -> usize;
...
fn make_family_module(name: &'static str, specs: &'static [CodecSpec]) -> Result<*mut PyObject, String> {
...
fn family_getcodec_function(module_name: &'static str, specs: &'static [CodecSpec]) -> Result<*mut PyObject, String> {
    ...
    let function = unsafe { abi::pon_make_function(entry as *const u8, VARIADIC_ARITY, intern("getcodec")) };
    if function.is_null() {
...
const TW_CODECS: &[CodecSpec] = &[
    CodecSpec { py_name: "big5", iconv_name: "BIG5" },
    CodecSpec { py_name: "cp950", iconv_name: "CP950" },
];
...
unsafe extern "C" fn getcodec_iso2022_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
fn getcodec_entry(argv: *mut *mut PyObject, argc: usize, specs: &'static [CodecSpec], module_name: &str) -> *mut PyObject {
    let args = match unsafe { arg_slice(argv, argc) } {
        Some(args) => args,
        None => return raise_type_error(&format!("{module_name}.getcodec() missing codec name")),
    };
    if args.len() != 1 {
        return raise_type_error(&format!("{module_name}.getcodec() takes exactly one argument ({} given)", args.len()));
    }
    ...
    let Some(spec) = specs.iter().copied().find(|spec| spec.py_name == normalized) else {
...
fn iconv_convert(to_code: &str, from_code: &str, input: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let from = CString::new(from_code).map_err(|_| format!("invalid iconv source label {from_code:?}"))?;
    let cd = unsafe { iconv_open(to.as_ptr(), from.as_ptr()) };
    if iconv_failed(cd) {
        return Err(format!("iconv codec {from_code} -> {to_code} is not available"));
    }
    let result = convert_with_open_iconv(cd, input).map_err(|error| format!("{from_code} -> {to_code} conversion failed: {error}"));
    let close_rc = unsafe { iconv_close(cd) };
    if close_rc != 0 && result.is_ok() {
        return Err(format!("iconv close failed for {from_code} -> {to_code}: {}", io::Error::last_os_error()));
    }
    result
}
...
unsafe fn codec_receiver_and_args<'a>(
    ...
    match codec_from_object(receiver) {
        ...
        None => Err(raise_type_error(&format!("descriptor '{method}' for 'MultibyteCodec' objects doesn't apply"))),
    }
}

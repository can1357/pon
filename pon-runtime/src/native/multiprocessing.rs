...
//! real POSIX process/fd primitives (`os.fork`, `os.waitpid`, `_posixsubprocess`),
//! so this module intentionally implements that semaphore surface and nothing
//! from Windows-only connection/socket support.
...
use crate::abi::{self, pon_const_bool, pon_const_int, pon_const_str, pon_is_true, pon_make_function, pon_none};
use crate::intern::intern;
use crate::object::{PyObject, PyObjectHeader, PyType};
use crate::thread_state::pon_err_set;
use crate::types::exc::ExceptionKind;
use crate::types::{dict, float, int, method, type_};
...
fn module_function(name: &str, entry: BuiltinFn) -> Result<*mut PyObject, String> {
    ...
    let function = unsafe { pon_make_function(entry as *const u8, crate::native::builtins_mod::VARIADIC_ARITY, intern(name)) };
    if function.is_null() {
...
unsafe extern "C" fn semlock_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    ...
    if positional.len() != 5 {
        return raise_type_error(&format!("SemLock() takes exactly 5 arguments ({} given)", positional.len()));
    }
    ...
    let handle = unsafe { libc::sem_open(c_name.as_ptr(), libc::O_CREAT | libc::O_EXCL, 0o600, value as libc::c_uint) };
...
unsafe extern "C" fn semlock_release_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if argc != 1 {
        return raise_type_error(&format!("release() takes no arguments ({} given)", argc.saturating_sub(1)));
    }
    match lock.release() {
        Ok(()) => unsafe { pon_none() },
...
unsafe extern "C" fn semlock_enter_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    if argc != 1 {
        return raise_type_error(&format!("__enter__() takes no arguments ({} given)", argc.saturating_sub(1)));
    }
...
unsafe extern "C" fn semlock_exit_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if argc > 4 {
        return raise_type_error(&format!("__exit__() takes at most 3 arguments ({} given)", argc - 1));
    }
...
unsafe extern "C" fn semlock_count_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if argc != 1 {
        return raise_type_error(&format!("_count() takes no arguments ({} given)", argc.saturating_sub(1)));
    }
    py_int(i64::from(lock.state.lock().unwrap_or_else(|poison| poison.into_inner()).count))
}
...
unsafe extern "C" fn semlock_is_mine_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if argc != 1 {
        return raise_type_error(&format!("_is_mine() takes no arguments ({} given)", argc.saturating_sub(1)));
    }
    py_bool(lock.is_mine())
}

unsafe extern "C" fn semlock_get_value_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let Some(lock) = receiver(argv, argc, "_get_value") else {
        return ptr::null_mut();
    };
    if argc != 1 {
        return raise_type_error(&format!("_get_value() takes no arguments ({} given)", argc.saturating_sub(1)));
    }
    #[cfg(target_os = "macos")]
    {
        let _ = lock;
        raise(ExceptionKind::NotImplementedError, "sem_getvalue() is not implemented on this platform")
    }
    #[cfg(not(target_os = "macos"))]
    {
        match lock.value() {
...
unsafe extern "C" fn semlock_is_zero_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if argc != 1 {
        return raise_type_error(&format!("_is_zero() takes no arguments ({} given)", argc.saturating_sub(1)));
    }
    match lock.is_zero() {
...
unsafe extern "C" fn semlock_after_fork_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let Some(lock) = receiver(argv, argc, "_after_fork") else {
        return ptr::null_mut();
    };
    if argc != 1 {
        return raise_type_error(&format!("_after_fork() takes no arguments ({} given)", argc.saturating_sub(1)));
    }
    lock.state.lock().unwrap_or_else(|poison| poison.into_inner()).count = 0;
    unsafe { pon_none() }
}
...
unsafe extern "C" fn semlock_rebuild_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.len() != 4 {
        return raise_type_error(&format!("_rebuild() takes exactly 4 arguments ({} given)", args.len()));
    }
...
unsafe extern "C" fn sem_unlink_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    let args = unsafe { argv_slice(argv, argc) };
    if args.len() != 1 {
        return raise_type_error(&format!("sem_unlink() takes exactly one argument ({} given)", args.len()));
    }
...
fn alloc_semlock(handle: SemHandle, kind: i32, maxvalue: i32, name: Option<String>) -> *mut PyObject {
...
fn parse_acquire_args(argv: *mut *mut PyObject, argc: usize) -> Result<AcquireMode, ()> {
    ...
    if argc > 3 {
        let _ = raise_type_error(&format!("acquire() takes at most 2 arguments ({} given)", argc - 1));
        return Err(());
    }

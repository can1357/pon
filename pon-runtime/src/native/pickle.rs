...
fn alloc_picklebuffer(base: *mut PyObject, data: *mut u8, len: usize, readonly: bool) -> *mut PyObject {
...
pub(crate) fn memoryview_over_picklebuffer(object: *mut PyObject) -> Option<*mut PyObject> {
    ...
    if buffer.base.is_null() {
        ...
        return Some(unsafe { abi::exc::pon_raise_value_error(RELEASED_ERROR.as_ptr(), RELEASED_ERROR.len()) });
    }
...
unsafe extern "C" fn picklebuffer_new(_cls: *mut PyType, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    ...
    if positional.len() != 1 {
        let message = format!("PickleBuffer() takes exactly 1 positional argument ({} given)", positional.len());
    ...
    if bytearray_::is_bytearray_type(ty) {
        ...
        return alloc_picklebuffer(source, bytearray.bytes.as_mut_ptr(), bytearray.bytes.len(), false);
    }
...
unsafe extern "C" fn picklebuffer_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {

...
unsafe extern "C" fn shm_open_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
	...
	let path =
		match path_arg(crate::tag::untag_arg(args[0]), "shm_open() argument 1 must be str, not") {
			Ok(path) => path,
			Err(error) => return error,
		};
	let flags =
		match int_arg(crate::tag::untag_arg(args[1]), "shm_open() argument 2 must be int, not") {
			Ok(value) => value,
			Err(error) => return error,
		};
	let mode = if argc == 3 {

...
pub(super) fn make_module() -> Result<*mut PyObject, String> {
	...
	install_module(name, vec![(intern("__name__"), name_obj), (fork_exec_name, fork_exec)])
}
...
fn spawn_child(spec: &ForkExecSpec) -> Result<libc::pid_t, SpawnError> {
	let mut actions = FileActions::new()
		.map_err(|errno| SpawnError::Setup { errno, context: "posix_spawn_file_actions_init" })?;
	let mut attr = SpawnAttr::new()
		.map_err(|errno| SpawnError::Setup { errno, context: "posix_spawnattr_init" })?;

	if let Some(cwd) = &spec.cwd {
		if let Some(errno) = cwd_precheck_errno(cwd) {
			return Err(SpawnError::Child { errno, message: "noexec:chdir" });
		}
	...
	add_close(&mut actions, spec.errpipe_write)
		.map_err(|errno| SpawnError::Setup { errno, context: "posix_spawn_file_actions_addclose" })?;
...
fn install_default_signal_set(attr: &mut SpawnAttr) -> Result<(), SpawnError> {
	...
	if unsafe { libc::sigemptyset(sigset.as_mut_ptr()) } != 0 {
		return Err(SpawnError::Setup { errno: last_errno(), context: "sigemptyset" });
	}
	// SAFETY: libc reported successful initialization.
	...
	for signal in restore_signal_numbers() {
		...
		if unsafe { libc::sigaddset(&mut sigset, signal) } != 0 {
			return Err(SpawnError::Setup { errno: last_errno(), context: "sigaddset" });
		}
	}
	add_spawn_call(
...
fn configure_close_fds(
	...
	*flags |= libc::POSIX_SPAWN_CLOEXEC_DEFAULT as libc::c_int;
	for (fd, redirected) in
		[(0, spec.p2cread != -1), (1, spec.c2pwrite != -1), (2, spec.errwrite != -1)]
	{
		if !redirected {
...
fn configure_close_fds(
	...
	if spec.close_fds && spec.fds_to_keep.iter().any(|fd| *fd != spec.errpipe_write) {
		return Err(SpawnError::Setup { errno: libc::ENOSYS, context: "close_fds with pass_fds" });
	}
	Ok(())
}
...
fn add_inherit(actions: &mut FileActions, fd: libc::c_int) -> Result<(), SpawnError> {
	...
	if rc == 0 {
		...
		Err(SpawnError::Setup { errno: rc, context: "posix_spawn_file_actions_addinherit_np" })
	}
}

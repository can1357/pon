...
pub(super) fn make_module() -> Result<*mut PyObject, String> {
	...
	let attrs =
		vec![(intern("__name__"), name_object), (intern("Random"), random_type().cast::<PyObject>())];
...
unsafe fn method_args(
	...
	if argv.is_null() || argc == 0 {
		return Err(raise(ExceptionKind::TypeError, &format!("Random.{name}() missing receiver")));
	}

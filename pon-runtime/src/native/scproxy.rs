...
use crate::abi::{self, pon_const_str, pon_make_function};
use crate::intern::intern;
use crate::object::PyObject;
use crate::types::bool_;
...
unsafe extern "C" fn scproxy_get_proxy_settings(_argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    if argc != 0 {
...
fn build_proxy_settings(proxy_dict: CFDictionaryRef) -> *mut PyObject {
    let exclude_simple = dictionary_number(proxy_dict, KEY_EXCLUDE_SIMPLE).is_some_and(|value| value != 0);
...
fn dictionary_number(dictionary: CFDictionaryRef, key: &str) -> Option<i32> {
    ...
    if value.is_null() { None } else { cf_number_to_i32(value) }
}
...
fn cf_string(text: &str) -> Option<OwnedCf> {
    ...
    let string = unsafe { CFStringCreateWithCString(ptr::null(), text.as_ptr(), K_CFSTRING_ENCODING_UTF8) };
...
fn cf_number_to_i32(number: CFTypeRef) -> Option<i32> {
    ...
    let ok = unsafe { CFNumberGetValue(number, K_CFNUMBER_SINT32_TYPE, (&mut value as *mut i32).cast()) };
...
fn cf_string_to_string(string: CFStringRef) -> Option<String> {
    ...
    let ok = unsafe { CFStringGetCString(string, buffer.as_mut_ptr(), buffer_size, K_CFSTRING_ENCODING_UTF8) };
    if ok == 0 {
        return None;
    }
    // SAFETY: `CFStringGetCString` wrote a NUL-terminated C string on success.
    unsafe { CStr::from_ptr(buffer.as_ptr()) }.to_str().ok().map(str::to_owned)
}

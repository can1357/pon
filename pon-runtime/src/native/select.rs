...
unsafe extern "C" fn poll_register_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
	let args = match unsafe { arg_slice(argv, argc) } {
		Some(args) => args,
		None => {
			return crate::abi::return_null_with_error("poll.register received a null argv pointer");
		},
	};
...
unsafe extern "C" fn poll_unregister_method(
	...
	let args = match unsafe { arg_slice(argv, argc) } {
		Some(args) => args,
		None => {
			return crate::abi::return_null_with_error("poll.unregister received a null argv pointer");
		},
	};
	if args.len() != 2 {
...
unsafe extern "C" fn select_select(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
	let args = match unsafe { arg_slice(argv, argc) } {
		Some(args) => args,
		None => {
			return crate::abi::return_null_with_error("select.select received a null argv pointer");
		},
	};
	...
	let rc = unsafe {
		libc::select(max_fd + 1, &mut read_set, &mut write_set, &mut except_set, timeout_ptr)
	};
	if rc < 0 {
		return raise_errno(last_errno());
	}
...
fn timeout_ms_arg(object: *mut PyObject) -> Result<libc::c_int, *mut PyObject> {
	...
	let value =
		number_to_f64(object).ok_or_else(|| raise_type_error("timeout must be a number or None"))?;
	if !value.is_finite() {
...
fn timeout_timeval(object: *mut PyObject) -> Result<Option<libc::timeval>, *mut PyObject> {
	...
	Ok(Some(libc::timeval { tv_sec: sec as libc::time_t, tv_usec: usec as libc::suseconds_t }))
}
...
fn timeout_timespec(object: *mut PyObject) -> Result<Option<libc::timespec>, *mut PyObject> {
	...
	Ok(Some(libc::timespec { tv_sec: sec as libc::time_t, tv_nsec: nsec as libc::c_long }))
}

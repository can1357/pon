...
static ITERATOR_TYPE: LazyLock<usize> = LazyLock::new(|| {
	let mut ty =
		Box::new(PyType::new(ptr::null(), "callable_iterator", std::mem::size_of::<SreIterator>()));
...
unsafe extern "C" fn sre_compile(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
	...
	let pattern_text = match unsafe { unicode_text(pattern_obj) } {
		...
		None
			if !pattern_obj.is_null() && bytes_::is_bytes_type(unsafe { (*pattern_obj).ob_type }) =>
		{
			// SAFETY: The type check above guarantees a live `PyBytes`.
...
unsafe fn pattern_method_prelude<'a>(
	...
	let Some(subject) = Subject::from_object(args[subject_index]) else {
		let got =
			unsafe { crate::types::dict::type_name(untag(args[subject_index])) }.unwrap_or("object");
...
unsafe fn pattern_run_match(
	...
	let (Ok(pos), Ok(endpos)) =
		(optional_bound(args, 2, 0, units, name), optional_bound(args, 3, units, units, name))
	else {
		return ptr::null_mut();
	};
	if pos > endpos {
...
unsafe extern "C" fn pattern_findall_method(
	...
	for matched in matches {
		...
		for value in columns {
			...
			objects.push(
				value.map_or_else(|| subject.make_object(&[]), |value| matched_value_object(&value)),
			);
		}
		if objects.len() == 1 {
...
unsafe extern "C" fn pattern_split_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
	let Some((pattern, subject, args)) = (unsafe { pattern_method_prelude(argv, argc, "split", 1) })
	else {
		return ptr::null_mut();
	};
...
mod tests {
	...
	const A_PLUS_CODE: [u32; 13] =
		[14, 4, 0, 1, 4_294_967_295, 24, 6, 1, 4_294_967_295, 16, 97, 1, 1];

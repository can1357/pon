...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Truncated { pc, needed } => write!(f, "truncated SRE code at {pc}, need {needed} words"),
            ...
            Self::UnsupportedOpcode { pc, opcode } => write!(f, "unsupported SRE opcode {opcode} at {pc}"),
...
impl Pattern {
    ...
    pub fn findall_bytes(&self, subject: &[u8]) -> Result<Vec<Vec<Option<MatchedValue>>>, Error> {
        Ok(self.finditer_bytes(subject)?.iter().map(Match::findall_groups).collect())
    }
    ...
    fn build_match(&self, subject: SubjectData, start: usize, state: MatchState) -> Match {
        ...
        let lastindex = state.lastindex.filter(|index| spans.get(*index).is_some_and(Option::is_some));
...
impl Match {
    ...
    pub fn group_name(&self, name: &str) -> Option<MatchedValue> {
        self.groupindex.get(name).and_then(|index| self.group(*index))
    }

    pub fn groups(&self) -> Vec<Option<MatchedValue>> {
        (1..self.spans.len()).map(|index| self.group(index)).collect()
    }
...
impl Program {
    fn parse(code: &[u32]) -> Result<Self, Error> {
        ...
        if code[0] == INFO {
            ...
            let next = 1usize.checked_add(skip).ok_or(Error::InvalidSkip { pc: 1, skip: code[1] })?;
            if next > code.len() {
    ...
    while pc < end {
        ...
        match op {
            ...
            ASSERT | ASSERT_NOT => {
                ...
                nodes.push(Node::Assert { positive: op == ASSERT, width: as_usize(code[pc + 2]), body });
                pc = next;
            }
            BRANCH => {
...
fn parse_branch(code: &[u32], pc: usize) -> Result<(Vec<Vec<Node>>, usize), Error> {
    ...
    loop {
        ...
        let next = cursor.checked_add(as_usize(code[cursor])).ok_or(Error::InvalidSkip { pc: cursor, skip: code[cursor] })?;
        if next > code.len() || next < cursor + 1 {
            return Err(Error::InvalidSkip { pc: cursor, skip: code[cursor] });
        }
        let branch_end = if next >= 2 && code[next - 2] == JUMP { next - 2 } else { next };
...
fn execute(nodes: &[Node], subject: &SubjectData, start: MatchState, collect_all: bool) -> Result<Vec<MatchState>, Error> {
    let mut out = Vec::new();
    ...
    while let Some(mut thread) = stack.pop() {
        ...
        match node {
            ...
            Node::NotLiteral { value, case } => {
                if thread.state.pos < subject.len() && !matches_literal(subject.unit(thread.state.pos), *value, *case) {
                    let mut state = thread.state;
                    state.pos += 1;
                    stack.push(Thread { segments: rest, state });
                }
            ...
            Node::GroupRefExists { group, yes, no } => {
                let chosen = if group_is_matched(&thread.state, *group) { yes } else { no };
...
fn repeat_max_to_usize(value: u32) -> usize {
    if value == MAXREPEAT { usize::MAX } else { as_usize(value) }
}

fn max_repeat(value: u32) -> Option<usize> {
    if value == MAXREPEAT { None } else { Some(repeat_max_to_usize(value)) }
}
...
fn fold_unit(unit: u32, case: CaseMode) -> u32 {
    match case {
        ...
        CaseMode::Ascii | CaseMode::Locale => {
            if (b'A' as u32..=b'Z' as u32).contains(&unit) { unit + 32 } else { unit }
        }
...
fn match_group_ref(subject: &SubjectData, mut state: MatchState, group: usize, case: CaseMode) -> Option<MatchState> {
...
fn category_matches(category: u32, unit: u32) -> bool {
    let yes = match category {
        ...
        CATEGORY_UNI_NOT_WORD => !char::from_u32(unit).is_some_and(|ch| ch == '_' || ch.is_alphanumeric()),
...
fn bitmap_contains(words: &[u32], unit: u32) -> bool {
    ...
    let bit = unit % 32;
    words.get(index).is_some_and(|word| (word & (1u32 << bit)) != 0)
}

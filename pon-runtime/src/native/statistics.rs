...
use crate::types::{bool_, dict, float, int};
...
pub(super) fn make_module() -> Result<*mut PyObject, String> {
    install_module(
        "_statistics",
        [(intern("__name__"), str_object("_statistics")?), function_attr("_normal_dist_inv_cdf", normal_dist_inv_cdf_entry)?],
    )
}

unsafe extern "C" fn normal_dist_inv_cdf_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if p <= 0.0 || p >= 1.0 {
        return crate::abi::exc::raise_kind_error_text(ExceptionKind::ValueError, "inv_cdf undefined for these parameters");
    }
...
fn horner(x: f64, coeffs: &[f64]) -> f64 {
    coeffs.iter().copied().fold(0.0, |acc, coefficient| acc * x + coefficient)
}
...
unsafe fn coerce_f64(object: *mut PyObject) -> Result<f64, *mut PyObject> {
    ...
    if let Some(value) = unsafe { int::to_bigint(object) } {
        ...
        if converted.is_infinite() {
            return Err(crate::abi::exc::raise_kind_error_text(ExceptionKind::OverflowError, "int too large to convert to float"));
        }
        return Ok(converted);
    }
    if let Some(method) = unsafe { try_get_attr(object, "__float__") } {
        let result = crate::tag::untag_arg(unsafe { crate::abi::pon_call(method, ptr::null_mut(), 0) });
        if result.is_null() {
...
fn function_attr(name: &str, entry: BuiltinFn) -> Result<(u32, *mut PyObject), String> {
    ...
    (!function.is_null()).then_some((intern(name), function)).ok_or_else(|| format!("failed to allocate _statistics.{name}"))
}

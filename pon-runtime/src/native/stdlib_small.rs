...
//!
//! These are intentionally real implementations, not import shells: `_functools`
//! exposes working `reduce`/`cmp_to_key`, `_json` provides the string scanner and
//! string encoders used by `json`, `_locale` delegates to the platform C locale,
//! and `_datetime` re-exports the vendored pure-Python datetime types when the C
//! API capsule is not available.
...
fn int_arg(object: *mut PyObject, what: &str) -> Result<i64, *mut PyObject> {
    ...
    value.to_i64().ok_or_else(|| overflow_error(&format!("{what} is too large")))
}
...
fn import_module_attr(module: &str, attr: &str) -> Result<*mut PyObject, String> {
    ...
    crate::import::module_attr(module_name, intern(attr)).ok_or_else(|| format!("{module} did not define {attr}"))
}
...
fn dict_from_pairs(pairs: Vec<(&str, *mut PyObject)>) -> *mut PyObject {
    ...
    unsafe { abi::map::pon_build_map(if flat.is_empty() { ptr::null_mut() } else { flat.as_mut_ptr() }, flat.len() / 2) }
}
...
unsafe extern "C" fn cmp_key_factory_call(callee: *mut PyObject, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    if !kwargs.is_null() {
...
unsafe extern "C" fn cmp_key_getattro(object: *mut PyObject, name: *mut PyObject) -> *mut PyObject {
    ...
    abi::exc::raise_attribute_error_text(&format!("'functools.KeyWrapper' object has no attribute '{name}'"))
}
...
fn cmp_result_value(object: *mut PyObject) -> Result<i32, *mut PyObject> {
    ...
    if let Some(value) = unsafe { crate::types::float::to_f64(object) } {
        return Ok(if value < 0.0 { -1 } else if value > 0.0 { 1 } else { 0 });
    }
    Err(type_error("comparison function must return a number"))
}

unsafe extern "C" fn cmp_key_richcmp(left: *mut PyObject, right: *mut PyObject, op: c_int) -> *mut PyObject {
    let left = unsafe { &*left.cast::<PyCmpKey>() };
    if right.is_null() || unsafe { (*crate::tag::untag_arg(right)).ob_type } != (*CMP_KEY_TYPE as *mut PyType).cast_const() {
...
const FUNCTOOLS_PY_ALIASES: &[&str] = &["partial", "Placeholder", "_PlaceholderType", "_lru_cache_wrapper"];
const FUNCTOOLS_DIR: &[&str] = &["Placeholder", "_PlaceholderType", "_lru_cache_wrapper", "cmp_to_key", "partial", "reduce"];
...
unsafe extern "C" fn functools_getattr(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    if args.len() != 1 {
        return type_error(&format!("_functools.__getattr__ expected 1 argument, got {}", args.len()));
    }
...
unsafe extern "C" fn json_floatstr_call(callee: *mut PyObject, args: *mut PyObject, kwargs: *mut PyObject) -> *mut PyObject {
    if !kwargs.is_null() {
...
unsafe extern "C" fn json_encode_basestring_ascii(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    json_encode_basestring_impl(argv, argc, true)
}

unsafe extern "C" fn json_encode_basestring(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    json_encode_basestring_impl(argv, argc, false)
}

fn json_encode_basestring_impl(argv: *mut *mut PyObject, argc: usize, ensure_ascii: bool) -> *mut PyObject {
    let args = match unsafe { argv_slice(argv, argc) } {
        Some(args) => args,

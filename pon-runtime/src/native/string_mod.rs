...
static FORMATTER_ITER_TYPE: LazyLock<usize> = LazyLock::new(|| {
	iterator_type("formatteriterator", size_of::<PyFormatterIter>(), formatter_iter_next)
});
...
static FIELD_NAME_ITER_TYPE: LazyLock<usize> = LazyLock::new(|| {
	iterator_type("fieldnameiterator", size_of::<PyFieldNameIter>(), field_name_iter_next)
});

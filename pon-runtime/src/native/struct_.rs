...
fn writable_buffer_arg<'a>(object: *mut PyObject) -> Result<&'a mut [u8], String> {
	...
	Err(format!("argument must be read-write bytes-like object, not {}", value_type_name(object)))
}
...
fn parse_format(fmt: &str) -> Result<Format, String> {
	...
	while index < body.len() {
		...
		items.push(Item { code, kind, count, size, pad_before });
	}

	Ok(Format { little, items, size: pos, arg_count })
}

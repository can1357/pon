...
use crate::types::{dict, list::PyList, type_};
...
pub(super) fn make_module() -> Result<*mut PyObject, String> {
    install_module(
        "_suggestions",
        [(intern("__name__"), str_object("_suggestions")?), function_attr("_generate_suggestions", generate_suggestions_entry)?],
    )
}

unsafe extern "C" fn generate_suggestions_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    for &item in items {
        ...
        let max_distance = ((candidate.chars().count() + wrong_name.chars().count() + 3) * MOVE_COST / 6)
...
fn function_attr(name: &str, entry: BuiltinFn) -> Result<(u32, *mut PyObject), String> {
    ...
    (!function.is_null()).then_some((intern(name), function)).ok_or_else(|| format!("failed to allocate _suggestions.{name}"))
}

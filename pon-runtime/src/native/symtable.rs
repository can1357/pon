...
//! compiler's lexical-scope analysis pass; Pon does not expose that subsystem to
//! runtime native modules yet, so `_symtable.symtable()` raises a precise
...
fn function_attr(name: &str, entry: BuiltinFn) -> Result<(u32, *mut PyObject), String> {
    ...
    (!function.is_null()).then_some((intern(name), function)).ok_or_else(|| format!("failed to allocate _symtable.{name}"))
}

fn int_object(value: i64) -> Result<*mut PyObject, String> {
    let object = unsafe { crate::abi::pon_const_int(value) };
    (!object.is_null()).then_some(object).ok_or_else(|| format!("failed to allocate int {value}"))
}

...
use crate::{
	abi::{pon_const_bool, pon_const_int, pon_const_str, pon_make_function, return_null_with_error},
	intern::intern,
...
fn build_version_info() -> Result<*mut PyObject, String> {
	...
	// SAFETY: Runtime allocation helpers return NULL with a diagnostic on failure.
	let releaselevel =
		unsafe { pon_const_str(VERSION_INFO_RELEASELEVEL.as_ptr(), VERSION_INFO_RELEASELEVEL.len()) };
...
fn float_info_attr() -> Result<(u32, *mut PyObject), String> {
	static FLOAT_INFO: std::sync::LazyLock<Result<usize, String>> = std::sync::LazyLock::new(|| {
		build_structseq(&FLOAT_INFO_SPEC, &[
	...
	let text = if object.is_null() {
		...
		if dict.is_null() {
			...
			for (index, (name_id, value)) in unsafe { (&*dict).iter() }.enumerate() {
				let separator = if index == 0 { "" } else { ", " };
				let name =
					crate::intern::resolve(name_id).unwrap_or_else(|| format!("<interned:{name_id}>"));

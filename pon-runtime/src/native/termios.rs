...
fn termios_from_object(object: *mut PyObject) -> Result<libc::termios, *mut PyObject> {
    ...
    if unsafe { libc::cfsetispeed(&mut term, ispeed) } != 0 || unsafe { libc::cfsetospeed(&mut term, ospeed) } != 0 {
...
fn cc_byte(object: *mut PyObject) -> Result<libc::cc_t, *mut PyObject> {
    if let Some(bytes) = bytes_like(object) {
        if bytes.len() != 1 {
            return Err(raise_type_error("tcsetattr: elements of attributes must be characters or integers"));
        }
...
fn raise_termios_error(message: &str) -> *mut PyObject {
    ...
    let exception = unsafe { abi::pon_call(termios_error_type().cast::<PyObject>(), args.as_mut_ptr(), args.len()) };

...
pub(super) fn make_underscore_module() -> Result<*mut PyObject, String> {
    ...
    let attrs = vec![
        ...
        (intern("_remove_dead_weakref"), module_function("_remove_dead_weakref", native_remove_dead_weakref)?),
    ];
...
unsafe fn native_args<'a>(argv: *mut *mut PyObject, argc: usize, name: &str, expect: usize) -> Option<&'a [*mut PyObject]> {
    if argv.is_null() {
    ...
    if args.len() != expect {
        raise_type_error(&format!("{name} expected exactly {expect} argument(s) ({} given)", args.len()));
        return None;
    }
    Some(args)
}
...
unsafe extern "C" fn native_getweakrefcount(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
...
unsafe extern "C" fn native_remove_dead_weakref(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {

...
//! leaked `PonThreadState` mutex so existing `thread_state_lock()` users keep the
//! same guard-based API while GC-facing code can enumerate live thread handles.
...
use crate::thread_state::{PonThreadState, pon_err_set};
...
fn detach_current_thread(registry: &'static ThreadRegistry) -> Result<(), &'static str> {
    ...
    if let Some(index) = threads.iter().position(|record| ptr::addr_eq(record.state, state_ptr)) {
...
/// pointer to the thread's state.  In the default non-free-threaded build this is
...
pub unsafe extern "C" fn pon_thread_start_new(entry: *const u8, arg: *mut PyObject) -> i32 {
    ...
    match catch_unwind(AssertUnwindSafe(|| {
        ...
        #[cfg(feature = "free-threading")]
        {
            ...
            match os_thread::Builder::new().name("pon-runtime".to_string()).spawn(move || {

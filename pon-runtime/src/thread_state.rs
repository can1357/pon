...
use crate::abi::{HandlerInfo, PyFrame};
...
pub struct PonThreadState {
    ...
    /// Python frame stack roots for later traceback, generators, and GC integration.
...
pub fn test_state_lock() -> MutexGuard<'static, ()> {
    TEST_STATE_LOCK.lock().unwrap_or_else(|poison| poison.into_inner())
}

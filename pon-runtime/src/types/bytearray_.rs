...
pub fn insert(array: &mut PyByteArray, index: isize, value: u8) {
    ...
    let mut index = if index < 0 { index.saturating_add(len) } else { index };
...
fn normalize_existing_index(index: isize, len: usize) -> Result<usize, String> {
    let len_isize = isize::try_from(len).map_err(|_| "bytearray is too large for this platform".to_owned())?;
    let index = if index < 0 { index.saturating_add(len_isize) } else { index };
...
mod tests {
    ...
    fn mutators_update_backing_vec() {
        let mut array = PyByteArray { ob_base: PyObjectHeader::new(bytearray_type()), bytes: b"abc".to_vec() };

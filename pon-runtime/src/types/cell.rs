...
//!
//! Cells are deliberately tiny: they hold one boxed `PyObject` pointer and expose
//! get/set/delete helpers with explicit error returns.  The central object layout
//! integration will decide whether these become first-class GC allocations or
//! remain frame-owned storage; the semantics here already match Python closure
...
pub unsafe fn cell_get(cell: *mut PyCell) -> Result<*mut PyObject, String> {
    ...
    unsafe { (*cell).get().ok_or_else(|| "free variable referenced before assignment".to_owned()) }
}

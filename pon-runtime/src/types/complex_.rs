...
/// Implements the built-in `complex()` constructor once the builtin shim has sliced argv.
#[must_use]
...
pub fn hash_complex(real: f64, imag: f64) -> isize {
    ...
    let hash = crate::types::float::hash_f64(real).wrapping_add(HASH_IMAG.wrapping_mul(crate::types::float::hash_f64(imag)));
...
fn parse_complex_text(text: &str) -> Result<(f64, f64), &'static str> {
    ...
    if s.starts_with('(') || s.ends_with(')') {
        let Some(inner) = s.strip_prefix('(').and_then(|value| value.strip_suffix(')')) else {
...
fn parse_float_token(token: &str) -> Result<f64, &'static str> {
    ...
    match lower.as_str() {
        ...
        _ => token.parse::<f64>().map_err(|_| "complex() arg is a malformed string"),
    }
}
...
fn bound_complex_method(
    ...
    let function = unsafe { crate::abi::pon_make_function(entry as *const u8, 1, crate::intern::intern(name)) };
    if function.is_null() {
        return function;
    }
...
unsafe extern "C" fn complex_conjugate_entry(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    if argc != 1 {

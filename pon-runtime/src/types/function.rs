...
pub fn set_function_module_context(
	...
	if let Ok(mut table) = FUNCTION_MODULES.lock() {
		table.insert(function as usize, FunctionModuleRecord {
			name:          module,
...
pub(crate) unsafe fn positional_args_from_star(
	...
	match unsafe { dict::type_name(object) } {
		...
		Some(name) => {
			Err(raise_boxed_type_error(format!("argument after * must be an iterable, not {name}")))
		},
...
pub fn bind_arguments(
	...
	let keywords =
		KeywordArgs { names: keyword_names.as_slice(), values: keyword_values.as_slice() };
...
pub(crate) fn bind_native_keywords_for_name(
	...
	match name {
		...
		"round" => {
			bind_named_positional_keywords(positional, keywords, "round", &["number", "ndigits"], 1, 2)
		},
		"pow" => {
			bind_named_positional_keywords(positional, keywords, "pow", &["base", "exp", "mod"], 2, 3)
		},
		...
		"print" => bind_trailing_marker_keywords(positional, keywords, "print", &[
			"sep", "end", "file", "flush",
		]),
		...
		"md5" => {
			bind_optional_named_keywords(positional, keywords, "md5", &["data", "usedforsecurity"], 1)
		},
		"sha1" => {
			bind_optional_named_keywords(positional, keywords, "sha1", &["data", "usedforsecurity"], 1)
		},
...
fn bind_str_constructor_keywords(
	...
	let mut argv =
		bind_constructor_slots(positional, keywords, "str", &["object", "encoding", "errors"])?;
...
fn bind_bytes_constructor_keywords(
	...
	let argv = bind_constructor_slots(positional, keywords, function_name, &[
...
mod tests {
	...
	fn binds_positional_defaults_and_keyword_only_defaults() {
		...
		let bound =
			bind_arguments(function, &[arg_a], KeywordArgs { names: &[], values: &[] }, None, None)
				.unwrap();
	...
	fn binds_keyword_only_default_without_masking_later_required_parameter() {
		...
		let bound = bind_arguments(
			function,
			&[positional_arg],
			KeywordArgs { names: &[required_kwonly_name], values: &[supplied_required_kwonly] },
			None,
			None,
		)
		.unwrap();

		assert_eq!(bound, vec![positional_arg, defaulted_kwonly_default, supplied_required_kwonly]);
		unregister_function_record(function);
	}
	...
	fn rejects_duplicate_keyword_binding() {
		...
		let err = bind_arguments(
			function,
			&[positional],
			KeywordArgs { names: &[21], values: &[keyword] },
			None,
			None,
		)
		.unwrap_err();
	...
	fn overlong_defaults_override_aligns_to_trailing_positional_slots() {
		...
		unsafe {
			...
			pon_err_clear();
			let function =
				crate::abi::pon_make_function(dummy_entry as *const u8, 3, intern("tail_align_case"));
			assert!(!function.is_null());
			...
			let bound =
				bind_arguments(function, &[], KeywordArgs { names: &[], values: &[] }, None, None)
					.unwrap();
	...
	fn phase_a_short_call_fills_trailing_slots_from_defaults_override() {
		...
		unsafe {
			...
			pon_err_clear();
			let function =
				crate::abi::pon_make_function(dummy_entry as *const u8, 3, intern("phase_a_fill_case"));
			assert!(!function.is_null());
	...
	fn setattro_rejects_non_tuple_defaults_and_non_dict_kwdefaults() {
		...
		unsafe {
			...
			assert!(message.contains("__defaults__ must be set to a tuple object"), "got {message:?}");
			pon_err_clear();
	...
	fn kwdefaults_override_drives_keyword_only_binding_until_cleared() {
		...
		unsafe {
			...
			.unwrap_err();
			assert!(err.contains("missing 1 required keyword-only argument"), "got {err:?}");
			unregister_function_record(function);
		}
	}
	...
	fn defaults_override_accepts_tuple_subclass_instance_and_binds_from_its_storage() {
		...
		unsafe {
			...
			let tuple_base =
				crate::native::builtins_mod::builtin_native_type("tuple").expect("builtin tuple type");
	...
	fn call_bound_function_dispatches_live_entry_and_bumps_hotness() {
		...
		unsafe {
			...
			let result =
				call_bound_function(function, &[], KeywordArgs { names: &[], values: &[] }, None, None)
					.unwrap();

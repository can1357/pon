...
pub unsafe extern "C" fn generator_getattro(
	...
	match name {
		...
		"__next__" => unsafe {
			bound_generator_method(object, "__next__", 1, generator_dunder_next_method as *const u8)
		},
		"__iter__" => unsafe {
			bound_generator_method(object, "__iter__", 1, generator_dunder_iter_method as *const u8)
		},
...
pub unsafe extern "C" fn trace_gen_frame(object: *mut u8, visitor: &mut dyn FnMut(*mut u8)) {
	...
	let (sent, thrown, slot_count) =
		unsafe { ((*frame).sent_value, (*frame).thrown_exc, (*frame).slot_count) };
	if !sent.is_null() {

...
use crate::abi;
use crate::intern;
use crate::object::{PyMappingMethods, PyObject, PyObjectHeader, PySequenceMethods, PyType};
use crate::types::type_::{PyClassDict, PyHeapInstance, new_namespace, unicode_text};
...
unsafe fn backing<'a>(view: &PyInstanceDict) -> Option<&'a mut PyClassDict> {
    ...
    if dict.is_null() { None } else { Some(unsafe { &mut *dict }) }
}
...
unsafe extern "C" fn view_subscript_slot(object: *mut PyObject, key: *mut PyObject) -> *mut PyObject {
...
unsafe extern "C" fn view_ass_subscript_slot(object: *mut PyObject, key: *mut PyObject, value: *mut PyObject) -> c_int {
    let Some(view) = (unsafe { as_view(object) }) else {
        fail("instance __dict__ receiver is invalid");
        return -1;
    };
...
unsafe extern "C" fn view_contains_slot(object: *mut PyObject, key: *mut PyObject) -> c_int {
    ...
    c_int::from(unsafe { backing(view) }.and_then(|dict| dict.get(name)).is_some())
}
...
unsafe extern "C" fn view_iter(object: *mut PyObject) -> *mut PyObject {
    ...
    let keys: Vec<u32> = unsafe { entries_snapshot(view) }.into_iter().map(|(key, _)| key).collect();
    let info = GcTypeInfo {
...
unsafe extern "C" fn view_richcmp_slot(left: *mut PyObject, right: *mut PyObject, op: c_int) -> *mut PyObject {
...
unsafe extern "C" fn view_get_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let default = args.get(1).copied().unwrap_or_else(|| unsafe { abi::pon_none() });
    let Some(name) = (unsafe { key_id(args[0]) }) else {
        return default;
    };
    unsafe { backing(view) }.and_then(|dict| dict.get(name)).unwrap_or(default)
}
...
unsafe extern "C" fn view_values_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {
    ...
    let mut values = unsafe { entries_snapshot(view) }.into_iter().map(|(_, value)| value).collect::<Vec<_>>();
...
unsafe extern "C" fn view_setdefault_method(argv: *mut *mut PyObject, argc: usize) -> *mut PyObject {

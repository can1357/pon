...
//!
//! These are intentionally small CPython-shaped iterator types (`map`, `filter`,
//! `zip`, and `list_reverseiterator`) instead of generator frames.  They keep the
//! builtin surface lazy while using the runtime's normal iterator/call/sequence
//! protocols for every step.
...
static MAP_TYPE: LazyLock<usize> = LazyLock::new(|| iterator_type("map", mem::size_of::<PyMap>()) as usize);
static FILTER_TYPE: LazyLock<usize> = LazyLock::new(|| iterator_type("filter", mem::size_of::<PyFilter>()) as usize);
                ...
                if items.is_empty() {
                    for (later_index, later) in iter.iters.iter().copied().enumerate().skip(index + 1) {
                        match unsafe { next_item(later) } {
                            NextItem::Value(_) => return raise_value_error(&zip_mismatch_message(later_index, "longer")),
                            NextItem::Stop => {}
                            NextItem::Error => return ptr::null_mut(),
                        }
                    }

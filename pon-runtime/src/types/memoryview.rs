...
//! window.  Bytes-backed views are readonly; bytearray-backed views are
//! writable until bytearray reallocation.  The surface covers basic
//! construction, len/index/slice, `tobytes()`, readonly enforcement, and the
//! flat `cast`/`itemsize`/`tolist` trio that re._compiler's `_bytes_to_codes`

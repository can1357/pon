...
pub unsafe extern "C" fn trace_tuple_subclass_instance(object: *mut u8, visitor: &mut dyn FnMut(*mut u8)) {
    if object.is_null() {
        return;
    }
...
const _: () = {
    ...
    assert!(offset_of!(PyTuple, items) - offset_of!(PyTuple, len) == offset_of!(PyTupleStorage, items));
    assert!(core::mem::size_of::<PyTuple>() == offset_of!(PyTuple, len) + core::mem::size_of::<PyTupleStorage>());
    ...
    assert!(
        core::mem::size_of::<PyTupleSubclassInstance>() != core::mem::size_of::<crate::types::type_::PyHeapInstance>()
    );
    assert!(

...
fn normalized_union_arg(object: *mut PyObject) -> Option<*mut PyObject> {
    ...
    if is_type_object(object) || is_generic_alias(object) || is_type_alias(object) || is_typevar(object) {
        Some(object)
    } else {
        None
    }
...
pub fn union_repr(object: *mut PyObject) -> String {
    union_args(object).iter().copied().map(union_arg_text).collect::<Vec<_>>().join(" | ")
}
...
unsafe extern "C" fn union_richcmp(left: *mut PyObject, right: *mut PyObject, op: c_int) -> *mut PyObject {

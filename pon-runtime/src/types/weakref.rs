...
unsafe extern "C" fn weakref_richcmp(
		...
		&& unsafe { object_type_name(right) == Some("ReferenceType") }
	{
		...
		equal = if !left_ref.referent.is_null() && !right_ref.referent.is_null() {
			unsafe {
				crate::types::dict::object_equal(left_ref.referent, right_ref.referent).unwrap_or(false)
			}
		} else {
			left == right
		};
...
static WEAKPROXY_TYPE: LazyLock<usize> = LazyLock::new(|| {
	let mut ty =
		PyType::new(crate::abi::runtime_type_type(), "weakproxy", core::mem::size_of::<PyWeakRef>());
	ty.tp_new = Some(weakref_new);
...
pub unsafe extern "C" fn finalize_heap_instance(object: *mut u8) {
	...
	if !instance.finalized {
		...
		let del =
			unsafe { descr::lookup_in_type((*object).ob_type.cast_mut(), intern::intern("__del__")) };
		if !del.is_null() {
...
mod tests {
	...
	fn heap_instance_collection_runs_del_once_and_clears_weakrefs() {
		...
		unsafe {
			...
			let callback =
				pon_make_function(weakref_callback as *const u8, 1, intern::intern("weakref_callback"));

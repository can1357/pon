...
fn sre_surface_methods_basics() {
	...
	assert_eq!(
		search.groupdict().get("word"),
		Some(&Some(sre::MatchedValue::Str("abc".to_owned())))
	);
	...
	assert_eq!(
		repeat
			...
			.unwrap()
			.iter()
			.map(|m| m.span(0).flatten().unwrap())
			.collect::<Vec<_>>(),
		vec![(1, 3), (4, 6)]
	);
	assert_eq!(repeat.findall_str("baacaa").unwrap(), vec![
		vec![Some(sre::MatchedValue::Str("aa".to_owned()))],
		vec![Some(sre::MatchedValue::Str("aa".to_owned()))]
	]);
	assert_eq!(repeat.split_str("baacaa").unwrap(), vec![
		sre::MatchedValue::Str("b".to_owned()),
		sre::MatchedValue::Str("c".to_owned()),
		sre::MatchedValue::Str(String::new())
	]);
	...
	let bytes_case = root
		...
		.iter()
		.find(|case| {
			case.get("kind").string() == "bytes"
				&& case
					.get("pattern_bytes")
					.array()
					.starts_with(&[Json::Number(97), Json::Number(43)])
		})
		.expect("bytes a+ fixture exists");
	let bytes_pattern = compile_case(bytes_case);
	assert_eq!(
		bytes_pattern
			.search_bytes(b"xxaaab")
			.unwrap()
			.and_then(|m| m.span(0).flatten()),
		Some((2, 5))
	);
	assert_eq!(bytes_pattern.findall_bytes(b"baacaa").unwrap(), vec![
		vec![Some(sre::MatchedValue::Bytes(b"aa".to_vec()))],
		vec![Some(sre::MatchedValue::Bytes(b"aa".to_vec()))]
	]);
	assert_eq!(bytes_pattern.split_bytes(b"baacaa").unwrap(), vec![
		sre::MatchedValue::Bytes(b"b".to_vec()),
		sre::MatchedValue::Bytes(b"c".to_vec()),
		sre::MatchedValue::Bytes(Vec::new())
	]);
...
fn run_case(case: &Json) {
	...
	match case.get("kind").string() {
		"str" => {
			...
			assert_eq!(
				record(pattern.match_str(subject).unwrap().as_ref(), groups),
				expected(expect.get("match")),
				"{name} match"
			);
			assert_eq!(
				record(pattern.search_str(subject).unwrap().as_ref(), groups),
				expected(expect.get("search")),
				"{name} search"
			);
			assert_eq!(
				record(pattern.fullmatch_str(subject).unwrap().as_ref(), groups),
				expected(expect.get("fullmatch")),
				"{name} fullmatch"
			);
			...
			assert_eq!(
				actual
					.iter()
					.map(|m| record(Some(m), groups).unwrap())
					.collect::<Vec<_>>(),
				expected_list(expect.get("finditer")),
				"{name} finditer"
			);
		},
		"bytes" => {
			...
			assert_eq!(
				record(pattern.match_bytes(&subject).unwrap().as_ref(), groups),
				expected(expect.get("match")),
				"{name} match"
			);
			assert_eq!(
				record(pattern.search_bytes(&subject).unwrap().as_ref(), groups),
				expected(expect.get("search")),
				"{name} search"
			);
			assert_eq!(
				record(pattern.fullmatch_bytes(&subject).unwrap().as_ref(), groups),
				expected(expect.get("fullmatch")),
				"{name} fullmatch"
			);
...
fn record(matched: Option<&sre::Match>, groups: usize) -> Option<MatchRecord> {
	...
	Some(MatchRecord {
		...
		groups:    (1..=groups)
			.map(|index| matched.span(index).flatten())
			.collect(),
...
fn expected_list(value: &Json) -> Vec<MatchRecord> {
	value
		.array()
		.iter()
		.map(|item| expected(item).expect("finditer entries are matches"))
		.collect()
}

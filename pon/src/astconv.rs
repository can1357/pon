...
//! - flattened `elif` clauses → nested `If` in `orelse`, where each nested `If`
//!   spans clause start → outer-statement end (the CPython tail shape);
...
use pon_runtime::{
	dynexec::{DynCodeMode, DynCompileRequest},
	native::{AstNode, AstValue, NodeSpan},
};
...
pub(crate) fn parse_dynamic_ast(request: DynCompileRequest<'_>) -> Result<AstNode, String> {
	...
	match (request.mode, parsed.into_syntax()) {
		...
		(DynCodeMode::Exec, Mod::Module(module)) => Ok(AstNode {
			...
			span:   None,
			fields: vec![
				("body", converter.stmt_list(&module.body)?),
				("type_ignores", AstValue::List(Vec::new())),
			],
		}),
...
impl<'a> Converter<'a> {
		...
		match stmt {
			...
			ast::Stmt::Match(s) => {
				...
				Ok(AstNode {
					...
					span,
					fields: vec![
						("subject", self.expr_value(&s.subject)?),
						("cases", AstValue::List(cases)),
					],
				})
			},
			...
			ast::Stmt::Assert(s) => Ok(AstNode {
				...
				fields: vec![
					("test", self.expr_value(&s.test)?),
					("msg", self.opt_expr(s.msg.as_deref())?),
				],
			}),
			ast::Stmt::Import(s) => {
				Ok(AstNode { class: "Import", span, fields: vec![("names", self.aliases(&s.names))] })
			},
			...
			ast::Stmt::Expr(s) => {
				Ok(AstNode { class: "Expr", span, fields: vec![("value", self.expr_value(&s.value)?)] })
			},
			...
			ast::Stmt::IpyEscapeCommand(_) => {
				Err("IPython escape commands have no _ast representation".to_owned())
			},
		}
	}
	...
	fn expr(&self, expr: &ast::Expr) -> Result<AstNode, String> {
		...
		match expr {
			...
			ast::Expr::Named(e) => Ok(AstNode {
				...
				span,
				fields: vec![
					("target", self.expr_value(&e.target)?),
			...
			ast::Expr::Set(e) => {
				Ok(AstNode { class: "Set", span, fields: vec![("elts", self.expr_list(&e.elts)?)] })
			},
			...
			ast::Expr::BytesLiteral(e) => Ok(AstNode {
				...
				span,
				fields: vec![
					("value", AstValue::Bytes(e.value.bytes().collect())),
					("kind", AstValue::None),
				],
			}),
			ast::Expr::NumberLiteral(e) => Ok(constant(span, match &e.value {
			...
			ast::Expr::Name(e) => Ok(AstNode {
				...
				span,
				fields: vec![
					("id", AstValue::Str(e.id.as_str().to_owned())),
					("ctx", context_node(e.ctx)),
				],
			}),
	...
	fn parameters(&self, parameters: Option<&ast::Parameters>) -> Result<AstValue, String> {
		...
		for (list, source) in
			[(&mut posonlyargs, &parameters.posonlyargs), (&mut args, &parameters.args)]
		{
		...
		for type_param in &type_params.type_params {
			let (class, name, bound, default, range) = match type_param {
				ast::TypeParam::TypeVar(p) => {
					("TypeVar", &p.name, p.bound.as_deref(), p.default.as_deref(), p.range)
				},
				ast::TypeParam::ParamSpec(p) => {
					("ParamSpec", &p.name, None, p.default.as_deref(), p.range)
				},
				ast::TypeParam::TypeVarTuple(p) => {
					("TypeVarTuple", &p.name, None, p.default.as_deref(), p.range)
				},
			};
	...
	fn pattern(&self, pattern: &ast::Pattern) -> Result<AstNode, String> {
		...
		match pattern {
			...
			ast::Pattern::MatchSingleton(p) => Ok(AstNode {
				...
				span,
				fields: vec![("value", match p.value {
...
fn constant(span: Option<NodeSpan>, value: AstValue) -> AstNode {
	AstNode { class: "Constant", span, fields: vec![("value", value), ("kind", AstValue::None)] }
}

...
pub fn download_artifact(
	...
	let parent = artifact_path.parent().ok_or_else(|| {
		Error::Index(format!("could not determine cache directory for artifact `{filename}`"))
	})?;
...
fn cached_artifact_path(cache_dir: &Path, url: &str, filename: &str) -> Result<PathBuf> {
	if Path::new(filename)
		...
		!= Some(filename)
	{
		return Err(Error::Index(format!("artifact filename `{filename}` is not a plain file name")));
	}
	...
	let result = (|| {
		...
		loop {
			let len = reader.read(&mut buffer).map_err(|error| {
				Error::Index(format!("failed to read artifact response `{url}`: {error}"))
			})?;
			if len == 0 {
				break;
			}
...
fn check_digest(actual: &str, policy: &HashPolicy<'_>) -> Result<DigestCheck> {
		...
		.any(|hash| hash.eq_ignore_ascii_case(actual))
	{
		...
		Ok(DigestCheck::Mismatch { expected: expected.display })
	}
}
...
fn file_url_path(url: &str) -> Result<PathBuf> {
	let without_scheme = url
		.strip_prefix("file://")
		.ok_or_else(|| Error::Index(format!("artifact URL `{url}` is not a file URL")))?;
...
fn hash_mismatch_error(filename: &str, expected: &str, actual: &str) -> Error {
	Error::Index(format!("hash mismatch for `{filename}`: expected {expected}, got sha256:{actual}"))
}

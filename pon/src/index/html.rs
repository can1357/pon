...
use super::{DistInfoMetadata, ProjectPage, simple_json::project_file_from_parts};
use crate::{
	error::{Error, Result},
	names,
};

/// Parse a PEP 503 project page into the same `ProjectPage` shape as PEP 691
/// JSON.
pub fn parse_project_html(base_url: &str, body: &str) -> Result<Option<ProjectPage>> {
	...
	while let Some(anchor) = next_anchor(body, offset) {
		...
		if let Some(file) =
			project_file_from_parts(filename, url, hashes, requires_python, yanked, dist_info_metadata)
		{
			files.push(file);
		}
	}

	Ok(Some(ProjectPage { meta_api_version: "1.0".to_owned(), name, files }))
}

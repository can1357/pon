...
use super::{
	DistInfoMetadata, NO_OB_REFCNT_C_ABI_REFUSAL, PackageIndex, ProjectFile, ProjectPage,
	download::{HashPolicy, download_artifact},
	html::parse_project_html,
};
use crate::{
	error::{Error, Result},
	names,
...
impl SimpleJsonIndex {
	...
	fn fetch_project(&self, normalized_name: &str) -> Result<Option<ProjectPage>> {
		...
		match fetch_simple_json(&url, etag.as_deref()) {
			Ok(FetchOutcome::Found { body, etag, max_age, content_type }) => {
			...
			Err(error) if cache_path.is_file() => {
				...
				parse_project_response(&url, &body, None).map_err(|parse_error| {
					Error::Index(format!(
						"failed to fetch `{url}` ({error}) and cached response could not be parsed: \
						 {parse_error}"
					))
				})
			},
...
fn fetch_simple_json(url: &str, etag: Option<&str>) -> Result<FetchOutcome> {
	...
	let response = match request.call() {
		...
		Err(error) => {
			return Err(Error::Index(format!("failed to fetch simple index `{url}`: {error}")));
		},
	};
	...
	let body = response.body_mut().read_to_string().map_err(|error| {
		Error::Index(format!("failed to read simple index response `{url}`: {error}"))
	})?;
...
pub fn parse_project_json(body: &str) -> Result<Option<ProjectPage>> {
	...
	let files = response
		...
		.collect::<Vec<_>>();
	Ok(Some(ProjectPage { meta_api_version: response.meta.api_version, name, files }))
}
...
fn parse_project_response(
	...
	match parse_project_json(body) {
		Ok(page) => Ok(page),
		Err(_error) if !content_type.is_empty() && !content_type.contains("json") => {
			parse_project_html(url, body)
		},
		Err(error) => Err(error),
	}
}
...
fn classify_package_file(filename: &str) -> PackageKind {
	...
	if wheel.abi_tag.split('.').any(is_refcount_cpython_abi) {
		return PackageKind::CAbiRefused { reason: NO_OB_REFCNT_C_ABI_REFUSAL.to_owned() };
	}
	PackageKind::Native
}
...
fn cache_is_fresh(cache_path: &Path, metadata_path: &Path) -> Result<bool> {
	...
	let fetched_at =
		metadata_value(&metadata, "fetched_at").and_then(|value| value.parse::<u64>().ok());

...
impl ResolvedRecord {
	...
	pub fn wheel(
		...
		Self {
			...
			artifact: PackageArtifact::Wheel { path: path.into(), direct_url: None },
		}
	}
	...
	pub fn sdist(
		...
		Self {
			name:     name.into(),
			version:  version.into(),
			artifact: PackageArtifact::Sdist { path: path.into(), direct_url: None },
		}
	}
	...
	pub fn dir(
		...
		Self {
			name:     name.into(),
			version:  version.into(),
			artifact: PackageArtifact::Dir { path: path.into(), editable },
		}
	}
...
pub(crate) fn upsert_installed_package(
	env: &EnvLayout,
	record: InstalledPackageRecord,
) -> Result<()> {
	let mut records = read_installed_packages(env)?;
...
fn read_installed_packages_from_path(path: &Path) -> Result<Vec<InstalledPackageRecord>> {
	...
	for (line_index, line) in content.lines().enumerate() {
		...
		records.push(InstalledPackageRecord {
			...
			record_path: fields
				.get(4)
				.and_then(|field| (!field.is_empty()).then(|| PathBuf::from(field))),
		});
	}
	Ok(records)
}
...
mod tests {
	...
	fn registry_round_trips_record_path_column() {
		...
		write_installed_packages(&layout, &[InstalledPackageRecord {
			name:          "idna".to_owned(),
	...
	fn remove_uses_record_file_set_and_prunes_empty_parents() {
		...
		assert!(
			fs::read_dir(&layout.site_packages)
	...
	fn remove_record_files_rejects_entries_that_escape_pon() {
		...
		fs::create_dir_all(layout.site_packages.join("demo-1.0.dist-info")).expect("dist-info");
		fs::write(layout.site_packages.join("demo-1.0.dist-info/RECORD"), "../../../outside,,\n")
			.expect("record");

		let error =
			remove_record_files(&layout, Path::new("demo-1.0.dist-info/RECORD")).expect_err("escape");
	...
	fn temp_project(label: &str) -> PathBuf {
		let unique = SystemTime::now()
			...
			.expect("clock")
			.as_nanos();
		std::env::temp_dir().join(format!("pon-install-{label}-{unique}"))
	}
}

...
impl LockFile {
    ...
    pub fn from_resolution(
        ...
        Self::with_input_hash(
            resolution.dists.iter().map(LockedPackage::from_resolved_dist).collect(),
            requires_python,
            input_hash,
        )
    }
...
impl LockedPackage {
    ...
    pub fn from_resolved_dist(dist: &ResolvedDist) -> Self {
        let mut package = Self::with_kind(dist.name.clone(), dist.version.to_string(), LockedPackageKind::from(&dist.kind));
        package.marker = dist.marker.clone();
        match &dist.artifact {
            ResolvedArtifact::Wheel(file) => package.wheels.push(LockedWheel::from_project_file(file)),
            ResolvedArtifact::Sdist(file) => package.sdist = Some(LockedSdist::from_project_file(file)),
            ResolvedArtifact::Dir { path, editable } => {
                package.directory = Some(LockedDirectory {
                    path: path.clone(),
                    editable: *editable,
                });
            }
...
impl LockedDirectory {
    ...
    fn from_table(table: &dyn TableLike) -> Result<Self> {
        Ok(Self {
            path: PathBuf::from(required_string(table, "path", "pon.lock directory missing path")?),
            editable: optional_bool(table, "editable", "pon.lock directory editable")?.unwrap_or(false),
        })
    }
}
...
impl LockedVcs {
    ...
    fn from_table(table: &dyn TableLike) -> Result<Self> {
        Ok(Self {
            ...
            requested_revision: optional_string(table, "requested-revision", "pon.lock VCS requested-revision")?,
            commit_id: required_string(table, "commit-id", "pon.lock VCS entry missing commit-id")?,
        })
    }
}
...
fn parse_lock(input: &str) -> Result<LockFile> {
    ...
    let requires_python = required_string(root, "requires-python", "pon.lock missing requires-python")?;
    let input_hash = table_at(doc.as_item(), &["tool", "pon"])
        .ok_or_else(|| Error::Cli("pon.lock missing [tool.pon]".to_owned()))
        .and_then(|tool_pon| required_string(tool_pon, "input-hash", "pon.lock missing [tool.pon].input-hash"))?;
...
fn parse_package(table: &Table) -> Result<LockedPackage> {
    ...
    let sdist = parse_optional_table(table, "sdist", "pon.lock package sdist")?.map(LockedSdist::from_table).transpose()?;
    let directory =
        parse_optional_table(table, "directory", "pon.lock package directory")?.map(LockedDirectory::from_table).transpose()?;
    let vcs = parse_optional_table(table, "vcs", "pon.lock package vcs")?.map(LockedVcs::from_table).transpose()?;
...
mod tests {
    ...
    fn rejects_unsupported_major_version() {
        ...
        let error = lock.parse::<LockFile>().expect_err("major version should be rejected");

        assert!(error.to_string().contains("unsupported pon.lock lock-version `2.0`"));
    }

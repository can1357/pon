...
use crate::{
	...
	error::{Error, Result},
	install::{
		InstallReport, InstalledPackageRecord, ResolvedRecord, upsert_installed_package,
...
pub fn read_registry(env: &EnvLayout) -> Result<NativeRegistry> {
	let content = match fs::read_to_string(&env.native_registry_path) {
		...
		Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
			return Ok(NativeRegistry::default());
		},
...
impl NativeRegistry {
	pub fn upsert(&mut self, record: NativeModuleRecord) {
		...
		self
			.modules
			.sort_by(|left, right| left.import_name.cmp(&right.import_name));
	}
}
...
fn read_native_manifest(path: &Path) -> Result<NativeManifest> {
	...
	Ok(NativeManifest { package_name, version, import_name })
}
...
fn materialize_native_marker(env: &EnvLayout, manifest: &NativeManifest) -> Result<()> {
	...
	fs::write(
		env.site_packages
			.join(format!("{}.py", manifest.import_name)),
		marker,
	)?;
	Ok(())
}
...
mod tests {
	...
	fn rejects_local_fixture_version_mismatch() {
		...
		assert!(
			error
				.to_string()
				.contains("does not match resolved version")
		);
	}

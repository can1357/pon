...
use crate::{
	error::{Error, Result},
	names,
	requirement::{normalized_name_of, parse_requirement_input},
};
...
impl PyProject {
	...
	pub fn empty(path: impl Into<PathBuf>) -> Self {
		Self { doc: DocumentMut::new(), path: path.into() }
	}
	...
	pub fn build_system(&self) -> Option<BuildSystem> {
		self
			.table_like_at(&["build-system"])
			.map(|table| BuildSystem {
				requires:      table
					.get("requires")
	...
	pub fn tool_pon_allow_prerelease(&self) -> bool {
		self
			.bool_at(&["tool", "pon", "allow-prerelease"])
			.unwrap_or(false)
	}
	...
	pub fn add_dependency(&mut self, raw: &str) -> Result<bool> {
		...
		let added = dependencies
			.insert(normalized, raw.trim().to_owned())
			.is_none();
	...
	pub(crate) fn set_dependency_strings<'a>(
		&mut self,
	...
	fn parse_sources(&self) -> Result<BTreeMap<String, PonSource>> {
		...
		for (key, item) in table.iter() {
			...
			sources.insert(source_name, PonSource { path, editable, git, rev });
		}
		Ok(sources)
	}

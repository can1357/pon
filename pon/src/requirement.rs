...
use crate::{
	...
	names,
	pyproject::PyProject,
	wheel::filename::WheelFilename,
};

/// A requirement supplied by a user, manifest, or requirements file before it
/// is resolved into concrete candidates.
...
impl RequirementInput {
	...
	/// Return the bare URL, if this input was supplied without a `name @`
	/// prefix.
...
pub fn parse_requirement_input(raw: &str) -> Result<RequirementInput> {
	...
	if looks_like_path(raw) || is_existing_archive_path(raw) {
		return Ok(RequirementInput::Path { path: PathBuf::from(raw), editable: false });
	}
...
mod tests {
	...
	fn classifies_full_pep508_requirement() {
		let input = parse_requirement_input("pkg[extra1,extra2]>=1; python_version >= '3.12'")
			.expect("requirement");
	...
	fn classifies_pep508_direct_url_requirement() {
		let input = parse_requirement_input("demo @ https://example.com/demo-1.0-py3-none-any.whl")
			.expect("requirement");
	...
	fn classifies_bare_http_url() {
		let input =
			parse_requirement_input("https://example.com/demo-1.0-py3-none-any.whl").expect("url");
	...
	fn classifies_bare_git_url() {
		let input =
			parse_requirement_input("git+https://example.com/org/demo.git@v1#subdirectory=sub")
				.expect("url");
	...
	fn normalizes_directory_name_from_pyproject() {
		...
		let input = RequirementInput::Path { path: PathBuf::from("vendor/pkg"), editable: false };
	...
	fn normalizes_bare_url_archive_name() {
		let input =
			parse_requirement_input("https://example.com/packages/Friendly_Bard-1.2.3.zip#sha256=abc")
				.expect("url");

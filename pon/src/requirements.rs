...
///
/// Recursive `-r` includes are flattened into this structure in encounter
/// order. Requirement entries retain their source file and line so later
/// resolver and CLI phases can produce diagnostics against the original input.
...
fn parse_requirements_file_uncycled(
	path: &Path,
	stack: &mut Vec<PathBuf>,
) -> Result<RequirementsFile> {
...
fn parse_logical_line(
	...
	match option_name_and_value(&first.text, "--requirement") {
		...
		OptionMatch::NoMatch => match option_name_and_value(&first.text, "--constraint") {
			OptionMatch::Flag => {
				let constraint =
					required_option_value(&tokens, 0, "--constraint", file_path, line_number)?;
				reject_extra_tokens(&tokens, 2, file_path, line_number)?;
				file
					.constraints
					.push(resolve_relative_path(base_dir, constraint));
				Ok(())
			},
			OptionMatch::Inline(constraint) => {
				let constraint =
					require_inline_value(constraint, "--constraint", file_path, line_number)?;
				reject_extra_tokens(&tokens, 1, file_path, line_number)?;
				file
					.constraints
					.push(resolve_relative_path(base_dir, constraint));
			...
			OptionMatch::NoMatch => {
				parse_non_include_line(line, &tokens, line_number, file_path, base_dir, stack, file)
			},
		},
	}
}
...
fn parse_non_include_line(
	...
	file
		.entries
		.push(parse_requirement_entry(line, tokens, line_number, file_path, base_dir)?);
	Ok(())
}
...
fn parse_global_options(
	...
	while index < tokens.len() {
		...
		match token.as_str() {
			...
			"-i" | "--index-url" => {
				file.index_url = Some(
					required_option_value(tokens, index, token, file_path, line_number)?.to_owned(),
				);
				index += 2;
			},
			"--extra-index-url" => {
				file.extra_index_urls.push(
					required_option_value(tokens, index, token, file_path, line_number)?.to_owned(),
				);
				index += 2;
			},
			_ => match option_name_and_value(token, "--index-url") {
				...
				OptionMatch::NoMatch | OptionMatch::Flag => {
					match option_name_and_value(token, "--extra-index-url") {
						OptionMatch::Inline(value) => {
							let value =
								require_inline_value(value, "--extra-index-url", file_path, line_number)?;
...
fn parse_requirement_entry(
	...
	let option_index = tokens
		.iter()
		.position(|token| !token.quoted && token.text.starts_with('-'));
	let requirement_raw =
		option_index.map_or_else(|| line.trim(), |index| line[..tokens[index].start].trim_end());
	...
	for (offset, physical) in content.lines().enumerate() {
		...
		if let Some(continued) = trimmed.strip_suffix('\\') {
			...
			if !buffer.trim().is_empty() {
				lines.push(LogicalLine { text: buffer.trim().to_owned(), line: start_line });
			}
			buffer.clear();
		}
	}
...
fn tokenize(line: &str) -> Vec<Token> {
	...
	for (index, character) in line.char_indices() {
		...
		if character == '\'' || character == '"' {
			...
			if let Some(token_start) = start.take() {
				tokens.push(Token { text: std::mem::take(&mut text), start: token_start, quoted });
				quoted = false;
			}
		} else {
			if start.is_none() {
	...
	if let Some(token_start) = start {
		tokens.push(Token { text, start: token_start, quoted });
	}
...
mod tests {
	...
	fn detects_circular_recursive_includes() {
		...
		assert_eq!(error.to_string(), format!("circular -r include: {}", first.display()));
		fs::remove_dir_all(root).ok();
	}

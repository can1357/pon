...
use crate::{
	error::{Error, Result},
	index::{CatalogIndex, PackageIndex, ProjectFile},
	marker::pon_marker_env,
	metadata::CoreMetadata,
	names,
	pyproject::PyProject,
	requirement::{RequirementInput, parse_requirement_input},
	resolve::{
		package::PonPackage,
		source::{ArtifactSet, CandidateSource, IndexSource, PackageKind, PackageRecord},
		versionset::range_from_specifiers,
	},
};
...
impl ConstraintSet {
	...
	pub fn from_requirements_with_env<T, R>(
		requirements: T,
		marker_env: &MarkerEnvironment,
	) -> Result<Self>
	where
	...
	pub fn insert_requirement_with_env(
		&mut self,
		raw: impl AsRef<str>,
		marker_env: &MarkerEnvironment,
	) -> Result<()> {
...
impl<I> ResolveProvider<I> {
	...
	fn configure_provider<'a, S: CandidateSource>(
		&self,
		provider: PonProvider<'a, S>,
	) -> PonProvider<'a, S> {
		provider
...
impl<I: PackageIndex> ResolveProvider<I> {
	pub fn resolve_input(
		...
		version_specifier: impl AsRef<str>,
		...
		let root_names = [resolved.dist_name().to_owned()]
			.into_iter()
			.collect::<BTreeSet<_>>();
...
impl<'a, S: CandidateSource> PonProvider<'a, S> {
		...
		let pinned = root_reqs
			...
			.filter_map(|input| match input {
				ResolvedInput::Pinned { candidate, .. } => {
					Some((candidate.name.clone(), candidate.clone()))
				},
	...
	fn dependency_edges(
		...
		package: &PonPackage,
		version: &Version,
	) -> Result<DependencyConstraints<PonPackage, Ranges<Version>>> {
		...
		match package {
			...
			PonPackage::Extra(name, extra) => {
				...
				if !self.no_deps {
					let extra = ExtraName::from_str(extra)
						.map_err(|_| Error::InvalidRequirement(extra.clone()))?;
					for requirement in self.metadata_for_dist(name, version)?.requires_dist {
	...
	fn versions_for_name(&self, name: &str) -> Result<Vec<Version>> {
		...
		let versions = self
			.source
			.available_versions(&normalized, include_yanked)?;
					...
					&& requirement
						...
						.and_then(|version_or_url| match version_or_url {
							VersionOrUrl::VersionSpecifier(specifiers) => {
								Some(is_exact_equal_specifier(specifiers))
							},
	...
	fn ensure_pinned_git_requirement(
		&self,
		requirement: &Requirement,
		url: &str,
	) -> Result<PinnedCandidate> {
	...
	fn selected_artifact(
		...
		name: &str,
		version: &Version,
	) -> Result<(ResolvedArtifact, PackageKind)> {
		if let Some(candidate) = self
			.pinned_candidate(name)
			.filter(|candidate| &candidate.version == version)
		{
			return Ok((
				candidate.artifact.to_resolved(version, &candidate.kind),
		...
		if let Some(file) = artifacts.wheels.into_iter().find(wheel_installable_by_pon) {
	...
	fn record_rejects(&self, name: &str, version: &Version, artifacts: &ArtifactSet) {
		...
		if reasons.is_empty() {
			reasons
				.push("no pure Python wheel or source distribution is installable by pon".to_owned());
		}
	...
	fn raw_for_dist(&self, name: &str) -> String {
		...
		self
			...
			.find_map(|input| match input {
				ResolvedInput::Registry { raw, requirement }
					if requirement.name.as_ref() == normalized =>
				{
					Some(raw.clone())
				},
				ResolvedInput::Pinned { raw, candidate } if candidate.name == normalized => {
					Some(raw.clone())
				},
				_ => None,
			})
...
fn wheel_installable_by_pon(file: &ProjectFile) -> bool {
	matches!(file.kind, PackageKind::Pure)
}
fn source_distribution_allowed(artifacts: &ArtifactSet) -> bool {
	artifacts.sdist.is_some()
}

fn cabi_refusal_for_roots<S: CandidateSource>(
	...
	provider
		...
		.find_map(|((name, _version), reasons)| {
			...
			reasons
				.iter()
				.find(|reason| {
					reason.contains("C-ABI")
						|| reason.contains("ob_refcnt")
						|| reason.contains("CPython")
				})
				.map(|reason| {
					Error::UnsupportedArtifact(format!(
						"package `{name}` requires the CPython C-ABI (ob_refcnt): {reason}; this is a \
						 by-design limitation of pon"
					))
				})
		})
}
...
impl<'a, S: CandidateSource> DependencyProvider for PonProvider<'a, S> {
	...
	fn choose_version(
		...
		let name = package
			.dist_name()
			.expect("non-root package has a distribution name");
		...
		for version in versions.into_iter().rev() {
			...
			let artifacts = self.source.artifacts(name, &version)?;
			let has_installable_wheel = artifacts.wheels.iter().any(wheel_installable_by_pon);
			let has_installable_sdist = source_distribution_allowed(&artifacts);
		...
		RequirementInput::Pep508(requirement) => {
			...
			if let Some(VersionOrUrl::Url(url)) = &requirement.version_or_url {
				...
				if is_git_direct_url(&url) {
					let candidate = pin_git_url(&url)?;
					validate_pinned_requirement_name(&candidate, &requirement)?;
					return Ok(Some(ResolvedInput::Pinned { raw: raw.to_owned(), candidate }));
				}
				return Err(unsupported_direct_url(&url));
			}
			Ok(Some(ResolvedInput::Registry { raw: raw.to_owned(), requirement }))
		},
	}
}
...
fn validate_pinned_requirement_name(
...
fn apply_legacy_version_specifier(
		...
		&& requirement.version_or_url.is_none()
	{
		requirement.version_or_url =
			Some(VersionOrUrl::VersionSpecifier(parse_version_specifiers(version_specifier)?));
	}
	Ok(())
}
...
impl PinnedArtifact {
	fn to_resolved(&self, version: &Version, kind: &PackageKind) -> ResolvedArtifact {
		match self {
			Self::Dir { path, editable } => {
				ResolvedArtifact::Dir { path: path.clone(), editable: *editable }
			},
			Self::Sdist { path } => {
				ResolvedArtifact::Sdist(local_project_file(path, version.clone(), kind.clone()))
			},
			Self::Vcs { url, requested_rev, commit, dir } => ResolvedArtifact::Vcs {
				url:           url.clone(),
...
fn pin_local_path(path: &Path, editable: bool) -> Result<PinnedCandidate> {
	let (pyproject, artifact) = if path.is_dir() {
		...
		(PyProject::read(&manifest_path)?, PinnedArtifact::Dir { path: path.to_path_buf(), editable })
	} else if path
		.file_name()
	...
	let dependencies = pyproject
		...
		.map(|raw| match parse_requirement_input(&raw)? {
			...
			RequirementInput::Path { .. } | RequirementInput::Url { .. } => {
				Err(Error::InvalidRequirement(format!(
	...
	Ok(PinnedCandidate { name, version, kind, artifact, dependencies })
}
...
fn read_tar_gz_pyproject(path: &Path) -> Result<(String, String)> {
	let file = File::open(path).map_err(|_| {
		Error::InvalidRequirement(format!("unsupported local package source `{}`", path.display()))
	})?;
	...
	Err(Error::InvalidRequirement(format!("sdist `{}` is missing pyproject.toml", path.display())))
}
...
fn read_zip_pyproject(path: &Path) -> Result<(String, String)> {
	...
	for index in 0..archive.len() {
		let mut member = archive
			.by_index(index)
			.map_err(|error| zip_error(&label, error))?;
...
mod tests {
	...
	fn resolves_fastjson_pon_local_path_as_native() {
		...
		let fixture = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
			...
			.join("fastjson-pon");
		let record = provider
			.resolve_input(fixture.to_str().expect("fixture path"), "")
			.expect("record");
	...
	fn refuses_numpy_cabi_only_candidate() {
		...
		let error = provider
			.resolve_input("numpy", "")
			.expect_err("c-abi should be refused");
	...
	fn resolves_three_level_chain_from_sidecar_metadata() {
		...
		let resolved = provider
			.resolve_requirements(["pkg-a"].iter().copied())
			.expect("resolve");

		assert_eq!(
			resolved
				.iter()
				.map(|package| package.record.name.as_str())
				.collect::<Vec<_>>(),
			["pkg-c", "pkg-b", "pkg-a"]
		);
		assert_eq!(
			resolved
				.iter()
				.map(|package| package.record.version.as_str())
				.collect::<Vec<_>>(),
			["1.0.0", "1.0.0", "1.0.0"]
		);
	}
	...
	fn cached_chain_index() -> SimpleJsonIndex {
		...
		for (url, metadata) in [
			...
			(
				...
				"Metadata-Version: 2.3\nName: pkg-b\nVersion: 1.0.0\nRequires-Dist: pkg-c; \
				 implementation_name == 'pon'\nRequires-Dist: skipped; implementation_name == \
				 'cpython'\n",
			),
			(
	...
	impl CandidateSource for StaticSource {
		...
		fn artifacts(&self, name: &str, version: &Version) -> Result<ArtifactSet> {
			Ok(self
				...
				.map(|candidate| ArtifactSet { wheels: vec![candidate.artifact.clone()], sdist: None })
				.unwrap_or_default())
		}
	...
	fn project_file(name: &str, version: &Version) -> ProjectFile {
		ProjectFile {
			...
			url:                     format!(
				"https://files.example/{}-{}-py3-none-any.whl",
				name.replace('-', "_"),
				version
			),
			version:                 version.clone(),
			...
			requires_python_invalid: false,
			yanked:                  None,
			dist_info_metadata:      Some(DistInfoMetadata { hashes: BTreeMap::new() }),
		}
	}
	...
	fn temp_project(label: &str) -> PathBuf {
		let unique = SystemTime::now()
			...
			.expect("clock")
			.as_nanos();
		std::env::temp_dir().join(format!("pon-resolve-{label}-{unique}"))
	}
}

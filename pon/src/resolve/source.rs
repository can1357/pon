use std::{cell::RefCell, collections::HashMap, fs::File, io::Read, path::Path};
...
use crate::{
	error::{Error, Result},
	index::{PackageIndex, ProjectFile, ProjectPage},
...
impl<'a, I: PackageIndex> IndexSource<'a, I> {
	...
	pub fn new(index: &'a I) -> Self {
		Self { index, pages: RefCell::new(HashMap::new()) }
	}
...
fn read_zip_sdist_metadata(file: &ProjectFile, path: &Path) -> Result<String> {
	...
	for index in 0..archive.len() {
		let mut member = archive
			.by_index(index)
			.map_err(|error| zip_sdist_error(file, error))?;
		let member_name = member.name().to_owned();
		let filename = Path::new(&member_name)
			.file_name()

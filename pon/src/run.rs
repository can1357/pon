//!Library-first entry points for running Pon programs.
...
use pon_runtime::{
	...
	import::{
		...
		end_module_execution, install_module, set_source_module_loader,
	},
	intern, pon_const_str, pon_none, pon_runtime_init, pon_sys_set_argv,
};
...
/// Runs one Pon/Python source file through the JIT backend using the current
/// process environment.
///
...
pub(crate) fn run_module_as_main(module: &str, extra_args: Vec<String>) -> Result<()> {
	...
	let (path, package) = if is_package {
		...
		let Some((main_path, false)) = resolve(&format!("{module}.__main__")) else {
			bail!(
				"No module named {module}.__main__; '{module}' is a package and cannot be directly \
				 executed"
			);
		};
...
fn run_file_inner_with_package(
	path: &Path,
	argv: &[String],
...
fn execute_main(
	...
	if let Some(package) = main_package.filter(|package| !package.is_empty()) {
		...
		pon_runtime::import::store_module_attr(
			intern("__main__"),
			intern("__package__"),
			package_object,
		);
	...
	V: Into<OsString>,
{
	...
	let cwd =
		env::current_dir().context("cannot resolve the current directory for inline execution")?;
...
fn load_source_module(
	request: SourceModuleRequest<'_>,
) -> std::result::Result<*mut pon_runtime::PyObject, String> {
	let module = lower_source(request.source).map_err(|error| {
		format!("failed to parse/lower source module '{}': {error}", request.path.display())
	})?;
	let mut engine = JitEngine::new();
...
fn validate_dynamic_source(request: DynCompileRequest<'_>) -> std::result::Result<(), String> {
	...
	lower_source(&source).map(|_| ()).map_err(|error| {
		format!("failed to parse/lower dynamic source '{}': {error}", request.filename)
	})
}

fn execute_dynamic_source(
	request: DynExecuteRequest<'_>,
) -> std::result::Result<*mut PyObject, String> {
	let source = dynexec_source(request.source, request.mode);
	let module = lower_source(&source).map_err(|error| {
		format!("failed to parse/lower dynamic source '{}': {error}", request.filename)
	})?;
	let mut engine = JitEngine::new();
	engine.run(&module).map_err(|error| {
...
/// Compiles and runs one interactive entry inside the caller's active
/// `__main__` execution.
...
/// Runs one Pon/Python source file with additional environment visible to the
/// runtime.
///

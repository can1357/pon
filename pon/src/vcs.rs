...
/// Fetch a git repository into pon's VCS cache and return the selected checkout.
///
...
/// updated with `git fetch --tags`. The selected revision is `rev` when supplied,
/// otherwise any `@rev` embedded in `url`, otherwise the remote default branch.
/// The returned [`GitCheckout::dir`] is validated after applying an optional
...
pub fn fetch_git(cache_root: &Path, url: &str, rev: Option<&str>) -> Result<GitCheckout> {
    ...
    let repo_dir = cache_root.join("git").join(sha256_hex(reference.clone_url.as_bytes()));
    let parent = repo_dir.parent().ok_or_else(|| {
        Error::Cli(format!(
            "could not determine git cache directory for VCS requirement `{url}`"
        ))
    })?;
    ...
    if repo_dir.exists() {
        ...
        run_git(
            git_command().arg("-C").arg(&repo_dir).arg("fetch").arg("--tags"),
            url,
        )?;
    } else {
        run_git(
    ...
    match selected_rev.as_deref() {
        Some(revision) => {
            run_git(
                git_command().arg("-C").arg(&repo_dir).arg("checkout").arg(revision),
                url,
            )?;
        }
        None => {
    ...
    let output = run_git(
        git_command().arg("-C").arg(&repo_dir).arg("rev-parse").arg("HEAD"),
        url,
    )?;
    let commit = String::from_utf8_lossy(&output.stdout).trim().to_owned();
    if commit.is_empty() {
        return Err(Error::Cli(format!(
            "git did not report a commit for VCS requirement `{url}`"
        )));
    }
...
fn revision_separator_index(url: &str) -> Option<usize> {
    ...
    let path_start = url[scheme_end..].find('/').map(|offset| scheme_end + offset)?;
...
mod tests {
    ...
    fn rejects_absolute_or_escaping_subdirectories() {
        ...
        assert!(parse_git_reference("git+https://example.test/repo.git#subdirectory=../pkg").is_err());
    }
}

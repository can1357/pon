...
use self::{
	compat::{
		WheelCompatibility, classify_archive_member, classify_root_is_purelib, classify_tags,
		default_supported_tags,
	},
	filename::WheelFilename,
	record::{RecordEntry, parse_record, write_record},
	scripts::{EntryPoint, parse_entry_points},
};
use crate::{
	env::EnvLayout,
	error::{Error, Result},
	install::{
		InstallReport, InstalledPackageRecord, ResolvedRecord, direct_url::DirectUrl,
...
pub fn install_wheel(
	...
	let parsed_record_entries =
		parse_record(&inspection.record_text, &inspection.record_path.display().to_string())?;
	let dist_info_dir = dist_info_dir(&inspection.record_path, filename)?;
	let record_path = dist_info_dir.join("RECORD");

	env.create_dirs()?;
	let mut installed_record_entries =
		extract_archive(env, filename, &wheel, &mut archive, &parsed_record_entries, &record_path)?;
	...
	if let Some(entry_points_text) = &inspection.entry_points_text {
		let entry_points =
			parse_entry_points(entry_points_text, &format!("{filename} entry_points.txt"))?;
		for entry_point in entry_points
			.console_scripts
			.iter()
			.chain(entry_points.gui_scripts.iter())
		{
			for script_entry in write_entry_point_scripts(env, filename, entry_point)? {
...
fn inspect_archive(filename: &str, archive: &mut ZipArchive<File>) -> Result<WheelInspection> {
	...
	for index in 0..archive.len() {
		let mut member = archive
			.by_index(index)
			.map_err(|error| zip_error(filename, error))?;
...
fn extract_regular_member(
	...
	Ok(record_entry_from_digest(record_path_for_destination(env, destination)?, copied, &digest))
}
...
fn extract_script_member(
	...
	Ok(RecordEntry::from_bytes(record_path_for_destination(env, destination)?, &installed))
}

fn verify_recorded_member(
	filename: &str,
	record_entry: &RecordEntry,
	reader: &mut impl Read,
) -> Result<()> {
...
fn archive_destination(
	...
	match scheme {
		...
		"headers" => Ok(env
			.pon_dir
			.join("include")
			.join(&wheel.normalized_distribution)
			.join(rest)),
...
fn data_scheme<'a>(wheel: &WheelFilename, member_name: &'a str) -> Option<&'a str> {
	data_member_parts(wheel, member_name)
		.ok()
		.flatten()
		.map(|(scheme, _)| scheme)
}
...
fn write_entry_point_scripts(
	...
	Ok(vec![
		write_recorded_file(
			&script_path,
			record_path_for_destination(env, &script_path)?,
			script_content.as_bytes(),
			true,
		)?,
		write_recorded_file(
			&launcher_path,
			record_path_for_destination(env, &launcher_path)?,
			launcher_content.as_bytes(),
			false,
		)?,
	])
}
...
fn python_launcher_content(
	entry_point: &EntryPoint,
	script_dir: &Path,
	site_packages: &Path,
) -> String {
	let root_module = entry_point
		.module
		.split('.')
		.next()
		.unwrap_or(&entry_point.module);
...
fn verify_record_entry(
	...
	{
		return Err(Error::UnsupportedArtifact(format!(
			"wheel `{filename}` member `{}` failed RECORD size verification: expected {size}, got \
			 {copied}",
			record_entry.path
		)));
	}
...
fn cabi_refused(filename: &str, reason: &str) -> Error {
	Error::UnsupportedArtifact(format!(
		"wheel `{filename}` requires the CPython C-ABI (ob_refcnt): {reason}; this is a by-design \
		 limitation of pon"
	))
}
...
mod tests {
	...
	fn installs_idna_pure_wheel_by_extracting_fixture_and_registry_record() {
		...
		assert_eq!(registry[0].record_path.as_deref(), Some(Path::new("idna-3.10.dist-info/RECORD")));
	}
	...
	fn installs_flit_core_pure_wheel_by_extracting_fixture() {
		...
		let record =
			ResolvedRecord::wheel("flit-core", "3.12.0", "flit_core-3.12.0-py3-none-any.whl");

		install_wheel(&layout, &record, &wheel_path, None).expect("install");

		let marker = fs::read_to_string(layout.site_packages.join("flit_core").join("__init__.py"))
			.expect("marker");
		assert!(marker.contains("__version__ = \"3.12.0\""));
	}

	#[test]
	fn writes_entry_point_shell_shim_and_python_launcher() {
		let layout = EnvLayout::new(temp_project("entry-point-shim"));
		let entry_point = EntryPoint {
			name:   "demo".to_owned(),
			module: "demo.cli".to_owned(),
			attr:   "main".to_owned(),
		};

		let entries = write_entry_point_scripts(&layout, "demo-1.0-py3-none-any.whl", &entry_point)
			.expect("scripts");

		let script_path = layout.scripts_dir.join("demo");
		let launcher_path = layout.scripts_dir.join("demo.py");
	...
	fn remove_round_trip_deletes_record_files_and_empty_parents() {
		...
		assert!(
			read_installed_packages(&layout)
				.expect("registry")
				.is_empty()
		);

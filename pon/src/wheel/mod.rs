...
fn write_entry_point_scripts(env: &EnvLayout, filename: &str, entry_point: &EntryPoint) -> Result<Vec<RecordEntry>> {
...
mod tests {
    ...
    fn writes_entry_point_shell_shim_and_python_launcher() {
        ...
        let entries = write_entry_point_scripts(&layout, "demo-1.0-py3-none-any.whl", &entry_point).expect("scripts");
        ...
        #[cfg(unix)]
        assert_ne!(fs::metadata(script_path).expect("metadata").permissions().mode() & 0o111, 0);
    }

...
def _use_ansi_escapes() -> bool:
    ...
    print('ENTER_USE_ANSI', 'stdout=', sys.stdout, 'reconfigure=', getattr(sys.stdout, 'reconfigure', None), 'isatty=', getattr(sys.stdout, 'isatty', None))
    ...
    print('BEFORE_STDOUT_RECONFIGURE')
    sys.stdout.reconfigure(errors='replace')  # type: ignore[attr-defined]
    print('AFTER_STDOUT_RECONFIGURE')

    print('BEFORE_NO_COLOR_CHECK')
    if 'NO_COLOR' in os.environ:
        return False

    print('BEFORE_FORCE_COLOR_CHECK', os.environ.get('TERM'), sys.stdout.isatty())
...
def _log(string: str , **kwargs: Any) -> None:
    print('ENTER_LOG', repr(string), 'kwargs=', kwargs, '_use_type=', type(_use_ansi_escapes), repr(_use_ansi_escapes), 'call_attr=', getattr(_use_ansi_escapes, '__call__', None))
    _pon_use_ansi = _use_ansi_escapes()
    print('AFTER_USE_ANSI_CHECK', _pon_use_ansi)
    if not _pon_use_ansi:
        string = style.strip(string)
        print('AFTER_STRIP', repr(string))
    print('BEFORE_PRINT_LOG')
    print(string, **kwargs)
    print('AFTER_PRINT_LOG')
...
def _validate_pyproject_config(pyproject: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _table(scheme: Dict[str, Callable[[Any, str], Any]]) -> Callable[[Any, str], Dict[str, Any]]:
        def func(value: Any, name: str) -> Dict[str, Any]:
            ...
            for key, val in value.items():
                ...
                print('VALIDATE_TABLE', name, key, 'check_type=', type(check), 'check_repr=', repr(check), 'val_type=', type(val), 'val_repr=', repr(val))
                if check is None:
    ...
    def _strings(value: Any, name: str) -> List[str]:
        print('ENTER_STRINGS', name, 'value=', repr(value), 'is_list=', isinstance(value, list), 'all_type=', type(all), repr(all))
        _pon_all_result = all(isinstance(x, str) for x in value)
        print('STRINGS_ALL_RESULT', _pon_all_result)
        if not isinstance(value, list) or not _pon_all_result:
            raise ConfigError(f'Configuration entry "{name}" must be a list of strings')
        print('LEAVE_STRINGS', name)
        return value
...
class Project():
    ...
    def __init__(
        ...
        print('ABOUT_DEFAULTDICT', 'collections.defaultdict=', collections.defaultdict, 'list=', list)
        self._meson_args: MesonArgs = collections.defaultdict(list)
        print('MADE_DEFAULTDICT', type(self._meson_args), repr(self._meson_args), 'factory=', getattr(self._meson_args, 'default_factory', '<missing>'))
        ...
        print('PYPROJECT_CONFIG_DONE', repr(pyproject_config))
        for key, value in pyproject_config.get('args', {}).items():
            print('EXTENDING_MESON_ARGS', key, value, 'target_type_before=', type(self._meson_args[key]), repr(self._meson_args[key]), 'extend=', getattr(self._meson_args[key], 'extend', None))
            self._meson_args[key].extend(value)
            print('EXTENDED_MESON_ARGS', key, repr(self._meson_args[key]))
        ...
        print('BEFORE_GET_MESON_COMMAND', pyproject_config.get('meson'), '_get_type=', type(_get_meson_command), repr(_get_meson_command))
        self._meson = _get_meson_command(pyproject_config.get('meson'))
        print('AFTER_GET_MESON_COMMAND', repr(self._meson))

        print('BEFORE_NINJA', type(_env_ninja_command), repr(_env_ninja_command))
        self._ninja = _env_ninja_command()
        print('AFTER_NINJA', repr(self._ninja))
        if self._ninja is None:
            raise ConfigError(f'Could not find ninja version {_NINJA_REQUIRED_VERSION} or newer.')
        print('BEFORE_ENV_SETDEFAULT', type(os.environ), repr(getattr(os.environ, 'setdefault', None)))
        os.environ.setdefault('NINJA', self._ninja)
        print('AFTER_ENV_SETDEFAULT')

        # make sure the build dir exists
        print('BEFORE_MKDIR', self._build_dir)
        self._build_dir.mkdir(exist_ok=True, parents=True)
        print('AFTER_MKDIR')

        # if the build dir is empty, add .gitignore and .hgignore files
        print('BEFORE_ANY_ITERDIR', self._build_dir)
        if not any(self._build_dir.iterdir()):
            _add_ignore_files(self._build_dir)
        print('AFTER_EMPTY_CHECK')

        # setuptools-like ARCHFLAGS environment variable support
        print('BEFORE_SYS_PLATFORM', 'sysconfig.get_platform=', sysconfig.get_platform, 'result=', sysconfig.get_platform(), 'startswith=', getattr(sysconfig.get_platform(), 'startswith', None))
        if sysconfig.get_platform().startswith('macosx-'):
            archflags = os.environ.get('ARCHFLAGS', '').strip()
            print('MACOS_BRANCH_ARCHFLAGS', repr(archflags), 'strip_method=', getattr(os.environ.get('ARCHFLAGS', ''), 'strip', None))
            if archflags:
        ...
        print('AFTER_PLATFORM_CHAIN')
        # write the native file
        print('BEFORE_NATIVE_DEDENT', type(textwrap.dedent), repr(textwrap.dedent), 'sys.executable=', sys.executable)
        native_file_data = textwrap.dedent(f'''
            [binaries]
            python = '{sys.executable}'
        ''')
        print('AFTER_NATIVE_DEDENT', repr(native_file_data))
        self._meson_native_file.write_text(native_file_data, encoding='utf-8')
        print('AFTER_NATIVE_WRITE')
        ...
        print('BEFORE_RECONFIGURE_CHECK', self._build_dir, 'joinpath=', getattr(self._build_dir, 'joinpath', None))
        # goes wrong during setup.
        reconfigure = self._build_dir.joinpath('meson-private/coredata.dat').is_file()
        print('AFTER_RECONFIGURE_CHECK', reconfigure)

        print('BEFORE_CONFIGURE_CALL', type(getattr(self, '_configure', None)), repr(getattr(self, '_configure', None)))
        # run meson setup
        self._configure(reconfigure=reconfigure)
        print('AFTER_CONFIGURE_CALL')
    ...
    def _run(self, cmd: Sequence[str]) -> None:
        """Invoke a subprocess."""
        print('ENTER_RUN', repr(cmd), 'log=', type(_log), repr(_log), 'format=', getattr('{style.INFO}+ {cmd}{style.RESET}', 'format', None), 'join=', getattr(' ', 'join', None))
        ...
        print('AFTER_LOG_BEFORE_SUBPROCESS', 'subprocess.run=', subprocess.run, 'cwd=', self._build_dir)
        r = subprocess.run(cmd, cwd=self._build_dir)
        print('AFTER_SUBPROCESS', getattr(r, 'returncode', '<none>'))
        if r.returncode != 0:
            raise SystemExit(r.returncode)

    def _configure(self, reconfigure: bool = False) -> None:
        """Configure Meson project."""
        print('ENTER_CONFIGURE', reconfigure, 'meson_args=', repr(self._meson_args), 'setup_existing=', self._meson_args.get('setup', '<missing>'))
        print('BEFORE_SETUP_ARGS_LIST', 'source=', self._source_dir, 'build=', self._build_dir, 'setup_type=', type(self._meson_args['setup']), repr(self._meson_args['setup']))
        setup_args = [
        ...
        print('AFTER_SETUP_ARGS_LIST', repr(setup_args))
        if reconfigure:
            setup_args.insert(0, '--reconfigure')
        print('BEFORE_RUN_CONFIGURE', repr(self._meson + ['setup', *setup_args]), 'run=', getattr(self, '_run', None))

...
def _map_to_wheel(
    ...
    print('MP _map_to_wheel sources', list(sources.keys()), flush=True)
...
class Project():
    ...
    def __init__(
        ...
        print('MP Project.__init__ start', source_dir, build_dir, flush=True)
        ...
        print('MP before _get_meson_command', flush=True)
        self._meson = _get_meson_command(pyproject_config.get('meson'))
        print('MP after _get_meson_command', self._meson, flush=True)

        print('MP before _env_ninja_command', flush=True)
        self._ninja = _env_ninja_command()
        print('MP after _env_ninja_command', self._ninja, flush=True)
        ...
        print('MP before build_dir.mkdir', self._build_dir, flush=True)
        self._build_dir.mkdir(exist_ok=True, parents=True)
        print('MP after build_dir.mkdir', flush=True)
    ...
    def _run(self, cmd: Sequence[str]) -> None:
        print('MP _run enter', cmd, flush=True)
        """Invoke a subprocess."""
    ...
    def _configure(self, reconfigure: bool = False) -> None:
        print('MP _configure enter', reconfigure, flush=True)
    ...
    def build(self) -> None:
        print('MP build enter', flush=True)
    ...
    def _manifest(self) -> DefaultDict[str, List[_Entry]]:
        print('MP _manifest enter', flush=True)
    ...
    def wheel(self, directory: Path) -> pathlib.Path:
        ...
        print('MP wheel enter', directory, flush=True)
        self.build()
        print('MP wheel after build', flush=True)
        builder = _WheelBuilder(self._metadata, self._manifest, self._limited_api, self._allow_windows_shared_libs)
        print('MP wheel after builder', flush=True)
...
def _project(config_settings: Optional[Dict[Any, Any]] = None) -> Iterator[Project]:
    ...
    print('MP _project enter', config_settings, flush=True)
    with contextlib.ExitStack() as ctx:
        if build_dir is None:
            print('MP before TemporaryDirectory', source_dir, flush=True)
            build_dir = ctx.enter_context(tempfile.TemporaryDirectory(prefix='.mesonpy-', dir=source_dir))
            print('MP after TemporaryDirectory', build_dir, flush=True)
        print('MP before yield Project', flush=True)
        try:
            yield Project(source_dir, build_dir, meson_args, editable_verbose)
        finally:
            print('MP after yield Project', flush=True)
...
def build_wheel(
    ...
    print('MP build_wheel enter', wheel_directory, config_settings, metadata_directory, flush=True)
